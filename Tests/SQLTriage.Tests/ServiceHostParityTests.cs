...
namespace SQLTriage.Tests
{
    ...
    public class ServiceHostParityTests
    {
        ...
        private static readonly Dictionary<string, string> DesktopOnly = new(StringComparer.Ordinal)
        {
            ...
            // 2026-09-05: DashboardConfigService, IDbConnectionFactory and liveQueriesTableService
            // were excused here as the three arguments to the desktop startup pass that provisioned
            // a local table per dashboard panel. That pass and the service behind it are deleted, so
            // App.xaml.cs resolves none of the three at startup and all three excuses were dead text
            // — which is the defect The_desktop_only_list_carries_no_stale_entries exists to catch.
            // It caught them. The two interfaces are still registered; they are simply no longer
            // resolved in the startup block this guard scans.
