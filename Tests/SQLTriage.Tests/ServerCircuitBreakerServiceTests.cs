...
namespace SQLTriage.Tests
{
    public class ServerCircuitBreakerServiceTests : IDisposable
    {
        ...
        // Invariant B (2026-09-19): every failure carries its cause. These tests are about the
        // back-off schedule, so the cause is a real no-answer: a transport error at severity 20.
        private static readonly Exception NoAnswer = ServerAnswerClassifierTests.MakeSqlException((11001, 20));
        ...
        public void AfterThreeFailures_ShouldAttemptReturnsFalse()
        {
            ...
            svc.RecordFailure("srv1", NoAnswer);
            svc.RecordFailure("srv1", NoAnswer);
            svc.RecordFailure("srv1", NoAnswer);
