...
public class ServiceCatalogHonestyTests
{
    ...
    public void The_shipped_default_config_really_does_disable_updates()
    {
        ...
        // config.default\ first since 2026-09-10 (lane customer-update-path): the SHIPPED default now
        // lives there and config\appsettings.json is the operator's copy, seeded from it on first run.
        // This test is about the shipped default, so it reads that; config\ stays as the fallback for a
        // tree that predates the split.
        // This was the ONE site the 2026-09-10 lane repaired by hand; the other six copies of the same
        // probe were left behind and went blind. It now calls the shared resolver, which probes in this
        // same order, so a ninth copy of the folder list cannot drift from it (ShippedConfig.cs).
        var path = ShippedConfig.TryPath("appsettings.json");
        if (path is null)
        {
            _out.WriteLine("appsettings.json not in the test output (probed "
                           + string.Join(", ", ShippedConfig.ProbeOrder) + " under "
                           + AppContext.BaseDirectory + ") - premise not re-checked here.");
            return;
        }
