...
public class AlertNeverFiresTests
{
    ...
    [InlineData("response_time_probe", "Duration")]          // MeasureResponseTimeAsync: elapsed ms, a band (lane Q14)
    ...
    /// <summary>
    /// The shape reader answers for what the evaluator RUNS (lane Q14 fix round, 2026-09-18). The
    /// alert editor prints <c>QueryValueShape</c> under the threshold box. For an alert routed to a
    /// built-in handler the query field is text nobody executes, and reading it told operators that
    /// <c>instance_unreachable</c> and <c>machine_unreachable</c> (both Critical, inert
    /// <c>SELECT 1 AS value</c>) could never fire and should be turned off. Enumerated from the
    /// shipped file with the evaluator's own routing predicate, so a new routed alert is covered
    /// without an edit here. Non-vacuous: the same inert text with the routing removed still reads
    /// Constant, so the text alone would have misled.
    /// </summary>
    [Fact]
    public void The_shape_reader_answers_for_the_handler_on_every_routed_alert()
    {
        var routed = Shipped().Where(AlertEvaluationService.IsRoutedToBuiltInHandler).ToList();
        routed.Should().HaveCountGreaterThanOrEqualTo(8, "eight shipped alerts are routed to a built-in handler");
        routed.Select(a => a.Id).Should().Contain(new[] { "instance_unreachable", "machine_unreachable", "sql_response_time" });

        foreach (var a in routed)
            AlertEvaluationService.QueryValueShape(a).Should().Be(
                AlertEvaluationService.SpecialHandlerValueShape(a.QueryMode),
                a.Id + " is evaluated by its " + a.QueryMode + " handler, so its query text says nothing about its range");

        foreach (var id in new[] { "instance_unreachable", "machine_unreachable" })
            AlertEvaluationService.QueryValueShape(routed.Single(a => a.Id == id))
                .Should().NotBe(AlertEvaluationService.AlertValueShape.Constant,
                    id + " can fire, so the editor must not call it a fixed number");

        // The control: the same inert text, routed and not.
        AlertEvaluationService.QueryValueShape(new AlertDefinition { Query = "SELECT 1 AS value", QueryMode = "connectivity_check" })
            .Should().Be(AlertEvaluationService.AlertValueShape.BooleanFlag);
        AlertEvaluationService.QueryValueShape(new AlertDefinition { Query = "SELECT 1 AS value", QueryMode = null })
            .Should().Be(AlertEvaluationService.AlertValueShape.Constant);

        var srt = routed.Single(a => a.Id == "sql_response_time");
        srt.Query.Should().BeEmpty(
            "a query field nobody executes ships empty, as deadlock's does, so no reader of query text can run it or judge it");
        AlertEvaluationService.QueryValueShape(srt).Should().Be(AlertEvaluationService.AlertValueShape.Duration);
    }
