...
namespace SQLTriage.Tests.Build
{
    ...
    public sealed class ProfileGatedTestSyncTests
    {
        ...
        public void No_test_compiled_under_community_binds_a_symbol_that_build_removes()
        {
            ...
            Assert.True(surface.RemovedPages.Count > 0,
                "the community gated-page set came back empty — the model is not reading the " +
                "<Content Remove=\"...razor\"> entries in buildprofile.targets, and a guard that " +
                "scans for nothing must fail, not pass.");
            ...
            // A Content-Removed .razor page is a gated type too -- Razor generates a class named for
            // the file (Pages/Sessions.razor -> SQLTriage.Pages.Sessions) that a community build never
            // compiles. Missed until 2026-08-21: SessionsDisposeLifecycleTests bound typeof(SQLTriage.
            // Pages.Sessions) and broke the community build, though RemovedPages had been collected all
            // along. Filed as a full qualified NAME (not a bare class name) in gatedNamespaces: a bare
            // "Services" or "Remediation" collides with unrelated shipped namespace segments and was
            // proven, by mutation, to flood this scan with false positives.
            foreach (var qn in ProfileGateModel.QualifiedTypeNamesFromRemovedPages(surface.RemovedPages))
                gatedNamespaces.Add(qn);
