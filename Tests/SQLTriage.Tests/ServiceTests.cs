...
public class ChartThemeServiceTests
{
    private class FakeUserSettings : IUserSettingsService
    {
        ...
        public int GetAuditMaxConcurrentPerInstance() => UserSettingsService.AuditConcurrencyDefault;
    ...
    public void GetCurrentPalette_DefaultTheme_ReturnsExpectedColors()
    {
        ...
        Assert.Equal("#5BE58C", palette.Success);   // canonical ok green (--green-400)
        Assert.Equal("#F2B24B", palette.Warning);   // --warn-400
        Assert.Equal("#FF6B5E", palette.Critical);  // --danger-400
        Assert.Equal("#66756F", palette.Neutral);
    }
    ...
    public void GetSeriesPalette_ReturnsFixedAccentPalette()
    {
        ...
        // Pin the full ordered categorical palette: signal green leads, cyan second.
        Assert.Equal(
            new[] { "#34D17A", "#3DD6CB", "#A78BFA", "#F2B24B", "#FF6B5E", "#8FF5BE", "#F472B6", "#7FF0E6" },
            series);
    }
