...
namespace SQLTriage.Tests
{
    ...
    public class RbacPageGateCensusTests
    {
        ...
        // ── Round 10, 2026-08-26 (the alerts honesty lane, finding alerts-r2-10) ─────────

        /// <summary>
        /// The five Definitions-tab writes on <c>Pages/Alerts.razor</c> gated on
        /// <c>manage_alerts</c> this round, each with the permission it was gated on. Same
        /// discipline as <see cref="RoundEightGatedHandlers"/>: pinned by NAME so an un-gate fails
        /// here, and by PERMISSION so a re-gate onto a weaker one fails too.
        ///
        /// <para>These were INLINE <c>@onchange</c> lambdas that called
        /// <c>Definitions.UpdateAlert</c> directly — warning threshold, critical threshold, frequency
        /// seconds, next-alert delay and the send-email checkbox — in the same table row as a
        /// Severity select that was already <c>disabled="@(!MayManageAlerts)"</c>. A Viewer or
        /// Operator could raise every threshold (silencing the alert) from a page telling them
        /// severity was Admin-only. A lambda has no name, so the by-name
        /// <c>RbacRound8AlertsHandlerGateTests</c> census never enumerated them, and the per-handler
        /// net here resolves a lambda's CALLED names to their declarations — and
        /// <c>Definitions.UpdateAlert</c> has none in this page — so it skipped them too. They are now
        /// named handlers gated exactly like <c>OnSeverityChanged</c>, and
        /// <see cref="No_inline_alerts_lambda_performs_an_ungated_write"/> pins the SHAPE so the blind
        /// spot cannot reopen. EIGHTH TIME OF ASKING, same lesson (gate-change-shapes-are-a-blind-spot),
        /// a new axis: the binding is a lambda that does the write itself rather than delegating.</para>
        /// </summary>
        private static readonly (string Key, string Permission)[] RoundTenGatedHandlers =
        {
            ("Pages/Alerts.razor::OnWarningThresholdChanged",  "manage_alerts"),
            ("Pages/Alerts.razor::OnCriticalThresholdChanged", "manage_alerts"),
            ("Pages/Alerts.razor::OnFrequencySecondsChanged",  "manage_alerts"),
            ("Pages/Alerts.razor::OnNextAlertDelayChanged",    "manage_alerts"),
            ("Pages/Alerts.razor::OnSendEmailChanged",         "manage_alerts"),
        };

        [Fact]
        public void TheRoundTenDischargeHolds()
        {
            var live = CensusMutatingHandlers().Select(f => f.Key).ToHashSet(StringComparer.Ordinal);

            var regressed = RoundTenGatedHandlers
                .Where(h => live.Contains(h.Key))
                .Select(h => h.Key + " (was gated on " + h.Permission + ")")
                .ToList();

            Assert.True(regressed.Count == 0,
                "Round 10 gated these alerts-r2-10 handlers and the census can see them ungated again:\n  "
                + string.Join("\n  ", regressed));
        }

        [Theory]
        [MemberData(nameof(RoundTenHandlers))]
        public void EachRoundTenHandlerIsGatedOnItsRuledPermission(string key, string permission)
        {
            var parts = key.Split("::", StringSplitOptions.None);
            var (markup, code) = ReadSources(parts[0]);
            Assert.True(markup != null, parts[0] + " is gone; move the entry with the file or drop it.");
        ...
        public static IEnumerable<object[]> RoundTenHandlers =>
            RoundTenGatedHandlers.Select(h => new object[] { h.Key, h.Permission });

        /// <summary>
        /// THE INLINE-LAMBDA SHAPE, pinned so alerts-r2-10 cannot reopen. The per-handler census
        /// resolves a markup lambda to the NAMED methods it calls and analyses THEIR declarations; a
        /// dotted service write performed inside the lambda itself
        /// (<c>Definitions.UpdateAlert(alert)</c>) has no local declaration, so it is skipped and
        /// never asked whether it is gated. This reads the shipped Alerts markup and, over every
        /// event-binding LAMBDA, runs the SAME <see cref="MutatingCalls"/> net the census uses: a
        /// lambda that performs a dotted mutating call in its own body — rather than delegating to a
        /// named handler the census can see and gate — is a finding. After alerts-r2-10 every such
        /// write delegates to a gated named handler, so the offender list is empty; a future inline
        /// write reopens it here rather than silently in production.
        ///
        /// <para>Scoped to <c>Pages/Alerts.razor</c>, the page finding alerts-r2-10 names. A
        /// tree-wide inline-lambda arm (folding this into <see cref="AnalysePage"/>) is a larger,
        /// separate change to the delicate analyser and is left as a follow-up.</para>
        /// </summary>
        [Fact]
        public void No_inline_alerts_lambda_performs_an_ungated_write()
        {
            var markup = Strip(File.ReadAllText(
                Path.Combine(RawPassedScan.RepoRoot().FullName, "Pages", "Alerts.razor")));

            var offenders = InlineLambdaWrites(markup);

            Assert.True(offenders.Count == 0,
                "These Alerts.razor event bindings perform a persisted write INSIDE an inline lambda, "
                + "where no by-name census can see the call and gate it. Move each write into a NAMED "
                + "handler that checks MayManageAlerts (as OnSeverityChanged and the r2-10 handlers do), "
                + "so the lambda only delegates:\n  " + string.Join("\n  ", offenders));
        }

        /// <summary>
        /// MUTATION for <see cref="No_inline_alerts_lambda_performs_an_ungated_write"/>: a delegating
        /// lambda is clean, an inline dotted write is caught, and a lambda whose only call is bare
        /// (a local handler the census resolves on its own) is not a false positive.
        /// </summary>
        [Fact]
        public void TheInlineLambdaShapeNetCatchesADirectWriteAndNotADelegation()
        {
            const string delegating =
                @"<input @onchange=""(e) => OnWarningThresholdChanged(alert, e)"" />";
            const string inlineWrite =
                @"<input @onchange=""(e) => { alert.Thresholds.Warning = 1; Definitions.UpdateAlert(alert); }"" />";
            const string bareLocal =
                @"<input @onchange=""(e) => EditAlert(alert)"" />";

            Assert.Empty(InlineLambdaWrites(Strip(delegating)));
            Assert.Empty(InlineLambdaWrites(Strip(bareLocal)));

            var caught = InlineLambdaWrites(Strip(inlineWrite));
            Assert.True(caught.Count == 1, "got: " + string.Join(" | ", caught));
            Assert.Contains("Definitions.UpdateAlert", caught[0]);
        }

        /// <summary>Every event-binding lambda whose OWN body performs a dotted mutating call, with
        /// no gate inside the lambda. A bare-name binding (<c>@onchange="X"</c>) and a lambda that
        /// only delegates to a named method produce no dotted mutating call and are not
        /// findings.</summary>
        private static List<string> InlineLambdaWrites(string markup)
        {
            var offenders = new List<string>();
            foreach (Match b in EventBinding.Matches(markup))
            {
                var value = b.Groups[1].Value;
                if (!value.Contains("=>", StringComparison.Ordinal)) continue;   // bare name: the by-name census resolves it
                if (AnyGate.IsMatch(value)) continue;                            // an inline gate, unusual but covered
                var calls = MutatingCalls(value).ToList();
                if (calls.Count == 0) continue;
                offenders.Add(value.Trim() + "  ->  " + string.Join(", ", calls));
            }
            return offenders;
        }
