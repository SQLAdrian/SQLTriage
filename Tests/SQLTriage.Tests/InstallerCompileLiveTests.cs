...
//   2. The .iss compiles against a COMMUNITY publish shape, which has no BPScripts\ folder.
...
// only BPScripts\ is community-conditional. So this harness cannot see the one case the deliberately
...
public class InstallerCompileLiveTests
{
    ...
    private static string BuildStubPublishTree(string root, bool community)
    {
        ...
        foreach (var d in new[]
                 {
                     src, Path.Combine(src, "Config"), Path.Combine(src, "wwwroot"),
                     Path.Combine(src, "scripts"), Path.Combine(src, "Assets", "brand"),
                     Path.Combine(src, "docs", "compliance"),
                 })
            Directory.CreateDirectory(d);
        if (!community) Directory.CreateDirectory(Path.Combine(src, "BPScripts"));
        ...
        // matching ...\docs\compliance\*" and exit 2 - measured 2026-08-28, and exactly the loud
        // failure that line exists to produce.
        Stub(Path.Combine("docs", "compliance", "incident-response-runbook.md"), "# stub runbook");
        if (!community) Stub(Path.Combine("BPScripts", "stub.sql"), "SELECT 1");
    ...
    public void The_installer_compiles_in_both_profiles_and_packages_the_diagnostic_scripts()
    {
        ...
        try
        {
            ...
            Assert.Contains(Path.Combine("scripts", "sp_PerfCheck.sql"), full.PackagedFiles,
                StringComparer.OrdinalIgnoreCase);
            Assert.Contains(Path.Combine("BPScripts", "stub.sql"), full.PackagedFiles,
                StringComparer.OrdinalIgnoreCase);
            ...
            Assert.Contains(Path.Combine("docs", "compliance", "incident-response-runbook.md"),
            ...
            Assert.True(community.ExitCode == 0,
                ...
                + "which has no BPScripts folder (exit " + community.ExitCode + "). That is the "
            ...
            Assert.Contains(Path.Combine("Assets", "brand", "sqltriage-mark.png"), community.PackagedFiles,
                StringComparer.OrdinalIgnoreCase);
            Assert.DoesNotContain(Path.Combine("BPScripts", "stub.sql"), community.PackagedFiles,
                StringComparer.OrdinalIgnoreCase);
