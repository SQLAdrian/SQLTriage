...
public class DocPointerDeliveryTests
{
    ...
    public void Every_operator_facing_docs_pointer_is_installed_by_the_inno_script()
    {
        ...
        // THE ROUTE CHANGED 2026-09-10 (round two of lane customer-update-path) and this test changed
        // with it, deliberately. The pack used to be installed straight to {app}\docs\compliance with
        // ignoreversion, which meant every upgrade replaced the operator's FILLED-IN documents -
        // sign-off-log.md is an append-only evidence record auditors read - with our blank templates.
        // It now installs to {app}\docs.default\compliance and Data\Services\ConfigDefaultsSeeder
        // creates {app}\docs\compliance\<file> on first run, per file, only when it is missing.
        //
        // So a pointer is DELIVERED if the .iss ships the path itself OR ships its docs.default mirror.
        // The second half of that claim is only true while the seeder actually maps one to the other,
        // which is why the mapping is asserted here rather than assumed: if someone renames either
        // constant, this test names the pointer that stops arriving.
        Assert.Equal("docs.default", SQLTriage.Data.Services.ConfigDefaultsSeeder.DocsDefaultsFolderName);
        Assert.Equal("docs", SQLTriage.Data.Services.ConfigDefaultsSeeder.DocsFolderName);

        var uncovered = OperatorFacingDocPointers()
            .Select(p => p.docPath)
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .Where(d => !DeliveryCandidates(d).Any(c => sourceLines.Any(l => IssShips(l, c))))
            .ToList();

        Assert.True(uncovered.Count == 0,
            "These docs/ paths are shown to an operator but installer\\SQLTriage.iss has no Source "
            + "line that installs them - directly, or as the docs.default template the app seeds from:\n  "
            + string.Join("\n  ", uncovered)
    ...
    /// <summary>
    /// The paths that would each satisfy a pointer: the document itself, and the pristine template the
    /// seeder copies it from. Seeding is what lets the pack ship as a default without breaking the
    /// pointer, and it is the same trade config.default makes.
    /// </summary>
    private static IEnumerable<string> DeliveryCandidates(string docPath)
    {
        yield return docPath;

        var norm = docPath.Replace('\\', '/');
        var docs = SQLTriage.Data.Services.ConfigDefaultsSeeder.DocsFolderName + "/";
        if (norm.StartsWith(docs, StringComparison.OrdinalIgnoreCase))
            yield return SQLTriage.Data.Services.ConfigDefaultsSeeder.DocsDefaultsFolderName + "/" + norm.Substring(docs.Length);
    }
    ...
    public void The_compliance_source_line_is_deliberately_unguarded()
    {
        ...
        // "compliance", not @"docs\compliance": from 2026-09-10 round two the pack is installed from
        // docs.default\compliance, so the narrower match would find nothing and this guard would pass
        // vacuously on a file that had stopped shipping the pack altogether.
        var line = IssLogicalLines(iss).FirstOrDefault(
            l => l.StartsWith("Source:", StringComparison.OrdinalIgnoreCase)
                 && l.Contains("compliance", StringComparison.OrdinalIgnoreCase));

        Assert.NotNull(line);
        Assert.Contains("compliance", line!, StringComparison.OrdinalIgnoreCase);
        ...
        // Deploy\ and BPScripts.default\ are wrapped in #if DirExists because they are legitimately absent
        ...
        Assert.False(guardedRegion.Success,
            ...
            + "That guard is what lets Deploy\\ and BPScripts.default\\ compile when a profile legitimately "
