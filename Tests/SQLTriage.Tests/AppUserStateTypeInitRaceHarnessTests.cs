...
public sealed class AppUserStateTypeInitRaceHarnessTests
{
    ...
    private static string FindHarnessExe()
    {
        ...
        while (dir != null)
        {
            ...
            {
                // ⚠ "Debug carries no RuntimeIdentifier" — the premise the first two entries were
                // written on — IS FALSE FOR THE WAY THIS REPO MUST BE BUILT. Every build here passes
                // `-r win-x64` (a RID-less restore strips the Tests lock's win-x64 target and the next
                // --locked-mode restore fails NU1004), so a Debug build lands the harness at
                // bin\Debug\net10.0-windows\win-x64\ and this probe found nothing. PROVED 2026-09-11:
                // that was one of 28 red tests at base c216df6 (the other 27 share this lane's one cause,
                // a config resolved out of a stale bin\ rather than the payload), and it was red at
                // dev main c216df6 too — not a regression, a hand-written list of output shapes that
                // the required build outgrew. Rather than guess a third shape, the walk below is now
                // exhaustive over bin\, so a future RID or configuration cannot blind it again.
                var binRoot = Path.Combine(dir.FullName, "Tests", "RaceHarness", "bin");
                if (Directory.Exists(binRoot))
                {
                    var found = Directory
                        .EnumerateFiles(binRoot, "AppUserStateRaceHarness.exe", SearchOption.AllDirectories)
                        .OrderByDescending(File.GetLastWriteTimeUtc)   // the most recently built wins
                        .FirstOrDefault();
                    if (found != null) return found;
                }

                throw new InvalidOperationException(
                    $"Found {projectPath} but no built AppUserStateRaceHarness.exe anywhere under " +
                    $"{binRoot}. SQLTriage.Tests.csproj carries a ProjectReference to that " +
