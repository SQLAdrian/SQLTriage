...
public sealed class BlitzDashboardServiceTests
{
    ...
    // ── Retired CheckIDs (FRK 20260708 dropped 129 and 157) ────────────────────
    //
    // A check the deployed script can no longer emit must leave the universe. Left in, its weight
    // lands in the numerator on every instance forever: a permanent free PASS, and a line the
    // client reads as "assessed, healthy" when nothing assessed it. See
    // BlitzDashboardService.RetiredCheckIds for the provenance of the two ids.

    [Fact]
    public void BuildCatalog_DropsRetiredCheckIds_FromTheCorpusSide()
    {
        var corpus = new[] { Blitz("00129", "129"), Blitz("00157", "157"), Blitz("00048", "48") };
        var cat = BlitzDashboardService.BuildCatalog(corpus, null);

        Assert.False(cat.ContainsKey(129), "129 is retired upstream and must not be a universe member.");
        Assert.False(cat.ContainsKey(157), "157 is retired upstream and must not be a universe member.");
        Assert.True(cat.ContainsKey(48), "a live id beside them must be unaffected.");
    }

    [Fact]
    public void BuildCatalog_DropsRetiredCheckIds_FromTheRoadmapMapSide()
    {
        const string mapJson = """
        {
          "blitzCheckMap": [
            { "checkId": 129, "findingName": "Dangerous Build of SQL Server (Corruption)", "category": "Reliability", "IsBad": 1, "effort_estimate": "2 hr" },
            { "checkId": 157, "findingName": "Dangerous Build of SQL Server (Security)",   "category": "Reliability", "IsBad": 1, "effort_estimate": "30 min" },
            { "checkId": 273, "findingName": "Constitution.md Present", "category": "Configuration", "IsBad": 1, "effort_estimate": "30 min" }
          ]
        }
        """;

        var cat = BlitzDashboardService.BuildCatalog(System.Array.Empty<SqlCheck>(), mapJson);

        Assert.False(cat.ContainsKey(129));
        Assert.False(cat.ContainsKey(157));
        Assert.True(cat.ContainsKey(273), "the new 8.34 id must still map through.");
    }

    [Fact]
    public void RetiredCheckId_LeavesTheDenominator_SoItCannotInflateTheScore()
    {
        // Two corpus checks with identical weight: one live, one retired. Nothing fires.
        // With 129 in the universe the score is still 100, so a bare score read proves nothing --
        // the UNIVERSE SIZE is what moves, and that is what the denominator is built from.
        var withRetired = BlitzDashboardService.BuildCatalog(
            new[] { Blitz("00048", "48"), Blitz("00129", "129") }, null);
        var liveOnly = BlitzDashboardService.BuildCatalog(new[] { Blitz("00048", "48") }, null);

        var a = BlitzDashboardService.ComputeInstanceReport(FiredFile("S"), withRetired);
        var b = BlitzDashboardService.ComputeInstanceReport(FiredFile("S"), liveOnly);

        Assert.Equal(1, a.UniverseSize);
        Assert.Equal(b.UniverseSize, a.UniverseSize);

        // And the honest half: with the retired id ALSO firing on a live check, the score must be
        // the same as if the retired id were not in the file at all.
        var scoredWithRetiredFiring = BlitzDashboardService.ComputeInstanceReport(
            FiredFile("S", 48, 129), withRetired);
        var scoredWithoutIt = BlitzDashboardService.ComputeInstanceReport(FiredFile("S", 48), liveOnly);
        Assert.Equal(scoredWithoutIt.HealthScore, scoredWithRetiredFiring.HealthScore);
    }

    [Fact]
    public void FiredRetiredCheckId_IsNotRenderedAndIsNotAnUnclassifiedDing()
    {
        // A historical CSV written by an older sp_Blitz still carries 129. It must not become a
        // nameless "sp_Blitz Check 129" ding, which is what the unknown-id branch would do.
        var cat = BlitzDashboardService.BuildCatalog(new[] { Blitz("00048", "48") }, null);
        var r = BlitzDashboardService.ComputeInstanceReport(FiredFile("S", 129), cat);

        Assert.Equal(0, r.UnclassifiedFired);
        Assert.Equal(0, r.ChecksFired);
        Assert.DoesNotContain(r.Findings, f => f.BlitzCheckId == 129);
        Assert.Equal(100.0, r.HealthScore);
    }

    [Fact]
    public void The_retired_set_holds_exactly_the_two_ids_FRK_20260708_dropped()
    {
        // Pinned by value, so growing the set is a deliberate act with a commit message attached.
        Assert.Equal(new[] { 129, 157 }, BlitzDashboardService.RetiredCheckIds.OrderBy(i => i).ToArray());
    }
}
