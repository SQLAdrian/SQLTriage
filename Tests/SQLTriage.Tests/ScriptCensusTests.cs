...
public class ScriptCensusTests
{
    ...
    private static readonly IReadOnlyDictionary<string, string> FoldersTheRuntimeResolves =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["BPScripts"] =
                "Data/BPScriptService.cs:27, Data/Services/ProductionReadinessGate.cs:95 and "
                + "Pages/ScheduledTasks.razor:147. Full profile only - buildprofile.targets:170-171 "
                + "removes the SOURCE glob from a community publish. SEEDED, NOT INSTALLED, from "
                + "2026-09-11: see FoldersSeededFromDefaults below.",
        };

    /// <summary>
    /// Folders the runtime resolves that the installer must NOT lay down, because the operator
    /// authors in them - mapped to the <c>&lt;name&gt;.default\</c> folder the installer ships
    /// instead, from which ConfigDefaultsSeeder creates them on first run.
    ///
    /// <para>WHY THIS CATEGORY EXISTS. <c>Every_folder_the_runtime_resolves_is_shipped_by_the_installer</c>
    /// used to have one answer for every folder: it must appear in [Files]. That is now WRONG for a
    /// folder on the seed-once side, and dangerously so - a file the installer lays down goes into the
    /// uninstall log, and CurStepChanged runs the previous version's uninstaller before the new [Files]
    /// copy, so laying it down is what would delete the operator's work. The requirement is not
    /// dropped, it is redirected: the DEFAULTS must ship, or the seeder has nothing to seed from and
    /// the folder is never created at all.</para>
    /// </summary>
    private static readonly IReadOnlyDictionary<string, string> FoldersSeededFromDefaults =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["BPScripts"] =
                "BPScripts.default",
    ...
    private static readonly IReadOnlyDictionary<string, string> FoldersAbsentInSomeProfile =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["BPScripts.default"] =
                "buildprofile.targets:170-171 removes BPScripts\\** - the SOURCE glob - from a COMMUNITY "
                + "publish, and from 2026-09-11 those same files publish to BPScripts.default\\, so the "
                + "PAYLOAD folder that is legitimately absent is now the .default one and the #if guard "
                + "moved with it. Unguarded, this line aborted the community installer with iscc exit 2 "
                + "and the public release script degraded the release to zip-only without failing. "
                + "Measured 2026-08-24 against a community stub tree.",
        };
    ...
    public void Every_folder_the_runtime_resolves_is_shipped_by_the_installer()
    {
        ...
        // 2026-09-11: a folder on the seed-once side is satisfied by its DEFAULTS shipping, not by
        // itself shipping - and for that folder, shipping itself would be the defect (see
        // FoldersSeededFromDefaults). The requirement is redirected, never dropped: if neither the
        // folder nor its defaults are in [Files], nothing puts it on an installed machine.
        var missing = FoldersTheRuntimeResolves
            .Where(kv => !shipped.Contains(kv.Key))
            .Where(kv => !(FoldersSeededFromDefaults.TryGetValue(kv.Key, out var defaults)
                           && shipped.Contains(defaults)))
            .ToList();

        Assert.True(missing.Count == 0,
            "installer/SQLTriage.iss has no [Files] entry for folders the app resolves at run time, and "
            + "no shipped defaults folder to seed them from either: "
            + string.Join("; ", missing.Select(kv => kv.Key + " (read by " + kv.Value + ")"))
            + ". Without one or the other the folder is simply not on an installed machine.");

        // ...and the redirection must be REAL in the other direction too: a folder recorded as seeded
        // must NOT be laid down by [Files]. Anything the installer lays down enters the uninstall log,
        // and CurStepChanged runs the previous version's uninstaller before the new copy - so laying it
        // down is exactly what deletes the operator's work on the next upgrade.
        var laidDownAnyway = FoldersSeededFromDefaults
            .Where(kv => shipped.Contains(kv.Key))
            .ToList();

        Assert.True(laidDownAnyway.Count == 0,
            "installer/SQLTriage.iss installs into folders that are supposed to be SEEDED, not shipped: "
            + string.Join("; ", laidDownAnyway.Select(kv => kv.Key + " (defaults ship as " + kv.Value + ")"))
            + ". A file [Files] lays down is a file the pre-upgrade uninstall deletes.");
    }
    ...
    {
        // config.default\ since 2026-09-10 (lane customer-update-path): the payload ships the
        // operator-editable files as DEFAULTS, and the app seeds config\ from them on first run. Both
        // source folders are accepted here because the DESTINATION - {app}\Config, onlyifdoesntexist -
        // is what this census is about, and that has not changed. A file that reverted to shipping out
        // of config\ would still be counted, which is right: it would still need preserving.
        var rx = new Regex(@"\\config(?:\.default)?\\(?<name>[A-Za-z0-9._-]+\.json)""[^\r\n]*onlyifdoesntexist",
