...
namespace SQLTriage.Tests
{
    ...
    public class ReportBundleServiceTests : IDisposable
    {
        ...
        /// <summary>
        /// Human-visible PDF text with ALL whitespace stripped.
        /// <para>Skia writes kerned glyph runs, so a rendered "72%" comes back out of the extractor as
        /// "7 2 %" and "Top 5 Risks" as "T op 5 Risks" — a normalise-to-single-space scan misses both.
        /// Space-stripping is the same basis the hunt's own renders were read on.</para>
        /// <para>Internal so the profile-gated Executive Summary fixture
        /// (Gated/ExecutiveSummaryEstateHonestyTests.cs) reuses it instead of carrying a second copy.</para>
        /// </summary>
        internal static string PdfText(byte[] pdf) =>
            System.Text.RegularExpressions.Regex.Replace(PdfTextExtractor.Extract(pdf), @"\s+", "");

        // ── reports-r2-09: "Total Audit Events: 0" for a count nobody obtained ────────────────────
        //
        // auditCount started at 0 and was assigned only inside the try. With no audit service (this
        // fixture) or a read that threw, the compliance PDF printed a measured-looking zero.
        [Fact]
        public async Task AuditEvidencePdf_SaysTheEventCountWasNotMeasured_WhenThereIsNoAuditService()
        {
            var (svc, vaState) = NewServiceWithState();   // auditLog: null — the ABSENT arm
            vaState.Results = new List<AssessmentResult> { Finding("test-server", "VA1001") };
            vaState.HasRun = true;

            var text = PdfText(await svc.BuildAuditEvidencePdfAsync("test-server", watermark: false));

            Assert.Contains("TotalAuditEvents", text);                // the row is still there…
            Assert.Contains("Notmeasured", text);                     // …carrying a fact, not a number
            Assert.DoesNotContain("TotalAuditEvents0", text);
        }

        [Fact]
        public async Task AuditEvidenceHtml_SaysTheEventCountWasNotMeasured_WhenThereIsNoAuditService()
        {
            // The HTML twin makes the same statement — one dataset, two deliverables, one story.
            var html = await NewService().PrepareAuditEvidenceHtmlAsync("test-server");

            Assert.Contains("<th>Total Audit Events</th><td>Not measured", html, StringComparison.Ordinal);
            Assert.DoesNotContain("<th>Total Audit Events</th><td>0</td>", html, StringComparison.Ordinal);
        }

        // ── reports-r1-03: a hard-coded reporting window beside the findings hash ─────────────────
        //
        // The "Report Integrity" table printed ("Report Period","Last 30 days") two rows above the
        // SHA-256 of the findings body — and GatherAuditEvidence takes no date parameter and applies
        // no time filter, so the window described nothing the hashed body was selected by. The
        // 30-day window belongs to the audit-EVENT count in the table above it, which is why the
        // literal survived a reader's glance for so long. Identical to the literal removed from the
        // Executive Summary on 2026-07-22 for exactly this reasoning.

        [Fact]
        public async Task AuditEvidencePdf_DoesNotClaimAReportingWindowTheHashedFindingsWereNotFilteredBy()
        {
            var (svc, vaState) = NewServiceWithState();
            vaState.Results = new List<AssessmentResult> { Finding("test-server", "VA1001") };
            vaState.HasRun = true;
            ...
            Assert.Contains("ReportIntegrity", text);                 // the extractor is reading it
            Assert.DoesNotContain("ReportPeriod", text);
            Assert.Contains("FindingsScope", text);
            Assert.Contains("Nodatefilterisappliedtothehashedfindings", text);

            // The genuinely 30-day thing on this page is untouched: the audit-EVENT summary above.
            Assert.Contains("AuditLogSummary(Last30Days)", text);
        }

        [Fact]
        public async Task AuditEvidenceHtml_CarriesTheSameFindingsScopeRow_SoTheTwinsCannotDisagree()
        {
            var html = await NewService().PrepareAuditEvidenceHtmlAsync("test-server");

            Assert.DoesNotContain("<th>Report Period</th>", html, StringComparison.Ordinal);
            Assert.Contains("<th>Findings Scope</th><td>Most recent completed assessment.", html, StringComparison.Ordinal);
        }

        // ── reports-r2-06: the Risk Register's empty state named a scan that cannot fill it ───────
        //
        // GatherRiskRegister reads GetMergedFindings, corpus-only since the 2026-07-16 ruling, so a
        // Vulnerability Assessment leaves this register exactly as empty. The PDF told the reader to
        // run one anyway; its HTML twin already said "Run the check suite (Audit Assessment) first".
        // The two other bundles in this class of defect are pinned in
        // Gated/ReportAttestationHonestyTests (both are behind community-off report toggles).

        [Fact]
        public async Task RiskRegisterPdf_SendsAnEmptyReaderToTheCheckSuite_NotToAVulnerabilityAssessment()
        {
            var text = PdfText(await NewService().BuildRiskRegisterPdfAsync(
                "test-server", acknowledgement: false, formalTone: true, preparedBy: "probe", watermark: false));

            Assert.Contains("RiskRegister", text);                    // the extractor is reading it
            Assert.Contains("Runthechecksuite(AuditAssessment)first", text);
            Assert.DoesNotContain("VulnerabilityAssessmentfirst", text);
        }

        [Fact]
        public async Task AuditEvidencePdf_StillNamesTheVulnerabilityAssessment_BecauseThatIsTheScanThatFillsIt()
        {
            // The deliberate exception, pinned so a later sweep of the r2-06 wording does not
            // "correct" a string that is already right. Audit Evidence reads GetCachedFindings
            // (VulnerabilityAssessmentStateService), not GetMergedFindings — the single-source MS VA
            // attestation the 2026-07-16 ruling explicitly carved out. Sending its reader to the
            // corpus check suite would be the same defect pointed the other way.
            var (svc, vaState) = NewServiceWithState();
            vaState.HasRun = true;                                    // ran, found nothing
            vaState.Results = new List<AssessmentResult>();

            var text = PdfText(await svc.BuildAuditEvidencePdfAsync("test-server", watermark: false));

            Assert.Contains("RunaVulnerabilityAssessmentfirst", text);
            Assert.DoesNotContain("Runthechecksuite", text);
        }

        // ── reports-r2-01: the estate denominator was built from the FINDINGS ─────────────────────
        //
        // "N servers scanned" and every "N of M servers impacted" cell counted servers that had a
        // FAILING finding, so a scanned-and-clean server was invisible and a wholly clean estate
        // reported "0 servers scanned". Measured directly on the seam: the scanned set is the
        // denominator, the finding labels only add the "(unattributed)" bucket.

        private static AssessmentResult FindingOn(string? server) => new()
        {
            ThisServer = server, CheckId = "C1", DisplayName = "check", Severity = "High", Status = "Failed",
        };

        [Fact]
        public void EstateServerScope_CountsTheScannedEstate_NotOnlyTheServersWithFindings()
        {
            var scanned = new[] { "SQL01", "SQL02" };          // both scanned, only SQL02 failing
            var findings = new[] { FindingOn("SQL02") };

            var scope = ReportBundleService.EstateServerScope(scanned, findings);

            Assert.Equal(2, scope.Count);
            Assert.Contains("SQL01", scope);                    // the clean server is not invisible
        }

        [Fact]
        public void EstateServerScope_CountsACleanEstate_InsteadOfReportingZeroScanned()
        {
            var scope = ReportBundleService.EstateServerScope(
                new[] { "SQL01", "SQL02" }, Array.Empty<AssessmentResult>());

            Assert.Equal(2, scope.Count);
        }

        [Fact]
        public void EstateServerScope_UnionsTheUnattributedBucket_SoImpactedCannotExceedTotal()
        {
            // A finding with no server attribution is a real column in the roll-up. Dropping it would
            // let a "1 of 0" cell onto the page.
            var scope = ReportBundleService.EstateServerScope(new[] { "SQL01" }, new[] { FindingOn(null) });

            Assert.Equal(2, scope.Count);
            Assert.Contains("(unattributed)", scope);
        }

        [Fact]
        public void EstateServerScope_FallsBackToTheFindingLabels_WhenTheScannedSetCouldNotBeRead()
        {
            // A floor, never an inflation: an unreadable scanned set still names the servers the
            // findings themselves carry.
            var scope = ReportBundleService.EstateServerScope(null, new[] { FindingOn("SQL02") });

            Assert.Equal(new[] { "SQL02" }, scope);
        }
