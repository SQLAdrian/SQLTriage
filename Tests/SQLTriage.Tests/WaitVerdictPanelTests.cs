...
namespace SQLTriage.Tests
{
    public class WaitVerdictPanelTests
    {
        ...
        /// same instrument DataGridErrorBodyRenderTests uses. Its one injected service (IJSRuntime,
        /// for the per-action confirmation added 2026-09-01) is supplied by a recording fake, so
        /// this is still the component itself and not a stand-in for it.</summary>
        private static async Task<string> RenderAsync(DataTable data, List<DataGridColumnColorRule> rules)
        {
            ...
            // 2026-09-01). Supplied so the component can be constructed; no interop fires here.
