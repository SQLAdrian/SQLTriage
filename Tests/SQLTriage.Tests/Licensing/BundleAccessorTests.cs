...
public class BundleAccessorTests
{
    ...
    public void GetText_ExistingKey_ReturnsText()
    {
        ...
        var m = new BundleManifest
        {
            ...
            {
                // A synthetic key with no loose repo counterpart: BundleAccessor's DEBUG-only
                // loose-source preference (GetText) resolves a real on-disk Config/*.json ahead of
                // the bundled copy, so a real path like Config/control_mappings.json would return the
                // repo file here, not "{}". This fixture proves the pure "existing key → text" path.
                ["Config/_bundle_unit_test_fixture.json"] = "{}"
            }
        };
        acc.Replace(m, Tier.Full);
        Assert.Equal("{}", acc.GetText("Config/_bundle_unit_test_fixture.json"));
    }
