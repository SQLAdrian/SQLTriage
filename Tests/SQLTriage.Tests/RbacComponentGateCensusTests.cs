    ...
    public class RbacComponentGateCensusTests
    {
        ...
        private static readonly Dictionary<string, ReviewedComponentEdge> Edges =
            new(StringComparer.Ordinal)
            {
                ...
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.TryActivateAsync"] =
                    new(null,
                        "The BOUNDED wrapper the card calls (2026-09-02, licence-card-hardening round 3); "
                        + "the synchronous TryActivate underneath is unchanged and is what it runs. The "
                        + "wrapper adds a wall-clock ceiling on the WAIT and no reach of its own: it takes "
                        + "the same two arguments, calls the same method, and returns the same result "
                        + "record with a TimedOut flag added. Nothing about who may activate changes. "
                ...
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.DeactivateAsync"] =
                    new(null,
                        "Reverts this install to the Free tier. Same surface, same host, same open question. "
                        + "The card calls the bounded wrapper (2026-09-02, round 3), which runs the "
                        + "unchanged synchronous Deactivate on a pool thread and returns whether it "
                        + "finished inside the budget. No change of reach."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.InstallBundleFileAsync"] =
                    new("settings",
                        ...
                        + "⚠ 2026-09-02: the component now calls InstallBundleFileAsync, which races the same "
                        + "synchronous InstallBundleFile behind a 15-second ceiling and returns a message on "
                        + "timeout. Every gate above is unchanged: the SEC-3 refusal is still in the click "
                        + "handler, before any path reaches the service, and the SEC-2 locality guard is still "
                        + "the first thing the synchronous method does. The bounded wrapper adds a second "
                        + "refusal in front of the blocking calls — a source whose drive root is not "
                        + "DriveType.Fixed (a mapped or removable drive) is turned away before File.Exists, "
                        + "which is a NARROWING of reach, not a widening."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.Initialize"] =
                    new(null,
                        "Re-runs the boot-time licence resolution after a bundle file has just been installed, "
                        + "so the per-file list beside the button includes the file that landed and carries the "
                        + "state a real unlock pass gave it. It reads user-settings.json and the install folder "
                        + "and swaps the in-memory bundle; it writes nothing. It cannot RAISE the tier beyond "
                        + "what the already-saved key opens — the same call TryActivate and Deactivate make."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.LastBundleScan"] =
                    new(null,
                        "READ of the per-file outcome of the last unlock pass, to render each *.aesgcm with a "
                        + "plain state word. It replaced a Directory.GetFiles the component ran on every render. "
                        + "It carries file names and states only — never key material and never file content."),
                // ── Key-file pickup (board #19, 2026-09-02) ────────────────────────────────
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.LastKeyFileScanSnapshot"] =
                    new(null,
                        "READ of the *.key.txt files the last pickup pass saw and the state word it gave each. "
                        + "It exists so the card does NOT enumerate the install folder on the render path — the "
                        + "same defect LastBundleScan replaced. It carries file names, the customer name parsed "
                        + "out of a key file, and a state enum; never the key, never the file's contents. The "
                        + "panel it feeds is deliberately visible to every caller who can see the card, because "
                        + "a plaintext licence key sitting in the install folder is not an admin-only fact — it "
                        + "is the one thing that makes an un-consumed drop visible at all. It carries the "
                        + "moment the list was measured in the SAME value, so the card can date the panel "
                        + "without a second read of a service another circuit may be re-scanning."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.RunKeyFilePickupNow"] =
                    new("settings",
                        "The \"Check for a key file now\" button. It runs the pass regardless of the one-shot "
                        + "(the service SETS that flag; it does not clear it) over the SAME pickup pass that "
                        + "start-up runs, so nothing exists that only this button can "
                        + "do. It CAN change the running licence — a pair whose customer equals the saved "
                        + "customer activates — and it CONSUMES the key file on success, so it is a write and is "
                        + "on the record as one. GATED at the component behind MayInstallBundle "
                        + "(UserState.IsAuthorized(\"settings\") && UserState.IsAdmin), the same authenticated-admin "
                        + "predicate SEC-3 put on the bundle install, and the click handler refuses on that same "
                        + "property BEFORE any path reaches the service — the gate is the decision, not the "
                        + "render. The identity ratchet still applies underneath: a pair naming a different "
                        + "customer is held here exactly as it is at start-up."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.AcceptKeyFilePair"] =
                    new("settings",
                        "The \"Accept licence for <name>\" button on a HELD pair — the ONE path past the identity "
                        + "ratchet, and therefore the widest reach this component gained in board #19. It "
                        + "activates a licence for a customer this install is NOT currently licensed to, which "
                        + "changes the name on every report and export and moves seat consumption onto another "
                        + "entitlement. That is why it is a deliberate click by an authenticated admin and never "
                        + "an automatic pickup: Adrian ruled RATCHET WITH ADMIN CONFIRM on 2026-09-02 15:35. "
                        + "GATED at the component behind MayInstallBundle and refused again in the click handler "
                        + "on the same property. The caller does NOT choose an arbitrary path: the argument is a "
                        + "key-file path taken from LicenseService's own published scan of the install folder, "
                        + "and the service re-enumerates and matches it, so a path the pass did not find "
                        + "activates nothing."),
                ["Components/Shared/ActivateFullAuditCard.razor → LicenseService.LastUnlockedBundlePath"] =
                    new(null,
                        "READ of which bundle file the running licence was decrypted from, so the status line "
                        + "names it instead of naming the first file alphabetically. A file name the operator "
                        + "can already see in their own install folder; no secret is disclosed."),
                ["Components/Shared/ActivateFullAuditCard.razor → ILogger<ActivateFullAuditCard>.LogError"] =
                    new(null,
                        "Records an exception that escaped one of the three click handlers. Added 2026-09-02 "
                        + "with the try/catch/finally that stops a fault stranding the card on \"Working...\". "
                        + "Writes to the app log only, and logs the exception — never the customer name, the "
                        + "phrase, or any part of a bundle."),
                ...
                ["Components/Shared/ActivateFullAuditCard.razor → ToastService.ShowInfo"] =
                    new(null, "\"Deactivated. This install is back on the Free tier.\" The em dash the old copy "
                        + "carried is what the voice lint refuses in a user-facing string."),
                ["Components/Shared/ActivateFullAuditCard.razor → ToastService.ShowWarning"] =
                    new(null,
                        "The two \"it worked, and something after it did not\" headlines. An install or an "
                        + "activation that SUCCEEDED and then faulted is announced as a warning, never as a "
                        + "refusal. Same singleton and same cosmetic reach as the other three toasts, and it "
                ...
                // ⚠ The three option-builder calls below were UNREGISTERED and INVISIBLE until
                // 2026-09-01. ShellSurfaceRegistry.EdgesIn required the '(' to follow the member
                // name immediately, so every GENERIC call on an injected service went unseen —
                // `ChartTheme.GetDonutOptions<StatValue>("")` among them. The census was green over
                // three unrecorded edges. Found while adding the DataGrid confirmation gate, whose
                // `JS.InvokeAsync<bool>("confirm", …)` has the same shape and was equally invisible.
                // The scanner now allows an optional type-argument list; these three are what that
                // change revealed, and they are read-only — each returns a fresh ApexChartOptions
                // built from the theme, touching nothing.
                ["Components/Shared/DonutChart.razor → IChartThemeService.GetDonutOptions"] =
                    new(null,
                        "Read: builds a fresh donut chart options object from the current theme. Returns "
                        + "a new object every call and mutates no shared state."),
                ["Components/Shared/HorizontalBarChart.razor → IChartThemeService.GetBarOptions"] =
                    new(null, "Read: the same options build for a horizontal bar chart."),
                ["Components/Shared/TimeSeriesChart.razor → IChartThemeService.GetLineOptions"] =
                    new(null, "Read: the same options build for a line chart."),
                ...
                // ── Ruling R1/R2(d), 2026-09-01: the action path journals what it sends ──
                // Not gated on a permission of their own, deliberately. These three record what the
                // gated path above did; a journal call that could be refused would leave the act
                // unrecorded, which is the opposite of the point. They reach the audit chain and the
                // change ledger on THIS box, not a monitored server.
                ["Components/Shared/DynamicDashboard.razor → DdlJournal.RecordAttempt"] =
                    new(null,
                        "Records (and flushes) the statement about to be sent by the gated action path, "
                        + "before it is sent — so no statement executes unjournaled. Writes the local "
                        + "HMAC-chained audit log; touches no monitored server."),
                ["Components/Shared/DynamicDashboard.razor → DdlJournal.RecordOutcomeAsync"] =
                    new(null,
                        "Records the terminal state of that statement — succeeded, cancelled or failed — "
                        + "in the audit chain and the change ledger. Called on every path, including the "
                        + "catch blocks, so a failed DDL is journaled as failed rather than as absent."),
                ["Components/Shared/DynamicDashboard.razor → DdlJournal.RecordBlocked"] =
                    new(null,
                        "Records a DangerousExecGuard refusal of the resolved double-hop statement. A "
                        + "refusal is often the only trace that anything was tried, so it goes in the "
                        + "chain rather than only into a rolling Serilog file."),
                ...
                // ══ Components/Shared/EvalFailurePanel.razor ════════════════════
                //
                // Added 2026-09-08 by the orchestrator, NOT by the lane, after the merge-push run for
                // dbfa9a34 failed EveryComponentEdgeIsOnTheRecord. All three edges were READ before
                // writing these entries, per this census's own instruction not to guess from the
                // member name. What was read is stated in each reason.
                //
                // ⚠ OPEN, NOT DECIDED HERE — the same shape as ActivateFullAuditCard above. These
                // three are genuinely ungated READS, so null is the factual entry. But the panel
                // renders on Pages/Index.razor ("/") and Pages/Alerts.razor, and NEITHER carries a
                // page-level gate — Index.razor has @page "/" and no IsAuthorized wrapper at all.
                // RbacService's own words: a null bootstrap proof yields "the plain permission
                // matrix, which for an unauthenticated circuit is viewer. Every remote client
                // arrives this way." So an unauthenticated remote client is viewer, reaches "/",
                // and sees SERVER NAMES and raw error text for every alert that cannot be measured.
                // Whether that is a disclosure worth gating is Adrian's to rule, and there is a real
                // argument on the other side: the eval-failure-visible ruling (2026-09-06) exists
                // precisely so a monitoring gap is VISIBLE rather than silent, and gating it to
                // privileged roles would partly defeat the thing the lane was built to guarantee.
                // Registering it honestly is not the same as endorsing it.

                ["Components/Shared/EvalFailurePanel.razor → AlertEvaluationService.EvaluationFailures"] =
                    new(null,
                        "READ, and a pure one: the member is an expression-bodied property, "
                        + "`_evalFailures.Values.ToList()`, returning a fresh IReadOnlyCollection "
                        + "snapshot. It cannot mutate. The component binds it once per render "
                        + "(`var failures = Engine.EvaluationFailures`) and renders four columns of "
                        + "each AlertEvalFailure record: alert name, ServerName, ErrorSummary, and "
                        + "FirstFailureUtc with FailureCount. UNGATED at the component. ⚠ The "
                        + "disclosure question above is about THIS edge — ServerName and ErrorSummary "
                        + "are the fields a viewer would not otherwise be handed on the landing page."),
                ["Components/Shared/EvalFailurePanel.razor → AlertEvaluationService.OnAlertsChanged"] =
                    new(null,
                        "NOT a data edge at all: `public event Action? OnAlertsChanged`. The component "
                        + "subscribes in OnInitialized and unsubscribes in Dispose, and its handler is "
                        + "`InvokeAsync(StateHasChanged)` — a re-render and nothing else. It carries no "
                        + "payload, reads no state and writes none. The subscribe/unsubscribe pair is "
                        + "balanced, so it also cannot leak a handler across circuits."),
                ["Components/Shared/EvalFailurePanel.razor → AlertDefinitionService.GetAlert"] =
                    new(null,
                        "READ of one definition by id, used for display only: "
                        + "`Definitions.GetAlert(alertId)?.Name ?? alertId`. The component takes the "
                        + "Name and nothing else, and falls back to the raw id when the alert is gone, "
                        + "so a deleted definition degrades to an identifier rather than throwing. The "
                        + "ids it passes are those already present in EvaluationFailures, so this edge "
                        + "widens no reach beyond the edge above."),
                ...
                ["Components/Shared/QueryPlanModal.razor → IndexOptionSubstitution.Apply"] =
                    new(null,
                        "Ruling F-B, 2026-09-01. Pure text substitution of the {OPTION} placeholders in a "
                        + "suggested-index template, on this box, in memory — no connection, no server, no "
                        + "file. Registered because the verb lexicon reads 'Apply' as an acting verb, which "
                        + "is the right default: 'Apply' usually does mean apply. It is the ONE substitution "
                        + "shared by the preview and the execution, so the string an operator reads is the "
                        + "string that runs. The act it feeds — cmd.ExecuteNonQueryAsync — is the gated "
                        + "edge above."),

                // ── Ruling R1, 2026-09-01: the index-create path journals what it sends ──
                ["Components/Shared/QueryPlanModal.razor → DdlJournal.RecordAttempt"] =
                    new(null,
                        "Records (and flushes) the CREATE NONCLUSTERED INDEX statement before it is sent, "
                        + "so no statement executes unjournaled. Local audit chain; no server touched."),
                ["Components/Shared/QueryPlanModal.razor → DdlJournal.RecordOutcomeAsync"] =
                    new(null,
                        "Records the terminal state — succeeded, cancelled or failed — in the audit chain "
                        + "and the change ledger. Cancellation is recorded as effect-unknown, because "
                        + "cancelling CREATE INDEX does not prove nothing happened on the server."),
                ["Components/Shared/QueryPlanModal.razor → DdlJournal.RecordBlocked"] =
                    new(null,
                        "Records the two statement-shape refusals in ExecuteIndexFromOperator (not a "
                        + "single CREATE NONCLUSTERED INDEX; a separator, comment marker or GO). Kept in "
                        + "the tamper-evident chain because a refusal is evidence in its own right."),

                // ══ Components/Shared/DataGrid.razor ══════════════════════════════════════════
                //
                // Moved off the INERT list on 2026-09-01 (ruling R2(a)). The delegates behind the
                // Execute and Run-Job buttons still belong to DynamicDashboard and are still gated
                // there; what lives here now is the per-action confirmation that had to gate them.

                ["Components/Shared/DataGrid.razor → IJSRuntime.InvokeAsync"] =
                    new(null,
                        "window.confirm for the per-action confirmation in front of the Execute button — "
                        + "the app's existing per-action pattern (Sessions.razor's KILL uses the same "
                        + "call). It asks a question in THIS circuit's browser and touches no server. "
                        + "Ungated on purpose: the permission check belongs to the delegate's owner "
                        + "(DynamicDashboard.MayExecuteActions), and a confirmation that could itself be "
                        + "refused would fail OPEN. When the answer cannot be obtained the handler "
                        + "declines, which is the safe direction for a write gate."),
        ...
        private static readonly Dictionary<string, string> InertInteractiveComponents =
            new(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // ⚠ DataGrid.razor SAT HERE until 2026-09-01, as "sort/filter/top-N grid. It
                // RENDERS the action and run-job buttons, but the delegates behind them belong to
                // DynamicDashboard and are gated there". The delegate claim is still true, and the
                // inert claim is not: ruling R2(a) put the per-action CONFIRMATION in this file,
                // because this component is the only place the Execute button exists and a future
                // consumer of ActionColumn must inherit the gate rather than remember it. It is on
                // the edge register now, with the interop call it makes.
        ...
        public void ActivateFullAuditCardIsReachedOnlyFromTheGatedSettingsPage()
        {
            ...
            foreach (var scanRoot in new[] { "Pages", "Components" })
            {
                ...
                {
                    // A component is not its own host. The card injects
                    // ILogger<ActivateFullAuditCard>, and that generic argument carries the same
                    // "<ActivateFullAuditCard" token this scan looks for — so without this line the
                    // card reports itself as a hosting page. Skipping the file BY PATH (not by
                    // filtering the token) keeps the scan's reach over every OTHER component
                    // unchanged: a real second host still fails this test.
                    if (string.Equals(Path.GetRelativePath(root.FullName, file.FullName).Replace('\\', '/'),
                            "Components/Shared/ActivateFullAuditCard.razor", StringComparison.Ordinal))
                        continue;
