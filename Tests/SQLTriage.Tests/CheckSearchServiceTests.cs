...
namespace SQLTriage.Tests
{
    public class CheckSearchServiceTests : IDisposable
    {
        ...
        public CheckSearchServiceTests()
        {
            ...
            // fresh-eyes security-9 / lane/xss-markdown-sinks: a check whose text carries raw HTML,
            // used to prove the FTS5 snippet is HTML-encoded (only the <b> highlight survives) before
            // it reaches the CheckValidatorTable MarkupString sink. The marker "ZZXSSMARKER" is unique
            // to this row, so no other test in this fixture matches it.
            checks.Add(new SqlCheck
            {
                Id = "CHK-051",
                Name = "Xss Probe Row",
                Description = "ZZXSSMARKER <img src=x onerror=alert(1)> <script>alert(2)</script>",
                Category = "Probe",
                Severity = "Warning",
                SqlQuery = "SELECT 51",
                Enabled = true
        ...
        [Fact]
        public void Search_Snippet_HtmlEncodesServerText_ButKeepsHighlight()
        {
            // The snippet renders through CheckValidatorTable.razor as MarkupString. It must carry the
            // <b> search highlight (allow-listed) but nothing else HTML: raw tags from check content
            // are encoded so a malicious description cannot inject markup (fresh-eyes security-9).
            var results = _search.Search("ZZXSSMARKER");
            Assert.NotEmpty(results);
            var hit = results.First(r => r.CheckId == "CHK-051");

            // The highlight is preserved as real markup.
            Assert.Contains("<b>", hit.Snippet);
            Assert.Contains("</b>", hit.Snippet);

            // The payload survives only as inert, encoded text.
            Assert.DoesNotContain("<img", hit.Snippet);
            Assert.DoesNotContain("<script", hit.Snippet);
            Assert.Contains("&lt;img", hit.Snippet);
