...
namespace SQLTriage.Tests
{
    ...
    public class DiLifetimeCensusTests
    {
        ...
        // ── 7. ServerConfigRunState is per-circuit, and nothing but a comment said so ────

        /// <summary>
        /// <b>Gate finding F1, 2026-09-08.</b> <c>ServerConfigRunState</c> was registered
        /// <c>AddScoped</c> with a paragraph of comment above it explaining why — and a probe that
        /// flipped that one word to <c>AddSingleton</c> left all 202 lane tests green, this census
        /// included. The lifetime was held by nothing.
        ///
        /// <para><b>Why the census above could not see it.</b>
        /// <see cref="CrossCircuitStateMarkers"/> flags three shapes: a
        /// <c>public event Func&lt;…&gt;</c>, a public delegate FIELD, and a public
        /// <c>Selected…</c> property. This service's cross-circuit state is
        /// <c>public Dictionary&lt;string, InstanceRunStatus&gt; PreviewStatuses/ApplyStatuses
        /// { get; }</c> — none of the three — and its <c>public event Action? Changed</c> is
        /// excluded by that scanner's own rule. So the generic net was green on a real singleton.
        /// A generic scanner will always have a shape it does not carry; this test names the
        /// service instead of describing it.</para>
        ///
        /// <para><b>What breaks if this goes red and somebody "fixes" it by editing this test.</b>
        /// One operator's run would appear in another operator's browser: the instance NAMES of
        /// their estate, each instance's row count, and the full <c>ExportedPath</c> of every
        /// change-control file on the host's disk — plus a Download button that reads that path and
        /// sends the file. The dictionaries are the payload, not just the display.</para>
        ///
        /// <para>Asserted at the DESCRIPTOR, so it is a fact about the registration rather than an
        /// inference from source text, and it holds whichever of the four registration files put it
        /// there. The second half of the assertion is the count: a second registration anywhere in
        /// the shared graph would win by MS.DI's last-one-wins rule and could quietly restore the
        /// singleton without touching line 99.</para>
        /// </summary>
        [Fact]
        public void ServerConfigRunStateIsRegisteredPerCircuit_SoOneOperatorsRunCannotAppearInAnothersBrowser()
        {
            var services = new ServiceCollection();
            services.AddLogging();
            ...
            var descriptors = services
                .Where(d => d.ServiceType == typeof(ServerConfigRunState))
                .ToList();

            Assert.True(descriptors.Count == 1,
                $"ServerConfigRunState is registered {descriptors.Count} time(s) in the shared graph. "
                + "Exactly one registration is expected: MS.DI takes the LAST one, so a second "
                + "registration silently decides the lifetime of a service whose declared lifetime "
                + "is the boundary between two operators. Lifetimes found: "
                + string.Join(", ", descriptors.Select(d => d.Lifetime.ToString())));

            Assert.True(descriptors[0].Lifetime == ServiceLifetime.Scoped,
                "ServerConfigRunState is registered "
                + descriptors[0].Lifetime
                + ", not Scoped. It holds PreviewStatuses and ApplyStatuses — one operator's "
                + "instance names, row counts and exported file PATHS — plus the Changed event the "
                + "page renders from. As a singleton those dictionaries are shared by every live "
                + "circuit, so one operator's multi-instance run shows up in another operator's "
                + "browser complete with a Download button over the first operator's files. If a "
                + "process-wide lifetime is genuinely wanted, it needs an entry in "
                + "DeclaredProcessWideState with the reason — not a change to this test.");
        }
