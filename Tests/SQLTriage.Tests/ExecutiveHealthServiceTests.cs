...
namespace SQLTriage.Tests
{
    ...
    public class ExecutiveHealthServiceTests : IDisposable
    {
        ...
        /// <summary>
        /// reports-r1-05: an all-unmeasured day used to write composite_score = 0 to
        /// governance-history.db. MaybeWriteSnapshot fired unconditionally, immediately after the
        /// caller computed `measured` and refused to publish the composite without it. Measured
        /// before the fix: GetHealthScoreAsync over an all-empty estate returned MeasuredScore=null
        /// and the store then held exactly one row, composite=0. Pages/Health.razor's 30-day
        /// sparkline plots that row with no measured guard, and the NEXT real measurement compared
        /// itself against the fabricated 0 — a green "Improving" arrow against a baseline nobody
        /// measured (the private GetTrendAsync returned (Improving, Compared) over a seeded 0).
        /// <para>An unmeasured day leaves a GAP now, which is the honest record.</para>
        /// </summary>
        [Fact]
        public async Task NothingMeasured_WritesNoSnapshotAtAll_RatherThanAScoredZero()
        {
            var history = new GovernanceHistoryService(
                NullLogger<GovernanceHistoryService>.Instance, dbDir: _tempDir);

            // POSITIVE CONTROL first: the store and this read work, so an empty result below is a
            // fact about the writer and not about a broken reader or an absent table.
            history.RecordHealthScore("measured-server", 74, new HealthScoreBreakdown());
            Assert.NotEmpty(history.GetHealthScoreTrend("measured-server"));

            var result = await NewService().GetHealthScoreAsync("cold-server");
            Assert.Null(result.MeasuredScore);   // precondition: nothing was measured

            // The write was a fire-and-forget Task.Run before the fix, so give it a real chance to
            // land instead of racing it. Any row appearing here is the fabricated zero.
            for (var i = 0; i < 20 && history.GetHealthScoreTrend("cold-server").Count == 0; i++)
                await Task.Delay(50);

            Assert.Empty(history.GetHealthScoreTrend("cold-server"));
        }
    }
