...
namespace SQLTriage.Tests.Cli
{
    ...
    public class AuditCliArgsTests : IDisposable
    {
        ...
        // ── --report (item 1 rescope: audit-evidence via the VA engine, a SEPARATE deliverable
        //    from --format) ─────────────────────────────────────────────────────────────────────

        [Fact]
        public void Parse_ReportDefault_IsNull()
        {
            var result = AuditCliArgs.Parse(WithBundle("--servers", ".\\old2017"));

            Assert.True(result.Success);
            Assert.Null(result.Args!.Report);
        }

        [Fact]
        public void Parse_ReportAuditEvidence_CaseInsensitive_Succeeds()
        {
            var result = AuditCliArgs.Parse(WithBundle("--servers", ".\\old2017", "--report", "Audit-Evidence"));

            Assert.True(result.Success, result.Error);
            Assert.Equal("audit-evidence", result.Args!.Report);
        }

        [Fact]
        public void Parse_UnrecognizedReport_Fails()
        {
            var result = AuditCliArgs.Parse(new[] { "--servers", ".\\old2017", "--report", "compliance-scorecard" });

            Assert.False(result.Success);
            Assert.Contains("compliance-scorecard", result.Error);
        }

        [Fact]
        public void Parse_ReportMissingValue_Fails()
        {
            var result = AuditCliArgs.Parse(new[] { "--servers", ".\\old2017", "--report" });

            Assert.False(result.Success);
            Assert.Contains("--report", result.Error);
        }
