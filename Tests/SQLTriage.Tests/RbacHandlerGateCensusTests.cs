...
namespace SQLTriage.Tests
{
    ...
    public class RbacHandlerGateCensusTests
    {
        ...
        public void TheExpectedGateTableCoversEveryMappedRoute()
        {
            ...
            // Same correction as EachGatedApiEndpointDeclaresTheExpectedPermission below, and found the
            // same way: "no longer mapped" is a claim about the WORLD that this assertion cannot make.
            // All it knows is that EnumerateApiEndpoints' regex did not produce the route. Measured
            // 2026-09-15 by inserting one line break between `api` and `.MapGet` — a formatting change
            // that leaves the route live and serving — and this assertion reported it as "no longer
            // mapped".
            var stale = pinned.Where(p => !mapped.Contains(p)).ToList();
            Assert.True(stale.Count == 0,
                "These entries in ExpectedApiGates were NOT FOUND BY THE SCAN of " + ApiFile + ". That is "
                + "not the same as the routes being gone — EnumerateApiEndpoints only matches a literal "
                + "api.Map{Verb}(\"…\") call, so a reformat, a MapGroup, MapMethods or a non-literal route "
                + "hides a route that is still serving. Open " + ApiFile + " and check each one before "
                + "deleting its row; if it is still mapped, fix the regex instead:\n  "
                + string.Join("\n  ", stale));
        }
        ...
        public void EachGatedApiEndpointDeclaresTheExpectedPermission(string route, string permission, bool failClosed)
        {
            ...
            // ⚠ THIS MESSAGE MUST NOT SAY "IT WAS REMOVED". It used to, and that was the only branch it
            // offered. What actually failed is that EnumerateApiEndpoints' REGEX did not match — which is
            // a fact about the scanner, not about the route. Every one of these leaves the route LIVE and
            // serving while the scan misses it: a MapGroup refactor (g.MapGet no longer matches
            // "api.Map"), a line break between `api` and `.MapGet`, MapMethods, or a route passed as a
            // constant instead of a literal. Obeying the old wording deleted the ONLY assertion on which
            // permission and which failClosedWithoutApiKey setting that still-serving route carries — and
            // the deletion was SILENT, because TheExpectedGateTableCoversEveryMappedRoute then finds it in
            // neither `mapped` nor `pinned`, and the anti-vacuity floor here is only >15 against 18 real
            // routes, so two can fall out with everything still green. Only the four /rbac/* routes have
            // an independent backstop; the other fourteen have none.
            Assert.True(endpoint != null,
                route + " was not found by the endpoint SCAN of " + ApiFile + ". That is not the same as "
                + "the route being gone. Open " + ApiFile + " and look before changing anything.\n"
                + "  • Route really removed?  then remove it from ExpectedApiGates too.\n"
                + "  • Route still mapped, but re-spelled, reformatted, moved into a MapGroup, switched to "
                + "MapMethods, or given a non-literal route?  then FIX THE REGEX in EnumerateApiEndpoints — "
                + "do NOT delete this row. It is the only assertion that this live route still carries the "
                + "permission and fail-closed setting it is supposed to, and deleting it goes unnoticed.");
