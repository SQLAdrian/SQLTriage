...
public class ProtectedConfigParityTests
{
    ...
    public static IEnumerable<object[]> ExpectedProtectedPaths() => new[]
    {
        ...
        // CredentialProtector.cs:41 resolves it under config\, so a bare leaf here would pass over an
        // entry that had drifted to the app root - the same reason the cipher key is pinned in full.
        new object[] { @"config\.credential-key" },
