...
public class SqlQueryRepositoryBundleTests
{
    ...
    // ── The constructor's load is tracked, not discarded ──────────────────────

    /// <summary>
    /// The constructor starts its load on a background thread. Awaiting InitializationComplete —
    /// and nothing else — is what tells a caller that load has finished, so this asserts the
    /// awaited task is sufficient on its own: no ReloadAsync() call here.
    /// </summary>
    [Fact]
    public async Task InitializationComplete_WhenAwaited_TheConstructorsLoadHasPublished()
    {
        var sqlDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "Sql");
        Directory.CreateDirectory(sqlDir);
        var path = Path.Combine(sqlDir, "init-complete-probe.sql");
        await File.WriteAllTextAsync(path, "SELECT 1 AS probe");
        ...
        try
        {
            ...
            await svc.InitializationComplete;

            var query = svc.Get("init-complete-probe");
            Assert.NotNull(query);
            Assert.Equal("SELECT 1 AS probe", query!.Sql);
        }
        finally
        {
