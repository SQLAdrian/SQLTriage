...
namespace SQLTriage.Tests
{
    ...
    public class GovernanceHistoryBaselineTests : IDisposable
    {
        ...
        // 2026-07-16: RecordBaseline moved to scored basis (CheckClassification) — a SKIP (via
        // permission error, Passed=true) no longer counts as "passed", and a genuine execution
        // error (Passed=false) no longer counts as "failed"; both are excluded entirely, matching
        // the main governance trend table's PassedFindings/FailedFindings for the same run.
        [Fact]
        public void RecordBaseline_ExcludesSkipAndErrorFromPassedAndFailedCounts()
        {
            using var svc = NewService();

            var skipViaPermission = Chk("SQLT-SKIP", passed: true);
            skipViaPermission.Message = "SKIP — permission denied (SQL error 229)";
            var genuineError = Chk("SQLT-ERR", passed: false);
            genuineError.ErrorMessage = "Timeout expired";

            var id = svc.RecordBaseline("SRV1", compositeScore: 70, new[]
            {
                Chk("SQLT-A", passed: true),   // genuine pass
                Chk("SQLT-B", passed: false),  // genuine open fail
                skipViaPermission,
                genuineError,
            });
            ...
            var b = svc.GetActiveBaseline("SRV1");
            Assert.NotNull(b);
            Assert.Equal(4, b!.TotalChecks);   // raw total unchanged
            Assert.Equal(1, b.PassedChecks);   // only SQLT-A — the skip no longer inflates this
            Assert.Equal(1, b.FailedChecks);   // only SQLT-B — the error no longer inflates this
        }
