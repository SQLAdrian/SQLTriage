...
namespace SQLTriage.Tests
{
    ...
    public class JobDiffEngineTests
    {
        ...
        // ── Separator byte-identity pin (2026-09-08, lane hygiene-tail) ─────────
        // StepsHash/ScheduleHash join fields with raw 0x1F (unit separator) and terminate a
        // record with raw 0x1E (record separator) -- 22 raw control bytes in this file at
        // dev main 0e93d3640b7f2eb4157ccb49521babc046599094. This pin captures the hash of a
        // FIXED synthetic job produced by the UNEDITED source (raw bytes) BEFORE the bytes are
        // rewritten as hex-escaped literals, so a passing test after the rewrite is a
        // byte-identity proof, not an assertion about C# escape semantics.
        [Fact]
        public void StepsHash_is_byte_identical_before_and_after_the_escape_rewrite()
        {
            // Captured 2026-09-08 against the UNEDITED source (raw 0x1F/0x1E bytes) at dev main
            // 0e93d3640b7f2eb4157ccb49521babc046599094, before the raw bytes were rewritten as
            // hex-escaped literals.
            var job = Job("PinJob", cmd: "SELECT 1");
            Assert.Equal("f53bb9516e92abc03c7d63ec13ec13919137c3100c8b578e63059cb31e332eef", JobDiffEngine.StepsHash(job));
        }

        [Fact]
        public void ScheduleHash_is_byte_identical_before_and_after_the_escape_rewrite()
        {
            // Captured 2026-09-08 against the UNEDITED source (raw 0x1F/0x1E bytes) at dev main
            // 0e93d3640b7f2eb4157ccb49521babc046599094, before the raw bytes were rewritten as
            // hex-escaped literals.
            var job = Job("PinJob", schedules: new[] { Sched("Nightly", 4, 20000) });
            Assert.Equal("42a5661509cfa57ddf1ffa35ff82716e1dd7d910beca5142fdc9c7d22beb2a01", JobDiffEngine.ScheduleHash(job));
        }
    }
}
