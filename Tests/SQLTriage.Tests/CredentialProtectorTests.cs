...
public class CredentialProtectorTests
{
    ...
    // ── H1: legacy-plaintext migration must encrypt EXACTLY once ──────────────

    [Fact]
    public void MigratePlaintext_SetPassword_EncryptsOnce_RoundTrips()
    {
        // This is the exact ConnectionManager migration path: a connection holding legacy
        // plaintext gets SetPassword(plaintext). It must encrypt once, so GetDecryptedPassword
        // (a single decrypt) returns the ORIGINAL plaintext — not inner ciphertext (the H1 bug,
        // where Encrypt() was called first and SetPassword encrypted again).
        const string plain = "S3cr3t!Legacy-P@ss";
        var conn = new SQLTriage.Data.Models.ServerConnection();
        conn.SetPassword(plain);

        Assert.True(CredentialProtector.IsEncrypted(conn.Password!), "stored value must be ciphertext");
        var decrypted = conn.GetDecryptedPassword();
        Assert.Equal(plain, decrypted);
        Assert.False(decrypted.StartsWith("aes:"), "a single decrypt must not still yield ciphertext");
    }
}
