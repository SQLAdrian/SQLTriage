...
namespace SQLTriage.Tests
{
    public class BuildCatalogueServiceTests
    {
        ...
        // ── Staleness surface (#82) ──────────────────────────────────────

        [Fact]
        public void AgeInDays_ComputedFromCatalogueLastUpdated_NotWallClock()
        {
            // Today is pinned to 2026-05-15; lastUpdated is 14 days earlier.
            var svc = Build(CatalogueWithLastUpdated(D(Today.AddDays(-14))));
            Assert.Equal(14, svc.AgeInDays);
        }

        [Fact]
        public void AgeInDays_WhenLastUpdatedMissing_IsNull()
        {
            var svc = Build("{}");
            Assert.Null(svc.AgeInDays);
        }

        [Fact]
        public void AgeInDays_WhenLastUpdatedUnparsable_IsNull()
        {
            var svc = Build(CatalogueWithLastUpdated("not-a-date"));
            Assert.Null(svc.AgeInDays);
        }

        [Fact]
        public void AgeInDays_WhenLastUpdatedInFuture_ClampsToZero()
        {
            var svc = Build(CatalogueWithLastUpdated(D(Today.AddDays(5))));
            Assert.Equal(0, svc.AgeInDays);
        }

        [Fact]
        public void IsStale_AtExactlyThirtyDays_IsFalse()
        {
            // Threshold is "older than 30 days" — exactly 30 is not yet stale.
            var svc = Build(CatalogueWithLastUpdated(D(Today.AddDays(-30))));
            Assert.False(svc.IsStale);
        }

        [Fact]
        public void IsStale_AtThirtyOneDays_IsTrue()
        {
            var svc = Build(CatalogueWithLastUpdated(D(Today.AddDays(-31))));
            Assert.True(svc.IsStale);
        }

        [Fact]
        public void IsStale_WhenCatalogueUnavailable_IsFalse()
        {
            var svc = Build("{ not json at all");
            Assert.False(svc.IsStale);
        }
        ...
        {
            // 18 is not yet mapped (no SQL Server "vNext-after-2025" major handled).
            var svc = Build(SampleFullCatalogue());
            Assert.Null(svc.Lookup("18.0.1000.0"));
        }

        // ── Major version resolution ────────────────────────────────────

        [Theory]
        [InlineData("17.0.1000.7", "2025")]
        ...
        public void Returns_Empty_When_Bundle_Locked()
        {
            ...
            var svc = new BuildCatalogueService(NullLogger<BuildCatalogueService>.Instance, bundle);
            Assert.False(svc.IsAvailable);
            Assert.Null(svc.Lookup("16.0.4115.5"));
        }

        // ── Real shipped catalogue (smoke test via FakeBundleAccessor) ──────

        [Fact]
        public void Bundle_WithRealCatalogueJson_IsAvailable()
        {
            var json = LoadRealCatalogueJson();
            if (json is null) return; // file not in test output dir; deferred to CI

            var bundle = new FakeBundleAccessor()
                .PutFile("Config/sql-build-catalogue.json", json);
            var svc = new BuildCatalogueService(NullLogger<BuildCatalogueService>.Instance, bundle);
            Assert.True(svc.IsAvailable);
        ...
        [Fact]
        public void Bundle_Catalogue_KnowsAbout2022Major()
        {
            var json = LoadRealCatalogueJson();
        ...
        private static string? LoadRealCatalogueJson()
        {
            var dir = new DirectoryInfo(AppDomain.CurrentDomain.BaseDirectory);
            while (dir != null)
            {
                var candidate = Path.Combine(dir.FullName, "Config", "sql-build-catalogue.json");
        ...
        private static string CatalogueWithLastUpdated(string lastUpdated) => $$"""
        {
        ...
        private static string SampleAllVersionsCatalogue() => """
        {
          "lastUpdated": "2026-05-01",
          "versions": {
            "2025":   { "majorBuild": 17,    "builds": [ { "build": "17.0.1000.7",  "label": "RTM",  "releaseDate": "2025-11-18" } ] },
