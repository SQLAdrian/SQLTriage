...
`app.css` defines `:root` custom properties for surfaces, borders, text, accent (signal green `#34D17A`, cyan secondary `#3DD6CB`), status colours, typography, radii, and motion curves. All page CSS files reference these variables — never hard-coded hex values.

**Design tokens re-based 2026-07-09 (Phase G).** The palette moved to the canonical S-mark green-on-ink brand (source of truth: the live site `sqldba-site-cloudflare/src/styles/global.css`); the earlier cyber-neon detour and deep-teal accent are retired. Variable names are preserved, so page CSS is untouched.
