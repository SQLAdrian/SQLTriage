...
window.queryPlanInteropV2 = (function () {
    ...
    // ── Cost → colour ─────────────────────────────────────────────────────────
    // Canonical status vars ONLY — body.colorblind-mode remaps them via
    // Accessibility.css. Never return literal status hex from here.
    ...
    // ── {OPTION} substitution for DISPLAY / COPY (F-B, ruled 2026-09-01) ──────
    //
    // WHAT WAS WRONG. This file used to substitute its own hardcoded literals here —
    // ONLINE→OFF, SORT_IN_TEMPDB→ON, MAXDOP→0, DATA_COMPRESSION→NONE, RESUMABLE and
    // OPTIMIZE_FOR_SEQUENTIAL_KEY dropped — while QueryPlanModal.ApplyIndexOptions substituted the
    // live checkbox state on execute. The C# defaults are ONLINE ON / SORT_IN_TEMPDB ON /
    // MAXDOP 8, so the preview disagreed with the executed statement on a freshly opened modal
    // with nothing touched. Always-on divergence, and the operator read the preview.
    //
    // THE FIX IS DELETION, NOT ALIGNMENT. Two implementations of one substitution drift; copying
    // the C# values into this file would have re-created the same defect with a longer fuse. The
    // substitution now happens in exactly one place — ApplyIndexOptions, the method execute calls —
    // and this file asks for its output. Byte-for-byte parity is structural, not maintained.
    //
    // THE COST. It is asynchronous, so a preview cannot be produced inside a synchronous render.
    // Every site below renders the TEMPLATE first (visibly a template — '{ONLINE}', not a wrong
    // literal), stores it on the element, and fills in the resolved text when .NET answers. If the
    // ref is missing the template is what stays on screen: unhelpful, but never a false statement
    // about what would run.

    const QP_TEMPLATE_ATTR = 'data-qp-ddl-template';

    // Resolves a {OPTION} template through the C# substitution. Returns the template unchanged
    // when there is no .NET ref — never a guessed literal.
    async function resolveOptions(ddl) {
        const text = ddl || '';
        if (!_dotNetRef) return text;
        try {
            return await _dotNetRef.invokeMethodAsync('PreviewIndexDdl', text);
        } catch (e) {
            console.warn('[queryPlanV2] index preview substitution failed; showing the template', e);
            return text;
        }
    }

    // Renders a resolved preview into an element and remembers the template on it, so
    // refreshOptionPreviews() can re-resolve every preview when an option checkbox changes.
    function renderOptionPreview(el, ddl) {
        const text = ddl || '';
        el.setAttribute(QP_TEMPLATE_ATTR, text);
        el.textContent = text;
        resolveOptions(text).then(resolved => { el.textContent = resolved; });
    }
    ...
    function createPane(container) {
        let pane = document.getElementById('qp-sidebar');
        if (pane) {
            pane.style.display = 'block';
            pane.innerHTML = '<div style="padding: 24px; color: var(--text-muted); text-align: center; margin-top: 50%;">Click an operator to view its properties</div>';
            return pane;
        }
    ...
    function populatePane(pane, node) {
        ...
        copyBtn.addEventListener('click', async () => {
            ...
            for (const [k, v] of kept) {
                lines.push(k.padEnd(22) + (k === 'Suggested Index' ? await resolveOptions(v) : v));
            }
        ...
        if (suggestedRow) {
            ...
            copyDdlBtn.addEventListener('click', async () => {
                copyToClipboard(await resolveOptions(suggestedRow[1]));
    ...
    function buildSuggestedIndex(node) {
        ...
        // ── Build DDL ─────────────────────────────────────────────────────────
        // Fully qualify the target from the operator's Object metadata so the
        // statement is correct regardless of which database the executing
        // connection lands in ([db].[schema].[table] is valid for CREATE INDEX).
        // SECURITY: table/schema/db come straight from the audited server's plan
        // XML (ExecutionPlanParser only strips outer brackets). QUOTENAME-escape by
        // doubling any interior ']' so a crafted object name cannot break out of its
        // brackets and inject DDL (paired with the C# semicolon backstop on execute).
        const qn = id => '[' + String(id).replace(/\]/g, ']]') + ']';
        let target = qn(table);
        if (node.objectSchema) target = qn(node.objectSchema) + '.' + target;
        if (node.objectDb) {
            if (!node.objectSchema) target = '[dbo].' + target;
            target = qn(node.objectDb) + '.' + target;
        }

        // Deterministic name from table + leading key columns (same input → same name).
        const nameCols = keyColsFinal.slice(0, 3)
            .map(c => c.replace(/[\[\]]/g, ''))
            .join('_')
            .replace(/[^\w]/g, '_');
        let idxName = 'IX_SQLDBA_' + table.replace(/[^\w]/g, '_') + '_' + nameCols;
        if (idxName.length > 120) idxName = idxName.substring(0, 120);

        const keyLines = keyColsFinal.map((c, i) => (i === 0 ? '  ' + c : '  ,' + c)).join('\n');
        let ddl = `CREATE NONCLUSTERED INDEX [${idxName}]\nON ${target}\n(\n${keyLines}\n)`;
        if (includeCols.length > 0)
            ddl += `\nINCLUDE(${includeCols.map(c => qn(c)).join(', ')})`;
        // {OPTION} placeholders: substituted by QueryPlanModal.ApplyIndexOptions — on execute
        // directly, and for display/copy via the PreviewIndexDdl callback. One substitution,
        // one set of values, so the preview and the executed statement cannot disagree.
        ddl += `\nWITH (ONLINE = {ONLINE}, SORT_IN_TEMPDB = {SORT_IN_TEMPDB}, MAXDOP = {MAXDOP}, DATA_COMPRESSION = {COMPRESSION}{RESUMABLE}{SEQUENTIAL_KEY})`;
    ...
    function render(container, graph) {
        ...
        graph.edges.forEach(edge => {
            ...
            const width = Math.max(1.5, Math.min(18, 1.5 + (Math.log10(rows + 1) / Math.log10(maxRows + 1)) * 16.5));
            ...
            let   edgeColor    = '#5a5a5a';   // neutral (not a status colour)
            ...
            path.style.stroke = edgeColor;    // style (not attribute) so var() resolves
            ...
            // Parser emits the column list as properties['Output'] (comma-joined) —
            // there is no outputList field on nodes.
            const outputProp = targetNode && targetNode.properties ? targetNode.properties['Output'] : null;
            let outputCols = null;
            if (outputProp) {
                const cols = outputProp.split(', ');
                outputCols = cols.slice(0, 8).join(', ') + (cols.length > 8 ? ` (+${cols.length - 8} more)` : '');
            }

            path.addEventListener('mouseenter', ev => {
                let html = `<div class="qp-edge-tip-title">Data Flow</div>`;
                html += `<table class="qp-edge-tip-table">`;
                if (srcNode) html += `<tr><td>From</td><td>${escHtml(srcNode.type || srcNode.physicalType || '')}</td></tr>`;
                if (dstNode) html += `<tr><td>To</td><td>${escHtml(dstNode.type || dstNode.physicalType || '')}</td></tr>`;
                ...
                if (mismatchTip) html += `<tr><td colspan="2" style="color:var(--red);font-size:11px;">${mismatchTip}</td></tr>`;
                if (outputCols) html += `<tr><td>Output</td><td style="font-size:10px;max-width:220px;word-break:break-all;">${escHtml(outputCols)}</td></tr>`;
                html += `</table>`;
            ...
            if (mismatchTip) {
                ...
                warnCirc.style.fill = edgeColor; warnCirc.setAttribute('opacity', '0.9');
            ...
            if (rows > 1) {
                ...
                txt.style.fill = mismatchTip ? edgeColor : '#888';
        ...
        graph.nodes.forEach(node => {
            ...
            if (nodeChildren.length > 0) {
                ...
                collapseBtn.addEventListener('click', (e) => {
                    ...
                    // dataset values are strings — stringify the ids or has() never matches.
                    const hiddenSet = new Set((isCollapsed ? [] : descendants).map(String));
            ...
            nodeDiv.addEventListener('click', (ev) => { 
                showPane(pane, node, nodeDiv); 
                document.querySelectorAll('.qp-node-v2').forEach(el => el.classList.remove('pinned'));
                nodeDiv.classList.add('pinned');
            });
            // The docked sidebar is click-to-pin — it must NOT fade on mouseleave.
            // Only the floating hover pane fades.
            nodeDiv.addEventListener('mouseleave', () => {
                if (pane.id !== 'qp-sidebar') startFade(pane);
            });
        ...
        // and re-apply any active search after a statement-tab re-render.
        _currentNodeElements = nodeElements;
        if (_currentSearchTerm) applySearchHighlight(_currentSearchTerm);
        ...
        if (graph.nodes.length >= 8) {
            ...
            graph.nodes.forEach(n => {
                ...
                rect.style.fill = costColor(n._individualPct ?? n.relativeCost);
    ...
    let _currentNodeElements = {};   // nodeId → div (written by render())
    ...
    function applySearchHighlight(term) {
        ...
        Object.values(_currentNodeElements).forEach(div => {
            if (!lc) {
                ...
                if (text.includes(lc)) {
                    div.style.outline = '2px solid var(--accent, #2196f3)';
                    div.style.opacity = '1';
                    div.style.zIndex  = '10';
                } else {
                    div.style.outline = '';
    ...
    return {
        showPlan: function (containerId, jsonData, planXml) {
            ...
            try {
                ...
                window._qpRenderStatement = function(index) {
                    ...
                    // Scroll the active tab into view inside the (overflow-x: auto)
                    // tab bar so keyboard / prev-next nav doesn't strand the user
                    // on an off-screen statement when there are >10 tabs.
                ...
                // We insert it as a sibling before the container in .query-plan-modal-body.
                {
                    ...
                    // The bar may live one level above the canvas row — search the modal body.
                    const tabHost = container.closest('.query-plan-modal-body') || container.parentElement;
                    const existingTabBar = tabHost
                        ? tabHost.querySelector('.qp-v2-statement-tabs')
                        : null;
                    ...
                    graphs.forEach((graph, idx) => {
                        ...
                        // Label = first real token: strip block comments and leading ;/whitespace
                        const cleaned = (graph.query || '').replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^[\s;]+/, '');
                        const opType = cleaned ? cleaned.split(/\s+/)[0].toUpperCase().substring(0, 12) : 'STATEMENT';
                        ...
                        // textContent (not innerHTML) — opType comes from plan XML
                        const numEl  = document.createElement('span');
                        numEl.className  = 'stmt-tab-num';  numEl.textContent  = String(idx + 1);
                        const typeEl = document.createElement('span');
                        typeEl.className = 'stmt-tab-type'; typeEl.textContent = opType;
                        const pctEl  = document.createElement('span');
                        pctEl.className  = 'stmt-tab-cost'; pctEl.textContent  = pctFixed + '%';
                        tab.append(numEl, typeEl, pctEl);
                        if (warningCount > 0) {
                            const badge = document.createElement('span');
                            badge.className = 'stmt-tab-badge';
                    ...
                    tabBar.addEventListener('keydown', (e) => {
                        if (e.key === 'ArrowLeft') {
                            const cur = window._qpActiveStatementIndex ?? 0;
                            if (cur > 0) { window._qpRenderStatement(cur - 1); e.preventDefault(); }
                        } else if (e.key === 'ArrowRight') {
                            const cur = window._qpActiveStatementIndex ?? 0;
                            if (cur < graphs.length - 1) { window._qpRenderStatement(cur + 1); e.preventDefault(); }
                        }
                    });
                    // Make the tab bar focusable for keyboard nav
                    tabBar.tabIndex = 0;
                    tabBar.setAttribute('role', 'tablist');
                    tabBar.setAttribute('aria-label', 'Statement tabs');
                    ...
                    // Insert tab bar ABOVE the canvas row. Modal body is a flex COLUMN:
                    // [tab bar] then [.qp-body-row = canvas + right sidebar], so the bar
                    // must be a sibling of the row, not of the canvas (which would make
                    // it a vertical strip inside the row).
                    const rowEl = container.parentElement;
                    if (rowEl && rowEl.classList.contains('qp-body-row') && rowEl.parentElement) {
                        rowEl.parentElement.insertBefore(tabBar, rowEl);
                    } else if (rowEl) {
                        rowEl.insertBefore(tabBar, container);
                    } else {
                ...
                if (_compactView) renderContainer.classList.add('qp-v2-compact');
        ...
        clearPlan: function (containerId) {
            ...
            if (el) {
                ...
                const tabHost = el.closest('.query-plan-modal-body') || el.parentElement;
                if (tabHost) {
                    const tabBar = tabHost.querySelector('.qp-v2-statement-tabs');
        ...
        // Compact view — hides secondary node text (object subtext + inline
        // predicate) via the .qp-v2-compact class; layout stays intact.
        setCompactView: function (enabled) {
            _compactView = !!enabled;
            const rc = document.getElementById('qp-render-container');
            if (rc) rc.classList.toggle('qp-v2-compact', _compactView);
        },
        ...
        // ── Register the .NET reference (index preview + No-Pants execution) ──
        setDotNetRef: function (ref) {
            _dotNetRef = ref || null;
        },

        // ── F-B: re-resolve every rendered preview after an option changes ───
        // Called from QueryPlanModal's @bind:after handlers. Without it the preview would be
        // byte-accurate only until the operator ticked a box — the same defect, one click later.
        refreshOptionPreviews: function () {
            const els = document.querySelectorAll('[' + QP_TEMPLATE_ATTR + ']');
            els.forEach(el => {
                const template = el.getAttribute(QP_TEMPLATE_ATTR) || '';
                resolveOptions(template).then(resolved => { el.textContent = resolved; });
            });
        },
