...
window.queryPlanInteropV2 = (function () {
    ...
    // Replace {OPTION} placeholders with safe literals for DISPLAY / COPY.
    // The Execute path sends the placeholder form to .NET, where the
    // No-Pants option checkboxes are substituted instead.
    ...
    function populatePane(pane, node) {
        ...
        while (i < filteredRows.length) {
            ...
            if (isWide) {
                ...
                if (cls === 'pane-row-index') {
                    ...
                    pre.textContent = substituteDefaultOptions(value);
                    td.appendChild(pre);
                } else {
        ...
        copyBtn.addEventListener('click', () => {
            const text = (node.physicalType || node.type) + '\n'
                + rows.filter(([, v]) => v != null && v !== '')
                      .map(([k, v]) => k.padEnd(22) + (k === 'Suggested Index' ? substituteDefaultOptions(v) : v))
                      .join('\n');
        ...
        if (suggestedRow) {
            ...
            copyDdlBtn.addEventListener('click', () => {
                copyToClipboard(substituteDefaultOptions(suggestedRow[1]));
    ...
    function buildSuggestedIndex(node) {
        ...
        // {OPTION} placeholders: substituted by the No-Pants panel on execute,
        // by substituteDefaultOptions() for display/copy.
    ...
    function render(container, graph) {
        ...
        if (graph.recommendations && graph.recommendations.length > 0) {
            ...
            graph.recommendations.forEach((rec, i) => {
                ...
                pre.textContent = substituteDefaultOptions(rec);
