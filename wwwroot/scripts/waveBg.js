...
(function () {
    ...
    // Three-colour palette (uColor1/2/3). Brand-dominant: green → cyan → green-400.
    var COLOR1 = [52, 209, 122];  // #34D17A  signal green (brand accent)
    var COLOR2 = [61, 214, 203];  // #3DD6CB  cyan (secondary HUD)
    var COLOR3 = [91, 229, 140];  // #5BE58C  green-400
    ...
    // The canvas lives at z-index -2, so ANY opaque background on <body> or on a
    // normal-flow descendant paints over it. app.css carries the transparency the
    // backdrop needs, scoped to this marker class, so the flat default keeps its
    // solid chrome and only an active backdrop makes the layers above it see-through.
    function syncBodyMarker() {
        try {
            var on = !!document.getElementById("cl-wave") || !!document.getElementById("cl-particles");
            document.body.classList.toggle("bg-canvas-on", on);
        } catch (e) { }
    }
    ...
    window.waveBgInterop = {
        ...
        // What is ACTUALLY on screen, for the Settings page to report instead of
        // asserting that a selected style is working. Observations only.
        getStatus: function () {
            return {
                style: bgStyle(),
                canvasPresent: !!(canvas && canvas.parentNode),
                marks: marks.length,
                animating: !!running,
