...
window.environmentView = (function () {
    ...
    var C = {
        bg:          'transparent',
        grid:        'rgba(52, 209, 122,0.05)',
        serverFill:  'rgba(52, 209, 122,0.15)',
        serverBorder:'#34D17A',
        serverGlow:  'rgba(52, 209, 122,0.5)',
        serverText:  '#7FF0E6',
        ...
        edgeActive:  'rgba(52, 209, 122,0.6)',
        flowDot:     '#34D17A',
    ...
    var EDGE_KINDS = {
        ...
        AgReplica:             { color: 'rgba(61,214,203,0.9)',   label: 'AG replica' },
        MirrorPartner:         { color: 'rgba(52, 209, 122,0.9)',   label: 'Mirroring' },
    ...
    function drawServerNode(ctx, nd, hover, timeMs) {
        ...
        if (!_rv.dense && !dim) {
            ...
            var glowColor = nd.error ? 'rgba(239,68,68,' : 'rgba(52, 209, 122,';
        ...
        grad.addColorStop(0, nd.error ? 'rgba(239,68,68,0.2)' : 'rgba(52, 209, 122,0.35)');
        grad.addColorStop(1, nd.error ? 'rgba(153,27,27,0.45)' : 'rgba(21,112,63,0.45)');
        ctx.fillStyle = grad;
        ctx.fill();
    ...
    function drawHostNode(ctx, nd, hover, active) {
        ...
        if (hover || active) {
            ctx.save();
            ctx.shadowColor = active ? 'rgba(52, 209, 122,0.6)' : 'rgba(52, 209, 122,0.3)';
            ctx.shadowBlur  = 15;
        }
    ...
    function renderTopology(containerId, jsonData) {
        ...
        function btn(action, lbl, on, title) {
            return '<button data-action="' + action + '" title="' + (title||'') + '" class="' + (on ? 'env-fd-btn-active' : '') +
                '" style="white-space:nowrap;width:auto;' + (on ? 'color:#34D17A;border-color:#34D17A;' : '') + '">' + lbl + '</button>';
        }
        var sep = '<span class="env-fd-sep"></span>';
        // Interactive controls only, grouped View | Filter | Search with separators.
        // The legend was split out (env-fd-legend-box, top-right) so it no longer
        // competes with the controls for one long, hard-to-read pill.
        function toolbarHtml() {
            // View cluster
            var view = '<button data-action="zoomin"   title="Zoom in">+</button>' +
                '<button data-action="zoomout"  title="Zoom out">−</button>' +
                '<button data-action="fit"      title="Fit graph to screen">⤢</button>' +
                '<button data-action="relayout" title="Re-run the force layout (unpins all nodes)" class="env-fd-btn-wide" style="white-space:nowrap;width:auto;">↺ Re-layout</button>';

            // Filter cluster — each sub-group appears only when it has something to act on
            var modeBtns = totalHosts > 0
                ? '<span class="env-fd-grp-label">Group</span>' +
                    btn('cm-all','All',clientMode==='all','Show every server individually') +
                    btn('cm-app','By app',clientMode==='app','Group servers by application') +
                    btn('cm-subnet','By subnet',clientMode==='subnet','Group servers by subnet')
                : '';
            var reachBtns = unreachCount > 0
                ? '<span class="env-fd-grp-label">Unreachable (' + unreachCount + ')</span>' +
                    btn('rm-show','Show',reachMode==='show','Show unreachable servers') +
                    btn('rm-dim','Dim',reachMode==='dim','Dim unreachable servers') +
                    btn('rm-hide','Hide',reachMode==='hide','Hide unreachable servers')
                : '';
            var corralBtn = standaloneNamesCount > 0
                ...
                : '';
            var filter = modeBtns + reachBtns + corralBtn;

            // Search cluster
            var findBox = '<input class="env-fd-find" type="text" placeholder="🔎 find server…" title="Type to highlight servers by name" ' +
                'style="width:120px;height:22px;background:rgba(15,23,42,0.8);border:1px solid rgba(148,163,184,0.3);' +
                'border-radius:4px;color:#e2e8f0;font-size:11px;padding:0 6px;" />';

            return view + (filter ? sep + filter : '') + sep + findBox;
        }
        // Legend — informational only; depends solely on the parsed data, so it is
        // rendered once into its own top-right overlay (never refreshed on toggles).
        function legendHtml() {
            var kindLegend = '';
            var seen = {};
            ...
            return '<span class="env-fd-leg-dot" style="background:#10b981"></span>SQL Server ' +
                '<span class="env-fd-leg-dot" style="background:#6366f1;margin-left:8px"></span>Host ' +
                kindLegend +
                '<span style="margin-left:10px;">Drag to pin · Click a group/corral to expand</span>';
        }
        vp.appendChild(tb);   // controls; innerHTML populated after the graph is built
        var lg = document.createElement('div');
        lg.className = 'env-fd-legend-box';
        lg.innerHTML = legendHtml();   // static per render — the graph counts don't affect it
        vp.appendChild(lg);
