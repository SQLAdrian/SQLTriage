...
file is useless if copied elsewhere. See `CredentialProtector.GetOrCreateAesKey`
in [`Data/CredentialProtector.cs`](https://github.com/SQLAdrian/SQLTriage/blob/main/Data/CredentialProtector.cs).

**The DPAPI `CurrentUser` fallback still writes.** A stored value prefixed
`enc:` is DPAPI at `CurrentUser` scope rather than AES under the machine-scope
key. `CredentialProtector.Decrypt` still decrypts that format on read, for
backward compatibility. It is also still **written**, by two live fallback paths
inside `CredentialProtector.Encrypt`. Both of them call
`EncryptDpapi(plainText, DataProtectionScope.CurrentUser, "enc:")`:

| Fallback path | Fires when |
|---------------|------------|
| `catch (KeyAsideRefusedException refusal)` | `GetOrCreateAesKey` refused to put a machine-scope key in force. It has two arms, described below. |
| `catch (Exception aesEx)` | AES-256-GCM encryption failed for any other reason. |

The refusal has two arms, and they leave the key file in different states. The
distinction matters, because the recovery is different.

- **The key file was not replaced.** `RefuseUnlessPreserved` throws when the
  existing key file would not unwrap and could not be moved safely aside. Nothing
  new was written. The original key file is still on disk.
- **The key file was already replaced.** `GetOrCreateAesKey` throws a second
  time, later in the same call, when the aside did succeed, a fresh key file was
  written, and `KeyAsideLifecycle.ProveReplacement` could not then prove that
  file reads back. The new key is refused as the key in force, because a password
  sealed under a key the next start cannot read comes back empty after a restart
  with nothing said at the time it was saved.

On both arms there is no usable AES key to seal under, and the DPAPI fallback is
the only way the save succeeds at all.

The two fallback paths do not log the same thing, and the difference matters to
anyone reading the log for evidence. The refusal path logs at **Error**
immediately before it falls back, and states the change of scope outright: the
credential was saved for the Windows account running SQLTriage at that moment, it
will read back empty for every other account including the SQLTriage Windows
service, and the key file named in the refusal above it needs fixing. The
AES-failure path logs at **Warning** with the text "AES-GCM encryption failed,
falling back to DPAPI". It does not state the scope change and does not state the
consequence.

If DPAPI then fails as well, neither path saves anything: both log an Error and
`Encrypt` returns the empty string, which the caller stores as a blank password.

The consequence of a successful fallback is worth stating plainly, because it
is a change of scope and not a change of format. An `aes:` credential is
readable by every account on the machine, including the SQLTriage Windows
service. An `enc:` credential is readable only by the Windows account that
wrote it. For any other account, `Decrypt` returns the empty string rather than
raising an error (`CredentialProtector.DecryptDpapi` catches the
`CryptographicException`, logs a Warning and returns empty), so the password
reads back blank and nothing at the point of use says why. The way out is to
repair the credential key file, then save the credential again. It is then
re-encrypted machine-scope as `aes:`.

The account of `CredentialProtector` above was read against
`Data/CredentialProtector.cs` on 2026-08-25 and is cited by symbol name. It
carried line numbers until that date. They were removed because they did not
resolve in the published copy linked above: the published snapshot and the
development tree hold the same symbols at different offsets, so a line number
that is right in one is wrong in the other, and one of them pointed past the end
of the file. Cite a symbol here, not a line. The line numbers elsewhere in this
document were not re-checked on 2026-08-25 and may have the same problem.

Closing the two fallbacks is a code change, and it has not been made.
...
Three roles, with the permission matrix enforced in one place — the `private static HasPermission`
method in
[`RbacService`](https://github.com/SQLAdrian/SQLTriage/blob/main/Data/Services/RbacService.cs)
(no line citation on purpose: the previous one had drifted by ~300 lines and pointed at unrelated
code):
...
- **Single-file, signed-where-possible distribution.** Release builds publish
  as a single self-contained executable (`SQLTriage.csproj` lines 92–95). The
  shipped assembly is not obfuscated: no build path applies ConfuserEx2. Do not
  claim obfuscation until a release pipeline actually performs it.
...
- **Framework-mapped findings.** Most audit checks carry citations to the
  framework controls they satisfy: NIST 800-53, CIS, ISO 27001 and others
