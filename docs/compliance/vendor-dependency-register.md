...
| Microsoft.Data.Sqlite.Core | 8.0.29 (pinned exact) | ADO.NET provider for the local SQLite stores. The `Microsoft.Data.Sqlite` metapackage was dropped 2026-09-02 because its transitive `SQLitePCLRaw.lib.e_sqlite3` 2.1.6 carries GHSA-2m69-gcr7-jv3q / CVE-2025-6965 (NU1903, high, no patched version). Dropping it takes `bundle_e_sqlite3`, `lib.e_sqlite3` and `provider.e_sqlite3` out of the graph; `.Core` depends only on `SQLitePCLRaw.core`, so the cipher row below is untouched. | MIT | 2026-09-03 |
