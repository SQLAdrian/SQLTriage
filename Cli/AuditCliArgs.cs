...
public sealed class AuditCliArgs
{
    ...
    /// <summary>
    /// null (default) = the ordinary --format corpus-check run. "audit-evidence" = a SEPARATE
    /// deliverable: runs the SQL Vulnerability Assessment engine against --servers (the same
    /// engine Pages/VulnerabilityAssessment.razor.cs drives) and writes the same Audit Evidence
    /// PDF/zip the desktop app and portal build, to --out. Not a --format value on purpose — it
    /// is a different engine (VA, not CheckExecutionService) producing a different artifact, and
    /// folding it into --format would read like just another corpus output.
    /// </summary>
    public string? Report { get; init; }

    public static readonly string[] ValidFormats = { "json", "csv", "pdf" };
    public static readonly string[] ValidAuth = { "integrated", "sql" };
    public static readonly string[] ValidReports = { "audit-evidence" };
...
Options:
  ...
  --report <type>       Skip the corpus CHECK run and build a different deliverable
                        instead. A corpus bundle is still required (--bundle, or the
                        shipped default) — it is loaded and validated same as always,
                        because it carries the licence for this run, even though its
                        checks never execute:
                        audit-evidence — runs the SQL Vulnerability Assessment engine
                        against --servers and writes the Audit Evidence PDF/zip (the
                        same artifact the app's Report Bundles page builds) to --out.
                        --format is ignored when --report is given.
...
Exit codes:
  ...
  4  refused by the corpus allocation (instances per 24h). No server was contacted
     ...
     (--report audit-evidence never returns 4 — the VA engine is not metered by this
     allocation, the same as the desktop Vulnerability Assessment page.)
""";
    ...
    public static AuditCliArgsResult Parse(string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            switch (raw.ToLowerInvariant())
            {
                ...
                case "--report":
                    if (++i >= args.Length) return AuditCliArgsResult.Fail("--report requires a value (audit-evidence)");
                    report = args[i].ToLowerInvariant();
                    break;
        ...
        if (report != null && !ValidReports.Contains(report))
            return AuditCliArgsResult.Fail($"--report contains unrecognized value '{report}' (valid: audit-evidence)");
