...
public partial class Settings
{
    ...
    // ── Break-glass recovery password (adminguard-breakglass, 2026-09-01) ──
    ...
    /// <summary>
    /// Sets the break-glass recovery password from Settings ▸ Security &amp; Access. This is the
    /// store-independent AdminGuard credential that gates the loopback recovery surfaces
    /// (Settings, Onboarding). Reachable only when the page rendered, which required
    /// <c>IsAuthorizedWithBreakGlass("settings")</c>; the guard here is defence in depth so a
    /// refactor of the markup gate cannot leave this handler open. AdminAuthService owns the
    /// length rule, the PBKDF2 hashing and the persistence to appsettings.
    /// </summary>
    private void SetRecoveryPassword()
    {
        // Belt-and-braces: this handler mints a credential, so it re-checks the same gate the page
        // rendered behind rather than trusting the render.
        if (!UserState.IsAuthorizedWithBreakGlass("settings")) return;

        _recoveryMessage = null;
        if (_recoveryPassword != _recoveryPasswordConfirm)
        {
            _recoveryMessage = "The two passwords do not match.";
            return;
        }
        ...
        if (error is null)
        {
            ...
            _recoveryMessage = null;
            Toast.ShowSuccess("Recovery password set", "The break-glass recovery surfaces now require it on this machine.");
        }
        else
        {
