...
Razor pages. Audit-first front door is `/cio` `/compliance-map` `/governance` `/diagnostics-roadmap` `/audit` `/fullaudit`; live monitoring companions are `/dashboard/{DashboardId}` `/sessions` `/blocking-forensics` `/waitevents` `/querystore`. See `.claude/docs/pages-index.md` for a per-page summary.
