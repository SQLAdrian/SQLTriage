...
namespace SQLTriage.Data.Parser
{
    ...
    public static class SqlCheckBuilder
    {
            ...
            ISet<string> seenIds)
        {
            ...
            // The structured `remediation:` frontmatter (SCHEMA.md "The remediation block" /
            // "The remediation.operation sub-block") — feeds the app's Remediation UI three-tier
            // contract. Never throws: a malformed block degrades to preview-only (Operation null)
            // and is recorded in infos, but this check still loads (fail-tolerant by design —
            // one bad remediation block must not zero the whole catalogue).
            var remediation = ParseRemediation(yaml, checkId, infos);
            ...
            var check = new SqlCheck
            {
                ...
                RecommendedAction = ScalarOrNull(yaml, "remediation_markdown"), // '## Remediation' markdown section (renamed from "remediation" 2026-07-14 — that key is now the structured frontmatter block)
        ...
        private static bool? BoolOrNull(YamlMappingNode map, string key)
        {
            var s = ScalarOrNull(map, key);
            if (string.IsNullOrEmpty(s)) return null;
            if (bool.TryParse(s, out var b)) return b;
        ...
        private static bool TryFindMapping(YamlMappingNode map, string key, out YamlMappingNode result)
        {
            foreach (var (k, v) in map.Children)
                if (k is YamlScalarNode ks && ks.Value == key && v is YamlMappingNode vm) { result = vm; return true; }
            result = null!;
            return false;
        }

        // ───────── remediation block (SCHEMA.md "The remediation block" / "The remediation.operation sub-block") ─────────

        /// <summary>
        /// Parses the frontmatter `remediation:` mapping (if present). NEVER throws — any parse
        /// problem is recorded as a <see cref="DerivationInfo"/> "malformed" entry and the check
        /// loads with <c>Remediation == null</c> (equivalent to carrying no remediation guidance
        /// at all). This is deliberately more conservative than degrading only the operation
        /// sub-block: a remediation block that doesn't even parse as a mapping correctly is not
        /// safe to trust for CmdletOrTemplate/RiskClass either.
        /// </summary>
        private static CheckRemediation? ParseRemediation(
            IReadOnlyDictionary<string, YamlNode> yaml, string checkId, List<DerivationInfo> infos)
        {
            if (!yaml.TryGetValue("remediation", out var remNode) || remNode is not YamlMappingNode remMap)
                return null; // no remediation guidance on this check — the common case (467 of 575)

            try
            {
                var remediation = new CheckRemediation
                {
                    AutoFixable = BoolOrNull(remMap, "auto_fixable") ?? false,
                    RiskClass = ScalarOrNull(remMap, "risk_class"),
                    Method = ScalarOrNull(remMap, "method"),
                    CmdletOrTemplate = ScalarOrNull(remMap, "cmdlet_or_template"),
                    Reversible = BoolOrNull(remMap, "reversible") ?? false,
                    SnapshotQuery = ScalarOrNull(remMap, "snapshot_query"),
                    VerifyQuery = ScalarOrNull(remMap, "verify_query"),
                };

                if (TryFindMapping(remMap, "operation", out var opMap))
                    remediation.Operation = ParseRemediationOperation(opMap, checkId, infos);

                return remediation;
            }
            catch (Exception ex)
            {
                // Fail-tolerant (REMEDIATION_APP_WIRING_CONTRACT.md §4 rule 3): never let a
                // malformed remediation block zero the WHOLE check. Degrade to "no remediation
                // info" and record why — visible in the load banner, never a silent drop.
                infos.Add(new DerivationInfo(checkId, "remediation", "malformed",
                    $"remediation block failed to parse ({ex.GetType().Name}: {ex.Message}); check loaded without remediation info."));
                return null;
            }
        }

        /// <summary>
        /// Parses the `remediation.operation` sub-block. Returns null (never throws) when the
        /// block is absent, has no <c>op_kind</c>, violates the value_param/value_fixed mutual-
        /// exclusion rule, or (for <c>db_set_option</c>) is missing the required
        /// <c>offenders_query</c> (REMEDIATION_APP_WIRING_CONTRACT.md §4 rule 2 — reject, don't
        /// guess). Any of these degrade the check to the honest preview-only tier; a warning is
        /// recorded via <paramref name="infos"/> either way.
        /// </summary>
        private static CheckRemediationOperation? ParseRemediationOperation(
            YamlMappingNode opMap, string checkId, List<DerivationInfo> infos)
        {
            try
            {
                var opKind = ScalarOrNull(opMap, "op_kind");
                if (string.IsNullOrWhiteSpace(opKind))
                {
                    infos.Add(new DerivationInfo(checkId, "remediation.operation", "malformed",
                        "operation sub-block has no op_kind; check falls back to preview-only."));
                    return null;
                }

                var valueParam = ScalarOrNull(opMap, "value_param");
                var valueFixed = IntOrNull(opMap, "value_fixed");
                if (!string.IsNullOrWhiteSpace(valueParam) && valueFixed.HasValue)
                {
                    infos.Add(new DerivationInfo(checkId, "remediation.operation", "malformed",
                        "value_param and value_fixed are mutually exclusive but both are present; check falls back to preview-only."));
                    return null;
                }

                var op = new CheckRemediationOperation
                {
                    OpKind = opKind,
                    ConfigName = ScalarOrNull(opMap, "config_name"),
                    AdvancedOption = BoolOrNull(opMap, "advanced_option") ?? false,
                    ValueParam = valueParam,
                    MinValue = IntOrNull(opMap, "min_value"),
                    MaxValue = IntOrNull(opMap, "max_value"),
                    ValueFixed = valueFixed,
                    OptionSql = ScalarOrNull(opMap, "option_sql"),
                    OffendersQuery = ScalarOrNull(opMap, "offenders_query"),
                };

                if (string.Equals(op.OpKind, "db_set_option", StringComparison.OrdinalIgnoreCase)
                    && string.IsNullOrWhiteSpace(op.OffendersQuery))
                {
                    infos.Add(new DerivationInfo(checkId, "remediation.operation", "malformed",
                        "op_kind db_set_option requires offenders_query; check falls back to preview-only."));
                    return null;
                }
            ...
            {
                infos.Add(new DerivationInfo(checkId, "remediation.operation", "malformed",
                    $"operation sub-block failed to parse ({ex.GetType().Name}: {ex.Message}); check falls back to preview-only."));
                return null;
            }
        }
    }
