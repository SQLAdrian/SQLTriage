    ...
    public sealed class SourceCatalogueLoader : ICheckCatalogueLoader
    {
        ...
        public static IReadOnlyDictionary<string, YamlNode> ParseMappingFromText(string text, string label)
        {
            ...
            if (!string.IsNullOrWhiteSpace(markdownBody))
            {
                ...
                // NOTE: this is intentionally NOT "remediation" — the frontmatter `remediation:`
                // key (when present) is a structured YamlMappingNode (auto_fixable/operation/etc.,
                // see SCHEMA.md "The remediation block"); writing the markdown prose under the SAME
                // key here would clobber it before SqlCheckBuilder ever sees the structured data.
                // "remediation_markdown" is a distinct synthetic field consumed by SqlCheckBuilder
                // for RecommendedAction only.
                ...
                if (remediationMatch.Success)
                    map["remediation_markdown"] = new YamlScalarNode(remediationMatch.Groups[1].Value.Trim());
