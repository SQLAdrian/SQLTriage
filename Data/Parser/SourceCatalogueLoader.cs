    ...
    public sealed class SourceCatalogueLoader : ICheckCatalogueLoader
    {
        public SourceCatalogue Load(ISourceReader source)
        {
            ...
            foreach (var handle in source.EnumerateCheckHandles())
            {
                ...
                // pair into the rolling hash. Pure bytes; no model serialisation.
            ...
            return new SourceCatalogue(checks, mappings, derivations, integrity);
        }

        // ───────── helpers ─────────

        private static IReadOnlyDictionary<string, YamlNode> ParseMappingFromText(string text, string label)
        {
