...
namespace SQLTriage.Data
{
    ...
    ///
    /// <para><b>Adopted as of 2026-08-05.</b> Refuse: portal-settings.json,
    /// notification-channels.json, remediation-grants.json, user-settings.json,
    /// server-connections.json, remediation-credit-ledger.json (which fails closed on the READ, so
    /// its write guard is a backstop). Announce: alert-thresholds.json, alert-definitions.json,
    /// finding-owners.json, bp-scripts.json, scheduled-tasks.json. Everything else is unguarded.</para>
    /// </summary>
    ...
    public static class ConfigFileHelper
    {
        ...
        public static string DescribeStoreRecovery(ConfigLoadOutcome outcome, string? quarantinedPath)
        {
            if (quarantinedPath != null)
                return $"Restore the file — the unreadable one was copied aside to {Path.GetFileName(quarantinedPath)}, "
                     + "so its contents are not lost — and restart SQLTriage.";

            if (outcome == ConfigLoadOutcome.Empty)
                return "The file on disk is EMPTY, so there was nothing to copy aside and nothing in it to "
                     + "recover — restore it from your own backup, or enter a configuration now, and restart "
                     + "SQLTriage.";

            // Damaged, and the copy itself failed (ACL, full disk). Saying a copy exists here
            // would be the same false claim in a rarer state.
            return "Restore the file from your own backup and restart SQLTriage. No .rejected- copy was kept — "
                 + "the attempt to take one failed, so the original is all there is; move it aside before "
                 + "re-entering settings if you want to keep it.";
        }
