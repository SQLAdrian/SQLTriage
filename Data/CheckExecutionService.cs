    ...
    public class CheckExecutionService : IDisposable
    {
            ...
            SemaphoreSlim throttle, int engineEdition, CancellationToken ct)
        {
            var result = new CheckResult
            {
                ...
                DefinitionFlaggedAdverse = check.IsBad,
            ...
            catch (SqlException sqlEx) when (TryMapExpectedConditionSkip(sqlEx, out var skipReason))
            {
                // Expected condition — either a permission denial, or an availability-group
                // replica that is not readable from this connection. Treat as SKIP (the check
                // is not applicable because this connection cannot see what it needs). SKIP is
        ...
        // ── Expected-condition → SKIP mapping ────────────────────────────────────
        //
        ...
        //
        // THE CONTRACT. Exactly TWO families map to SKIP:
        //   1. Canonical permission-denied SQL error numbers — the switch below.
        //   2. Availability-group non-readability — via the shared predicate
        //      AvailabilityGroupReadability.IsNotReadableOnThisReplica, never an
        //      inline number list. Add a call site there, not here.
        // Everything else falls through to the catch-all Exception handler and
        // surfaces as ERROR. Keep it that way.
        //
        // ⚠ SKIP IS A SEVERITY DOWNGRADE, so the bar for adding one is high. A SKIP sets
        // Passed = true, is filtered out of summary.Errors by IsSkipMessage(), and is
        // excluded from scoring by CheckClassification.IsScorable — it lands in
        // summary.Informational, neither numerator nor denominator. A condition belongs
        // here only when it is EXPECTED and PERMANENT for where we are connected. Both
        // families qualify: a missing permission and a non-readable replica each return
        // the identical answer on every retry until a human changes something.
        //
        // ⚠ SQL 924 (single-user mode) WAS ADDED HERE ON 2026-09-14 AND REMOVED THE SAME DAY.
        // Do not re-add it. It is an operational state, not a permission denial, and it is
        // TRANSIENT — a retry may succeed. No check in the catalogue detects single-user mode
        // by any other means, so the ERROR this method would have suppressed is the audit's
        // ONLY signal for it. To report it without failing the run, the tier is WARN, not SKIP.
        //
        // WHAT PINS THIS, precisely — do not read more assurance into it than it carries:
        //   · The AG number list (976/978/979/983) is pinned by AvailabilityGroupReadabilityTests.
        //   · This method's mapping is pinned by CheckExecutionServiceOperationalSkipTests.
        //   · ⚠ The WIRING IS NOT PINNED. No census asserts that the filter at the single call
        //     site (search: TryMapExpectedConditionSkip) still sits on a verdict-bearing path.
        //     The 2026-09-12 lane (082558ec) proved a guard can be wired, run, and still be
        //     compiled into the WRONG METHOD — source placement could not tell them apart, only
        //     an IL census could. If you move this call site, prove the new one withholds a
        //     verdict before you believe the guard still works.
        internal static bool TryMapExpectedConditionSkip(SqlException sqlEx, out string reason)
        {
            if (AvailabilityGroupReadability.IsNotReadableOnThisReplica(sqlEx))
            {
                reason = "this database is an availability-group replica that is not readable through this connection. This is expected, not a fault. Run against the replica where it is primary, or enable read access on the secondary. Nothing was changed.";
                return true;
            }
