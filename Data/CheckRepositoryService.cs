...
namespace SQLTriage.Data
{
    ...
    public class CheckRepositoryService
    {
        ...
        public async Task LoadChecksAsync()
        {
            ...
            if (_configuration?.GetValue("CheckRepository:UseSourceParser", false) ?? false)
            {
                var src = _configuration?.GetValue<string?>("CheckRepository:SourceParserPath");
        ...
        private void LoadViaBundle()
        {
            ...
            try
            {
                ...
                var handles = _bundle.EnumerateCorpusYamlHandles()
                    .Where(h => h.EndsWith(".yaml", StringComparison.OrdinalIgnoreCase))
                    .Select(h => new { Handle = h.Substring(0, h.Length - 5), YamlKey = h })
                    .ToList();
