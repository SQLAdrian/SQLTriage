    ...
    public class CheckRepositoryService
    {
        ...
        private void LoadViaBundle()
        {
            ...
            try
            {
                ...
                LogMalformedRemediationDerivations(catalogue.Derivations);
            }
        ...
        // Task #39 — SqlCheckBuilder never throws on a malformed `remediation:`/`operation:`
        // block (that check just loads with Remediation/Operation null, degraded to the honest
        // preview-only tier). That degrade is otherwise invisible at the app-log level — surface
        // it here as a targeted, per-check warning, distinct from the generic >50%-derivation
        // banner (which only reports a count, not which fields or why).
        private void LogMalformedRemediationDerivations(IReadOnlyList<SQLTriage.Data.Parser.DerivationInfo> derivations)
        {
            foreach (var d in derivations)
            {
                if (d.Source != "malformed") continue;
                if (d.Field != "remediation" && d.Field != "remediation.operation") continue;
                _logger.LogWarning(
                    "Check '{CheckId}' remediation block degraded to preview-only ({Field}): {Reason}",
                    d.CheckId, d.Field, d.Rationale);
            }
        }
