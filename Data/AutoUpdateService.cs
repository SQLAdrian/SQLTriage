...
namespace SQLTriage.Data
{
    ...
    /// <summary>
    /// Outcome of the most recent update check. Distinguishes "no newer version exists" from
    /// "we never asked" — a check that was disabled or that failed must not be reported to the
    /// user as being up to date.
    /// </summary>
    public enum UpdateCheckState
    {
        /// <summary>No check has been performed in this process yet.</summary>
        NotChecked,
        /// <summary>The endpoint answered and this build is current.</summary>
        UpToDate,
        /// <summary>The endpoint answered and a newer release exists.</summary>
        UpdateAvailable,
        /// <summary>Updates:Enabled is false — nothing was contacted.</summary>
        Disabled,
        /// <summary>The check was attempted and errored; see LastCheckError.</summary>
        Failed
    }
    ...
    public class AutoUpdateService
    {
        ...
        /// <summary>
        /// 0 = no background check started yet, 1 = one has been claimed. Guards
        /// <see cref="StartBackgroundCheck"/>: <see cref="LastCheckResult"/> is set only AFTER the
        /// 15s-delayed check completes, so the "already checked" test alone is racy — a Blazor
        /// Server prerender pass and the interactive circuit that follows it both call
        /// OnInitializedAsync on this same singleton within that window, and two concurrent
        /// circuits (e.g. two tabs opened together) hit the same window too. Interlocked makes the
        /// claim atomic so only the first caller schedules the delayed check.
        /// </summary>
        private int _backgroundCheckClaimed;
        ...
        /// <summary>
        /// Outcome of the last <see cref="CheckForUpdatesAsync"/> call. Callers must use this rather
        /// than inferring "up to date" from a false Available flag — Available is also false when
        /// updates are disabled or the check failed.
        /// </summary>
        public UpdateCheckState LastCheckState { get; private set; } = UpdateCheckState.NotChecked;

        /// <summary>False when "Updates:Enabled" is false in configuration — the whole update subsystem is inert.</summary>
        public bool UpdatesEnabled => _updatesEnabled;
        ...
        public async Task<(bool Available, UpdateInfo? Info)> CheckForUpdatesAsync()
        {
            ...
            if (!_updatesEnabled)
            {
                LastCheckState = UpdateCheckState.Disabled;
        ...
        public void StartBackgroundCheck()
        {
            ...
            // Atomic claim: the very next line schedules the only 15s-delayed check this process
            // will run. Without this, a second caller inside the delay window (prerender +
            // interactive, or two circuits initializing together) reads LastCheckResult as still
            // null above and schedules a second one.
            if (Interlocked.CompareExchange(ref _backgroundCheckClaimed, 1, 0) != 0)
                return; // another caller already claimed the check
        ...
        public void ApplyUpdateOnExit()
        {
            ...
            try
            {
                ...
                // Two appliers, in preference order.
                //
                // (1) The managed out-of-process updater (updater\SQLTriageUpdater.exe): runs from a
                //     sibling dir so it can swap the install dir without file-lock fights, preserves
                //     user config, and rolls back if the new build fails to start. HONESTY NOTE
                //     (platform-r1-06): tools/SQLTriageUpdater is NOT in SQLTriage.sln, no build or
                //     installer copies it, and no shipped install has an updater\ folder — so this
                //     branch is never taken on any client today. Its rollback-on-failed-start
                //     protection has never reached a client. Do not describe it as the norm until it
                //     actually ships (shipping it is a separate, un-taken product decision).
                // (2) The legacy .cmd applier is therefore what EVERY current install runs. It backs
                //     up and restores ProtectedConfigFiles, but it has NO rollback: if the new build
                //     fails to start, the old one is already overwritten.
        ...
        ///
        /// This is the list the legacy .cmd applier actually executes (it is the only applier any
        /// install ships — see ApplyUpdateOnExit). It must stay in step with the managed updater's
        /// own copy (tools/SQLTriageUpdater/Program.cs ProtectedConfig); the two had already drifted
        /// by one — the updater protected config\.sqlite-cipher-key and this list did not, so an
        /// update package carrying a runtime-generated cipher key would let robocopy overwrite it and
        /// leave the client's cache DBs unreadable (honesty-hunt platform-r1-10).
        /// Tests\SQLTriage.Tests\ProtectedConfigParityTests.cs:201 (Runtime_and_updater_protected_lists_are_identical)
        /// (Runtime_and_updater_protected_lists_are_identical) is the tripwire that keeps the two lists
        /// identical, and Each_protected_entry_keeps_its_directory pins the folder each entry lives in.
        /// The name cited here until 2026-09-10, "ProtectedConfigFilesMatchTheUpdater", is not a test
        /// that exists anywhere in the repo.
        /// </summary>
        private static readonly string[] ProtectedConfigFiles = new[]
        {
            "config\\server-connections.json",
            "config\\alert-definitions.json",
            // AlertingService's own store since 2026-08-04 — see SQLTriageUpdater's matching list.
            ...
            // Runtime-generated SQLite cipher key (SqliteCipherHelper). Without it a package that
            // carried a fresh key would overwrite the client's and make cached DBs unreadable.
            "config\\.sqlite-cipher-key",
            // Added 2026-09-10 (lane customer-update-path, round two) after a census found it in NONE
            // of the four lists that protect config state, while .sqlite-cipher-key was in three.
            // Data\CredentialProtector.cs:41 resolves it at
            // Path.Combine(AppContext.BaseDirectory, "config", ".credential-key") and it is the key
            // every stored credential is encrypted with - the same failure mode as the cipher key one
            // line up: an update package carrying a fresh key would leave the client unable to decrypt
            // credentials they had already saved. It is written lazily, on the first credential stored,
            // so an install may legitimately not have one; IsProtected only ever SKIPS a copy, so an
            // entry naming an absent file costs nothing.
            "config\\.credential-key",
            // Runtime-generated governance HMAC key (GovernanceHistoryService). This entry carries
            // NO directory segment ON PURPOSE: that constructor writes the key to
            // AppDomain.CurrentDomain.BaseDirectory, the app ROOT, not under config\. Writing it as
            // config\.governance-hmac-key would protect a path the app never uses, and the backup
            // line below keys the backup copy on Path.GetFileName, so it would silently back up
            // nothing. Losing this key does not just lose a secret: every row already in the
            // client's governance-history.db was signed with it, so a replacement key makes the
            // whole prior chain verify as tampered.
            ".governance-hmac-key",
        };
