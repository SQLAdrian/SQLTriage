        ...
        public void UpdateSuccessfulServers(string connectionId, List<string> successfulServers)
        {
            lock (_lock)
            {
                ...
                if (connection != null)
                {
                    ...
                    // pages-r1-03: this is the single place a probe's result reaches the estate, so
                    // it is the single place that can mark the status as measured NOW. The flag is
                    // [JsonIgnore], so a restart loads every connection as unchecked and no surface
                    // can render a stored status as a current one.
