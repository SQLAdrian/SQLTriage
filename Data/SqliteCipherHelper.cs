...
namespace SQLTriage.Data
{
    ...
    public static class SqliteCipherHelper
    {
        ...
        // ⚠ REVIEWED 2026-08-20 for the C1 DPAPI-scope ruling: KeyFilePath sits under
        // AppContext.BaseDirectory (the shared install folder), and this class is wired into both
        // hosts via AddSharedServices — same pattern, same reasoning as CredentialProtector's
        // .credential-key (see that class's 2026-08-20 note). The SQLite stores this key opens
        // (check-baselines.db and the config baselines) hold operator-entered data read by whichever
        // process is currently running, interactive or service. Genuinely shared; stays LocalMachine.
        ...
        private static readonly System.Collections.Generic.Dictionary<string, ReinitFallback> FallbackByStore =
            new(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // 2026-09-05: SQLTriage.db is GONE from this registry, not merely unused. Its only
                // opener was liveQueriesTableService (Data/SQLiteTableService.cs), deleted with the
                // panel-provisioning surface; a grep of the tree on that date found no other
                // producer or reader. This registry is a census of stores the APP OPENS, and
                // Every_store_this_application_opens_is_in_the_fallback_registry enforces set
                // equality in both directions against a scan of the source — so an entry whose
                // opener has gone is dead text and fails there. It caught this one. The Refuse
                // default for an unlisted name cannot bite a store nothing opens: FallbackPolicyFor
                // is never reached for it. Installs made before this commit may still carry the
                // file on disk; nothing in the product touches it.
