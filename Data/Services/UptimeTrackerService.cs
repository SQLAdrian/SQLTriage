...
namespace SQLTriage.Data.Services
{
    ...
    public sealed class UptimeTrackerService : IDisposable
    {
            ...
            bool startTimer = true)
        {
            ...
            InitializeSchema();
            // 2026-08-05: WriteEvent swallows its own failures, and this line logged "started"
            // regardless. An operator reading the log saw a session begin while the uptime store
            // held no row for it, so a later availability figure was computed over a window whose
            // start was never recorded. The line now reports what the write returned.
            if (WriteEvent("session_start"))
                _logger.LogInformation("[UPTIME] Session {SessionId} started", _sessionId);
            else
                _logger.LogWarning(
                    "[UPTIME] Session {SessionId} start was NOT recorded — the write failed, so "
                    + "availability computed over a window containing this moment is understated",
                    _sessionId);

            if (startTimer)
            {
                var intervalMs = TimeSpan.FromMinutes(DefaultHeartbeatMinutes).TotalMilliseconds;
                _heartbeatTimer = new Timer(_ => { _ = WriteEvent("heartbeat"); }, null,
        ...
        /// <summary>
        /// Writes one uptime row. Returns whether the row was written (2026-08-05): this used to
        /// return void and swallow every failure, so a caller could not tell a recorded event from
        /// a lost one, and logged the recorded sentence either way.
        /// </summary>
        private bool WriteEvent(string eventType)
        {
            try
            {
                using var conn = SqliteCipherHelper.OpenEncrypted(_connectionString);
        ...
        public void Dispose()
        {
            ...
            // Same law as the start line above: the sentence follows the write outcome.
            if (WriteEvent("session_end"))
                _logger.LogInformation("[UPTIME] Session {SessionId} ended", _sessionId);
            else
                _logger.LogWarning(
                    "[UPTIME] Session {SessionId} end was NOT recorded — the write failed, so this "
                    + "session has no closing row", _sessionId);
        }
    }
}
