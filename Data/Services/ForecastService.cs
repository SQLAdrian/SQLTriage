...
namespace SQLTriage.Data.Services
{
    ...
    public class ForecastService
    {
        ...
        public record ForecastResult
        {
            ...
            /// <summary>
            /// Whether <see cref="SlopePerDay"/> came out of a fitted regression at all. Defaults to
            /// FALSE, so a result assembled by hand claims no direction until something sets it —
            /// <see cref="CalculateLinearForecast"/> sets it on the one path that actually fits a
            /// line, and not on the degenerate path where every observation shares a timestamp and
            /// the slope is returned as a hardcoded 0.
            ///
            /// <para>2026-08-09. /capacity-planning built its disk rows by hand from a single live
            /// <c>dm_os_volume_stats</c> reading with <c>SlopePerDay = 0, DataPointCount = 1</c> —
            /// no regression anywhere — and the card then printed a "Stable" badge, "Trend:
            /// +0.00 GB/day" and "R²: 0.000" for every volume. Same over-claim class as SR-11: a
            /// zero that is the absence of a measurement, rendered as the measurement of an absence.
            /// Read <see cref="MeasuredSlopePerDay"/>, never <see cref="SlopePerDay"/>, to decide
            /// what to print.</para>
            /// </summary>
            public bool SlopeFitted { get; init; }

            /// <summary>
            /// The rate of change, or null when no line was ever fitted. Same shape and the same
            /// reason as <c>ExecutiveHealthScore.MeasuredScore</c>: <see cref="SlopePerDay"/> has to
            /// hold a double whatever happened, so the placeholder 0 must exist; printing it is the
            /// defect.
            /// </summary>
            public double? MeasuredSlopePerDay => SlopeFitted ? SlopePerDay : null;
        }
        ...
        /// <summary>
        /// Forecasts a series for every instance that has cached data for the query,
        /// discovered from the cache itself (there is no aggregate "__all__" key).
        /// </summary>
        public async Task<List<(string InstanceKey, ForecastResult Result)>> ForecastAllInstancesAsync(
            string queryId, string seriesName, double threshold, int lookbackHours = 168)
        {
            var results = new List<(string, ForecastResult)>();
            try
            {
                var from = DateTime.UtcNow.AddHours(-lookbackHours);
                var to = DateTime.UtcNow;
                foreach (var instanceKey in await _cache.GetTimeSeriesInstanceKeysAsync(queryId, from, to))
                {
                    var result = await ForecastAsync(queryId, instanceKey, seriesName, threshold, lookbackHours);
                    if (result != null)
                        results.Add((instanceKey, result));
                }
            }
            catch (Exception ex)
            {
                _logger.LogDebug(ex, "Per-instance forecast enumeration failed for {QueryId}/{Series}", queryId, seriesName);
            }
            return results;
        }
        ...
        public static ForecastResult CalculateLinearForecast(List<TimeSeriesPoint> data, double threshold)
        {
            ...
            // No spread on X (every observation shares a timestamp), so no line can be fitted. The 0
            // below is the absence of a slope, not a measured flat one — SlopeFitted stays false.
            ...
            return new ForecastResult
            {
                ...
                DataPointCount = n,
                SlopeFitted = true          // the one path that fits a line
            };
        }
    }
}
