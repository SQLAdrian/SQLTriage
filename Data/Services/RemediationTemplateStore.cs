...
namespace SQLTriage.Data.Services
{
    public class RemediationTemplateStore
    {
        ...
        public int CorpusMalformedCount { get; private set; }
        ...
        public void LoadCorpusTemplates(IEnumerable<SqlCheck>? checks)
        {
            ...
            int oneClick = 0, deduped = 0, malformed = 0;
            lock (_lock)
            {
                ...
                foreach (var check in checks)
                {
                    ...
                    try { candidate = BuildTemplateFromCheck(check, remediation, _logger); }
                    catch (Exception ex)
                    {
                        malformed++;
                ...
                CorpusMalformedCount = malformed;
            }
        ...
        private static RemediationTemplate? BuildTemplateFromCheck(SqlCheck check, CheckRemediation remediation, ILogger logger)
        {
            ...
            {
                case "sp_configure":
                    if (string.IsNullOrWhiteSpace(src.ConfigName)) return null;
                    operation = new RemediationOperation
                    {
                        OpKind = RemediationOpKind.SpConfigure,
                ...
                case "db_set_option":
                    if (string.IsNullOrWhiteSpace(src.OptionSql) || string.IsNullOrWhiteSpace(src.OffendersQuery))
                        return null; // hard contract: db_set_option REQUIRES offenders_query
                    operation = new RemediationOperation
                    {
                    ...
                    break;
                default:
                    return null; // unsupported op_kind (e.g. create_index — reserved, no corpus entries yet)
            }
