...
namespace SQLTriage.Data.Services
{
    public class BuildCatalogueService
    {
        ...
        // Staleness surface (#82): the shipped catalogue is a publish-time snapshot
        // (scripts/update-build-catalogue.py). Age is derived from the catalogue's own
        // "lastUpdated" stamp, never a hardcoded date, so it stays honest across builds.
        private const int StaleThresholdDays = 30;

        /// <summary>
        /// Whole days between the catalogue's own "lastUpdated" timestamp and the reference
        /// date (pinned in tests, wall-clock in production), or null if the catalogue is
        /// unavailable or the timestamp is missing/unparsable — never guessed.
        /// </summary>
        public int? AgeInDays
        {
            get
            {
                var stamp = LastUpdated;
                if (string.IsNullOrWhiteSpace(stamp)) return null;
                if (!DateTime.TryParse(stamp, CultureInfo.InvariantCulture,
                        DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal, out var parsed))
                    return null;
                var days = (_referenceDate.Date - parsed.Date).Days;
                return days < 0 ? 0 : days;
            }
        }

        /// <summary>True once the catalogue is older than the 30-day staleness threshold.</summary>
        public bool IsStale => AgeInDays is { } age && age > StaleThresholdDays;
