        ...
        public async Task<string> PrepareDbaHandoffHtmlAsync(string serverName)
        {
            ...
            else
            {
                ...
                foreach (var f in allFindings)
                {
                    ...
                    sb.Append($"<tr><td>{EscapeHtml(f.Id)}</td><td class=\"sev sev-{f.Severity?.ToLowerInvariant()}\">{EscapeHtml(f.Severity)}</td><td>{EscapeHtml(f.Name)}</td><td>{EscapeHtml(f.Category)}</td><td>{EscapeHtml(f.Source)}</td><td>{EscapeHtml(PlainText(f.Message))}</td></tr>");
                }
                sb.Append("</tbody></table>");
            }
            sb.Append("</section>");
