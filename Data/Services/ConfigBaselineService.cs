...
namespace SQLTriage.Data.Services
{
    ...
    public sealed class ConfigBaselineService
    {
        ...
        private static readonly string[] ExcludedPatterns =
        {
            ...
            "sqlite-cipher-key",
            "cipher-key",
            // Added 2026-09-10 for consistency with the three update-protection lists, which now all
            // name it. TrackedGlobs above reaches only appsettings.json and Config/*.json, so a
            // dotfile with no extension is out of scope today and this entry changes no behaviour; it
            // is here so that a future glob widening cannot start hashing a credential key by
            // accident. ".credentials" below does NOT cover ".credential-key" - different string.
            "credential-key",
            ".credentials",
        };
        ...
        private void CheckDrift(string baselinePath)
        {
            ...
            if (stored == null)
            {
                // Triaged against the write-guard tier rule on 2026-08-05. There is no silent
                // OVERWRITE to refuse here — CreateBaseline runs only when the file is Missing, and
                // otherwise only from ReBaseline(actor), which is an admin naming the consequence,
                // i.e. the ReplaceUnreadableStore intent expressed as an operator action. What was
                // wrong was the READ: this returned with no log at all, DriftDetected stayed false,
                // and CM-3 drift detection was simply not running while every surface that asks
                // reads "no drift". A control that reports clean when it is blind is the exact
                // shape this lane keeps finding, so it says so now.
                _logger.LogError(
                    "[CONFIG-BASELINE] {Path} exists and could not be read, so NO configuration drift check ran "
                    + "this startup. DriftDetected is false because nothing was compared, NOT because nothing "
                    + "changed. Re-baseline from Settings once the current configuration is confirmed correct.",
                    baselinePath);
                DriftDetected = false;
        ...
        private void CreateBaseline(string baselinePath, string actor)
        {
            ...
            // Windows: File.WriteAllText over an existing Hidden file (we set
            // Hidden below) — or a ReadOnly file from MSBuild content-copy of
            // Config/* — throws UnauthorizedAccessException. Clear attributes
            // first so re-baseline can overwrite the prior snapshot.
            if (File.Exists(baselinePath))
            {
                try { File.SetAttributes(baselinePath, FileAttributes.Normal); } catch { }
            }
