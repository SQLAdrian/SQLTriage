...
namespace SQLTriage.Data.Services
{
    ...
    /// </summary>
    public class RbacService : IRbacEnforcementPostureAccessor
    {
        ...
        /// <summary>
        /// The KIND of that last report, kept beside the key because the key alone cannot answer
        /// "did we just leave <see cref="PostureKind.Off"/>". The bootstrap-admin audit pair — one
        /// entry when the posture starts, one when it ends — is a statement about a TRANSITION, and
        /// a transition needs both ends. <c>null</c> means nothing has been reported yet, which is
        /// why a process that boots straight into Enforcing writes no "ended" entry for a posture it
        /// was never in.
        /// </summary>
        private PostureKind? _reportedKind;

        /// <summary>
        /// The audit sink, or <c>null</c> when the caller did not supply one (every test seam, and
        /// any host that builds this service without an audit log). Optional on purpose: a missing
        /// audit log must not stop RBAC loading, and <see cref="AuditLogService"/> depends on
        /// nothing in this class, so the reference is one-way and cannot cycle through DI.
        /// </summary>
        ...
        /// <summary>
        /// The DI constructor. <paramref name="audit"/> is optional so every existing
        /// <c>new RbacService(logger)</c> call site keeps compiling and so a host without an audit
        /// log still builds; the container fills it because <c>AuditLogService</c> is a registered
        /// singleton (ServiceCollectionExtensions, WindowsServiceHost).
        /// </summary>
        public RbacService(ILogger<RbacService> logger, AuditLogService? audit = null)
            : this(logger,
        ...
        public RbacService(ILogger<RbacService> logger, string configPath, string usersPath, AuditLogService? audit = null)
        {
            _logger = logger;
        ...
        /// <summary>
        /// The consequence of <see cref="PostureKind.Off"/>, in the words the Settings page has
        /// used since the remote-bootstrap switch was deleted (Pages/Settings.razor, "Setting up
        /// the first administrator").
        ///
        /// <para>⚠ IT LIVES HERE, not in the markup, because the shell banner's rule is that every
        /// word it renders is COMPUTED — the same rule the Lapsed and ConfigDidNotLoad arms already
        /// follow. A literal sitting beside a computed condition is the exact defect the Settings
        /// banner shipped seven rounds running, and putting this sentence in MainLayout.razor would
        /// reintroduce it one banner to the left.</para>
        ///
        /// <para>⚠ It states SCOPE, not just state. "Access control is off" invites an operator to
        /// assume the LAN can reach an admin session; <see cref="IsBootstrapEligible"/> says
        /// otherwise and nothing can widen it, so the sentence that ships beside the headline has
        /// to say so or the loud banner makes the install sound worse than it is.</para>
        /// </summary>
        internal const string BootstrapAdminScope =
            "Setting up the first administrator: while access control is unconfigured, SQLTriage serves every "
            + "connection from this machine as a full administrator. That holds only for connections from this "
            + "machine, and no setting can extend it to the network.";
        ...
        public EnforcementPosture DescribeEnforcementPosture()
        {
            lock (_lock)
            {
                ...
                return _config.Enabled
                    ...
                    : new EnforcementPosture(PostureKind.Off, "Access control is switched OFF.", BootstrapAdminScope, reasons);
            }
        }
        ...
        private void ReportEnforcementPosture(string trigger)
        {
            ...
            lock (_lock)
            {
                ...
                previousKind = _reportedKind;
                _reportedPosture = key;
                _reportedKind = posture.Kind;
            }
            ...
            else if (posture.Kind == PostureKind.Off)
                // ⚠ WARNING, NOT INFORMATION — Adrian's ruling of 2026-09-06 ("accept and be
                // loud"). An unconfigured install serves every loopback caller as a full
                // administrator; that is a deliberate, defended state, so nothing here refuses it,
                // but a headless service has no screen and INFORMATION is where an operator's log
                // level starts filtering. The level says "read me"; the Detail says how narrow the
                // hatch actually is, so the loudness does not overstate the exposure.
                _logger.LogWarning("[RBAC] {Headline} ({Trigger}) {Detail}", posture.Headline, trigger, posture.Detail);
            else
                _logger.LogInformation("[RBAC] {Headline} ({Trigger})", posture.Headline, trigger);

            // The audit half of the same ruling. Once per TRANSITION, off the same dedupe the log
            // uses, so this is never per request and never per render: one entry when the
            // bootstrap-admin posture starts, one when it ends. `previousKind` is null on the very
            // first report, so a process that boots straight into Enforcing writes neither.
            //
            // enforcedNow tells the audit method whether the wide-open window actually CLOSED.
            // Leaving Off only closes it when the destination is Enforcing; Off -> Lapsed and
            // Off -> ConfigDidNotLoad still serve every request as though access control were off,
            // so the "ended" claim would over-state the same way the enforcement banner once did.
            if (posture.Kind == PostureKind.Off)
                _audit?.LogBootstrapAdminPosture(active: true, trigger, posture.Headline, posture.Detail);
            else if (previousKind == PostureKind.Off)
                _audit?.LogBootstrapAdminPosture(active: false, trigger, posture.Headline, posture.Detail,
                    enforcedNow: posture.Kind == PostureKind.Enforcing);
        }
        ...
        public bool IsAuthorized(string role, string permission, AppUserState.BootstrapEligibilityProof? bootstrapProof)
        {
            ...
            if (!IsRbacEnforced())
            {
                // security-8 (2026-08-31): a previously-ENFORCED install whose store was later
                // DAMAGED flips IsRbacEnforced() false, and before this branch that handed the
                // anon loopback caller full admin through bootstrapEligible — a silent
                // enforced→unenforced downgrade on every IsAuthorized-gated page. The bootstrap
                // hatch exists for a NEVER-configured install (PostureKind.Off), not for one that
                // meant to enforce and cannot. When the posture is a PROBLEM (Lapsed /
                // ConfigDidNotLoad) the eligible caller collapses to the SAME result an enforced
                // install returns — HasPermission(role, permission), i.e. viewer for an
                // unauthenticated circuit — never the hatch. This touches ONLY the IsProblem
                // branch: never-configured (Off) keeps the hatch and stays byte-identical (B1).
                //
                // The recovery surfaces (Onboarding, Settings ▸ Access Control) are NOT stranded
                // by this: they are reached through AppUserState.IsAuthorizedWithBreakGlass, which
                // is independent of this decision (loopback + browser-hosted, unconditional), so a
                // lapsed install is still repairable from the box. The re-seed WRITE those surfaces
                // perform is governed by the store write guard (SaveUsers / WouldOverwriteUnreadStore),
                // and re-seeding creates the admin CREDENTIAL without signing this session in —
                // the caller stays viewer here until they AUTHENTICATE as the re-seeded admin.
                if (DescribeEnforcementPosture().IsProblem)
