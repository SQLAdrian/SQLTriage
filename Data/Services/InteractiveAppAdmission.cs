...
namespace SQLTriage.Data.Services
{
    ...
    public static class InteractiveAppAdmission
    {
        ...
        /// <summary>
        /// The dedup set above is BOUNDED (architecture-12, 2026-08-31). Every entry is one remote
        /// address string, added on refusal and never removed, so an address-rotating scanner —
        /// IPv6 makes 2^64 source addresses on one /64 free — would grow it for the whole process
        /// lifetime. It is only a log-noise optimisation, so a hard cap costs nothing real: past
        /// the cap a genuinely new address logs at Debug rather than Information, which is the
        /// correct volume for a caller arriving after thousands of distinct others have already
        /// been announced. The bound is the point; the exact number is not.
        /// </summary>
        internal const int MaxReportedRemotes = 4096;

        /// <summary>
        /// True the first time this remote should get an Information line, false thereafter — and
        /// false, without growing the set, once <see cref="MaxReportedRemotes"/> distinct addresses
        /// have been recorded. Split out from <see cref="Report"/> so the bound can be exercised
        /// directly: the middleware path that reaches Report needs a full HTTP pipeline, and a
        /// bound nobody can measure is a bound nobody keeps.
        /// </summary>
        internal static bool ShouldAnnounceRefusal(string remote)
            => _reported.Count < MaxReportedRemotes && _reported.TryAdd(remote, 0);

        /// <summary>How many distinct refused addresses are currently tracked. Test seam.</summary>
        internal static int ReportedRemoteCount => _reported.Count;

        /// <summary>Clears the dedup set. Test seam only — production never resets it.</summary>
        internal static void ResetReportedForTests() => _reported.Clear();
        ...
        {
            // REFUSE TO BUILD if the Host gate is not already installed. The admission boundary
            // exempts loopback, and a DNS-rebinding request arrives ON loopback carrying a foreign
            // Host header — so a Host check installed BEHIND admission never sees the request it
            // exists to refuse, because admission has already admitted it. HostOriginValidation must
            // run first. Same doctrine as UseSqlTriageAuth's refusal below: a service that will not
            // start is a five-minute fix; one that starts with the check mis-ordered is a silent
            // reopening of the loopback hatch. UseSqlTriageFrontDoor is what fixes the position, in
            // one place, for both hosts. (WebApplication implements IApplicationBuilder explicitly.)
            if (!((IApplicationBuilder)app).Properties.ContainsKey(HostOriginValidation.HostOriginInstalledKey))
                throw new InvalidOperationException(
                    "UseSqlTriageHostOrigin must be installed before UseSqlTriageAdmission. Call "
                    + "app.UseSqlTriageFrontDoor(rbac) first. It installs the raw-Host allow-list ahead of "
                    + "the admission boundary. Without it, a DNS-rebinding request arriving on loopback with "
                    + "a foreign Host header is admitted by the loopback exemption, which is the hatch the "
                    + "Host check exists to close.");
        ...
        {
            // The raw-Host allow-list goes at the VERY TOP — ahead of admission, so a DNS-rebinding
            // request carrying a foreign Host is refused before the loopback exemption can admit it.
            // It reads Request.Host on every request (documents, static assets, the /_blazor
            // negotiate POST and its WebSocket upgrade alike), because it runs before UseRouting and
            // branches on nothing but the Host header.
            app.UseSqlTriageHostOrigin();
        ...
        private static void Report(HttpContext ctx, AdmissionOutcome outcome)
        {
            ...
            if (ShouldAnnounceRefusal(remote))
            {
                Log.Information(
