...
namespace SQLTriage.Data.Services
{
    ...
    public static class ApiAuthorization
    {
            ...
            HttpContext context, RbacService rbac, string permission, bool failClosedWithoutApiKey)
        {
            ...
            //
            // WITHHELD ON A LAPSE (security-8, 2026-08-31). IsRbacEnforced() is false in three
            // postures: never-configured (Off), and the two PROBLEM states — Lapsed (switched on,
            // store damaged) and ConfigDidNotLoad (config itself damaged). The hatch is for the
            // FIRST only. A previously-enforced install whose store was later damaged must not
            // re-open the loopback hatch, exactly as the pages' gate no longer does
            // (RbacService.IsAuthorized). !IsProblem is true only for the never-configured Off
            // case, so this stays byte-identical for a fresh install and closes the downgrade for
            // a damaged one, which then resolves to the enforced result below (401/403).
            if (!failClosedWithoutApiKey
                && !rbac.IsRbacEnforced()
                && !rbac.DescribeEnforcementPosture().IsProblem
