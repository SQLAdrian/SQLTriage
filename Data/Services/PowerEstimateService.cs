...
namespace SQLTriage.Data.Services
{
    public class PowerEstimateService
    {
        ...
        // ── Tuning-headroom band (DD-honest — evidence-fired, never a flat percentage) ──
        // Each named contribution below is added to a server's RELATIVE tuning-headroom
        // low/high band ONLY when a specific, actually-executed corpus check verdict is a
        // real FAIL/WARN for that server. Does NOT touch the absolute watts/kWh/cost/CO2
        // model above (TuneCutLow/TuneCutHigh), which stays the cited Fan/Weber/Barroso
        // "if capacity is reclaimed" translation band.
        //
        // Verdict semantics — CORRECTED 2026-07-20 (ruling #4). The previous note here said
        // "Passed collapses PASS/SKIP/INFO -> true and ... only ceases to be true for a genuine
        // FAIL/WARN verdict", i.e. it asserted WARN sets Passed == false. That is no longer true:
        // ruling #4 moved WARN onto the not-a-failure tier, so WARN now rides Passed == true
        // alongside PASS/SKIP/INFO. Left unrevised, the comment would have licensed a future
        // reader to treat !r.Passed as "includes WARN" — it does not.
        //
        // The FILTER below is unchanged and is still correct, for a reason the old comment did
        // not state: a WARN means the check could not fully assess the target, so it establishes
        // no headroom evidence and must not fire a tuning contribution. Excluding it is the
        // honest behaviour, and it now happens via Passed == true rather than by the old
        // (mistaken) assumption. IsCorrupted (blocked, integrity mismatch) and a populated
        // ErrorMessage (execution exception) also set Passed == false but are NOT a verdict about
        // the server, so those stay explicitly excluded below.

        /// <summary>Power-plan-not-High-Performance check id (the OS-state corpus check).</summary>
        private const string PowerPlanCheckId = "SQLT-VA-POWER-PLAN-HIGH-PERFORMANCE";

        /// <summary>
        /// MAXDOP / parallelism-configuration checks. A real FAIL on ANY one of these fires
        /// the "parallelism" contribution once per server (not stacked per failing check) —
        /// they're evidence of the same underlying misconfiguration class.
        /// </summary>
        private static readonly HashSet<string> ParallelismCheckIds = new(StringComparer.OrdinalIgnoreCase)
        {
            "SQLT-BPCHK-00220-PARALLELISM-MAXDOP",                          // MAXDOP misconfigured
            "SQLT-CUSTOM-MAXDOP",                                           // MAXDOP not aligned to cores-per-NUMA-node
            "SQLT-CORE-TUNE-COST-THRESHOLD-FOR-PARALLELISM-DEFAULT-5-IS-O", // obsolete default cost threshold for parallelism
        };

        /// <summary>
        /// High-fragmentation / GUID-clustered-key index-health checks. A real FAIL on
        /// either fires the "index health" contribution once per server.
        /// </summary>
        private static readonly HashSet<string> IndexHealthCheckIds = new(StringComparer.OrdinalIgnoreCase)
        {
            "SQLT-CUSTOM-INDEX-FRAGMENTATION",                              // rowstore index fragmented above rebuild threshold
            "SQLT-CORE-DETECT-USE-OF-GUID-UNIQUEIDENTIFIER-IN-CLUSTERED-I", // GUID clustered key -> severe fragmentation
        };

        // Point bands per fired contribution.
        // Power-plan: Balanced's CPU frequency-scaling penalty on SQL Server workloads is
        // widely measured at ~10-15% throughput — Microsoft's own High-Performance
        // recommendation + Glenn Berry's published CPU-Z clock-throttling demonstrations —
        // this is a conservative energy-per-unit-work headroom band, not the full
        // throughput figure. Parallelism/index-health bands are heuristic and kept modest:
        // cross-socket scheduling / oversubscribed parallelism, and excess logical IO/CPU
        // per query from fragmented or GUID-keyed indexes, both burn cycles without
        // proportional work.
        private const int PowerPlanLowPts = 3, PowerPlanHighPts = 10;
        private const int ParallelismLowPts = 1, ParallelismHighPts = 4;
        private const int IndexHealthLowPts = 1, IndexHealthHighPts = 4;

        /// <summary>Ceiling for the ESTATE (cpu_count-weighted mean) tuning-headroom high band.</summary>
        public const int TuningHeadroomHighCap = 25;

        /// <summary>Stable driver key -> display label, shared with the estate driver-line builder.</summary>
        public static readonly IReadOnlyDictionary<string, string> TuningHeadroomLabels = new Dictionary<string, string>
        {
            ["power-plan"] = "Balanced power plan",
            ["parallelism"] = "parallelism findings",
            ["index-health"] = "index findings",
        };

        public PowerEstimateService(ILogger<PowerEstimateService> logger, ServerConnectionManager connections, CheckExecutionService checkExecutor)
        {
            _logger = logger;
        ...
        public async Task<ServerPowerEstimate?> GetEstimateAsync(string serverName, bool forceRefresh = false)
        {
            ...
            try
            {
                ...
                ApplyTuningHeadroom(est, serverName);
...
SELECT
    ...
    cpu_history_ok  = CASE WHEN @hostBusy IS NULL THEN 0 ELSE 1 END;";
        ...
        private ServerPowerEstimate Model(ServerPowerInputs i)
        {
            ...
            return new ServerPowerEstimate
            {
                ...
                // Evidence-based, not a flat constant — filled in by ApplyTuningHeadroom
                // right after Model() returns (see GetEstimateAsync). 0/0 here is the honest
                // "no evidence looked at yet" default, never a fabricated placeholder range.
                TuneCutLowPct = 0,
                TuneCutHighPct = 0,
        ...
        // Fills the DTO's tuning-headroom band from real corpus check verdicts for this
        // server. Best-effort: a lookup failure leaves the honest 0/0 "no signal" default
        // set by Model() rather than aborting the (already-valid) DMV-derived estimate.
        private void ApplyTuningHeadroom(ServerPowerEstimate est, string serverName)
        {
            try
            {
                var results = _checkExecutor.GetResults(serverName, maxCount: int.MaxValue);
                var headroom = ComputeTuningHeadroom(serverName, results);
                est.TuneCutLowPct = headroom.LowPct;
                est.TuneCutHighPct = headroom.HighPct;
                est.TuningHeadroomDrivers = headroom.Drivers;
                est.HeadroomState = headroom.State;
            }
            catch (Exception ex)
            {
                // The band stays 0/0, and the STATE now says why it is 0 rather than leaving the
                // reader to infer a measured zero from it (2026-08-08).
                est.HeadroomState = TuningHeadroomState.LookupFailed;
                _logger.LogDebug(ex, "Tuning-headroom evidence lookup failed for {Server}; leaving band at 0", serverName);
            }
        }

        /// <summary>
        /// Per-server tuning-headroom band, computed ENTIRELY from real corpus check
        /// verdicts for that server (see the class-level comment for the verdict-semantics
        /// basis). No qualifying evidence anywhere -> LowPct/HighPct are both 0 and Drivers
        /// is empty — the honest "no signal" state, never a fabricated range. Pure/static
        /// so it's independently unit-testable against synthetic CheckResult lists.
        ///
        /// <para><see cref="ServerTuningHeadroom.State"/> splits that zero in two, because the two
        /// halves are different facts and a reader is entitled to know which one they have: no check
        /// results for this server AT ALL (nothing was looked at) versus results present with nothing
        /// qualifying (something was looked at and found no headroom driver). The 0/0 band alone
        /// cannot tell them apart, and the status-bar chip printed "tuning headroom ~0–0%" for both —
        /// which reads as a measured zero either way.</para>
        /// </summary>
        public static ServerTuningHeadroom ComputeTuningHeadroom(string serverName, IReadOnlyList<CheckResult>? results)
        {
            var headroom = new ServerTuningHeadroom { ServerName = serverName };
            if (results is null || results.Count == 0) return headroom;   // State stays NoCheckResults

            // Results exist, so something WAS examined. Downgraded to EvidenceFired below if any
            // contribution actually fires.
            headroom.State = TuningHeadroomState.NoQualifyingFindings;

            // A real FAIL/WARN verdict: not-passed, not blocked (integrity), not an
            // execution exception (ErrorMessage populated) — an actual authored verdict.
            bool RealFail(string checkId) => results.Any(r =>
                string.Equals(r.CheckId, checkId, StringComparison.OrdinalIgnoreCase)
                && !r.Passed && !r.IsCorrupted && string.IsNullOrEmpty(r.ErrorMessage));

            if (RealFail(PowerPlanCheckId))
            {
                headroom.LowPct += PowerPlanLowPts;
                headroom.HighPct += PowerPlanHighPts;
                headroom.Drivers.Add("power-plan");
            }
            if (ParallelismCheckIds.Any(RealFail))
            {
                headroom.LowPct += ParallelismLowPts;
                headroom.HighPct += ParallelismHighPts;
                headroom.Drivers.Add("parallelism");
            }
            if (IndexHealthCheckIds.Any(RealFail))
            {
                headroom.LowPct += IndexHealthLowPts;
                headroom.HighPct += IndexHealthHighPts;
                headroom.Drivers.Add("index-health");
            }

            // Keyed off the drivers, not off the percentages: the drivers ARE the evidence, and a
            // future contribution worth 0 points must not read as "evidence fired".
            if (headroom.Drivers.Count > 0) headroom.State = TuningHeadroomState.EvidenceFired;
    ...
    public class ServerPowerEstimate
    {
        ...
        public double HostBusyPct { get; set; }
        public double SqlCpuPct { get; set; }
        /// <summary>From sys.dm_os_sys_info (ProbeAsync) — the estate rollup's weighting basis.</summary>
        public int CpuCount { get; set; }
        public int TuneCutLowPct { get; set; }
        public int TuneCutHighPct { get; set; }
        /// <summary>Which named contributions fired for this server (e.g. "power-plan", "parallelism", "index-health") — see PowerEstimateService.TuningHeadroomLabels for display text. Empty when TuneCutLowPct/HighPct are both 0.</summary>
        public List<string> TuningHeadroomDrivers { get; set; } = new();

        /// <summary>
        /// WHY the band is what it is. Defaults to <see cref="TuningHeadroomState.NoCheckResults"/>,
        /// which is the state a freshly modelled estimate is in before
        /// <c>ApplyTuningHeadroom</c> has looked at anything — so an estimate that never reached the
        /// evidence pass claims nothing. Read this before printing the band: 0–0% is not a
        /// measurement of zero headroom unless the state says findings were examined.
        /// </summary>
        public TuningHeadroomState HeadroomState { get; set; } = TuningHeadroomState.NoCheckResults;
        ...
        /// <summary>
        /// The honest, relative-first one-liner for the status-bar chip's tooltip. Conditioned on
        /// <see cref="HeadroomState"/> rather than on the band being 0, so the two kinds of zero are
        /// not described with one sentence: "no signal detected" implies detection was attempted, and
        /// with no check results for the server nothing was.
        /// </summary>
        public string RelativeHeadline => HeadroomState switch
        {
            TuningHeadroomState.EvidenceFired =>
                $"Tuning could cut this server's compute energy ~{TuneCutLowPct}–{TuneCutHighPct}% (modelled)",
            TuningHeadroomState.NoQualifyingFindings =>
                "No tuning-headroom finding for this server in the checks that have run (modelled)",
            TuningHeadroomState.LookupFailed =>
                "Tuning headroom not established — reading this server's check results failed",
            _ =>
                "Tuning headroom not assessed — no check results for this server yet",
        };

        /// <summary>
        /// The status-bar chip's relative-branch label. Says which of the four states it is in, in
        /// four words or so. Before 2026-08-08 it was interpolated straight from the band, so a
        /// server with no check results carried "tuning headroom ~0–0% (this server)" on every page —
        /// arithmetically the model's output, and read by anyone as a measured zero.
        /// </summary>
        public string RelativeChipLabel => HeadroomState switch
        {
            TuningHeadroomState.EvidenceFired =>
                $"tuning headroom ~{TuneCutLowPct}–{TuneCutHighPct}% (this server)",
            TuningHeadroomState.NoQualifyingFindings =>
                "tuning headroom: no findings (this server)",
            TuningHeadroomState.LookupFailed =>
                "tuning headroom: unavailable (this server)",
            _ =>
                "tuning headroom: not assessed (this server)",
        };
    ...
    /// <summary>
    /// Why a tuning-headroom band holds the numbers it holds. The default (0) must be the state that
    /// claims LEAST, so an object nobody filled in asserts nothing.
    /// </summary>
    public enum TuningHeadroomState
    {
        /// <summary>No check results for this server, so no evidence was examined. Not a zero result.</summary>
        NoCheckResults,
        /// <summary>Check results exist and none of them fires a headroom contribution.</summary>
        NoQualifyingFindings,
        /// <summary>Reading the server's check results faulted, so nothing was examined.</summary>
        LookupFailed,
        /// <summary>At least one named contribution fired; LowPct/HighPct are a real band.</summary>
        EvidenceFired,
    }

    /// <summary>
    /// A single server's evidence-fired tuning-headroom band (see
    /// PowerEstimateService.ComputeTuningHeadroom). LowPct/HighPct are both 0 and Drivers
    /// is empty when no qualifying corpus check FAILed/WARNed for this server — and
    /// <see cref="State"/> says whether that is because nothing qualified or because nothing was
    /// looked at.
    /// </summary>
    public class ServerTuningHeadroom
    {
        public string ServerName { get; set; } = "";
        public int LowPct { get; set; }
        public int HighPct { get; set; }
        public List<string> Drivers { get; set; } = new();
        public TuningHeadroomState State { get; set; } = TuningHeadroomState.NoCheckResults;
    }
}
