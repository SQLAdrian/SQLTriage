...
namespace SQLTriage.Data.Services
{
    ...
    public class GovernanceHistoryService : IDisposable
    {
            ...
            try
            {
                ...
                cmd.CommandText = @"
                    ...
                    CREATE TABLE IF NOT EXISTS check_results (
                        ...
                        message         TEXT,
                        duration_ms     INTEGER,
                        -- Ruling #4 (2026-07-20). The raw verdict tier, because `passed` alone
                        -- CANNOT represent one: PASS/SKIP/INFO/WARN all persist as passed=1, so a
                        -- WARN rehydrated after a restart came back indistinguishable from a
                        -- genuine PASS — Verdict null, IsWarn false, IsScorable TRUE — and every
                        -- surface reading rehydrated history (the audit grid, the per-check trend
                        -- pass-rate) rendered a check that could not see the server as a green
                        -- pass. The distinction had to survive the round-trip, not just the run.
                        -- NULL for rows written before this column existed: forward-only, no
                        -- backfill is possible (the information was never recorded).
                        verdict         TEXT
                    );
                    ...
                    CREATE TABLE IF NOT EXISTS baseline_check_results (
                        ...
                        verdict         TEXT,                 -- B1: raw verdict tier frozen with the
                                                              -- baseline; NULL on pre-B1 baselines.
                        PRIMARY KEY (baseline_id, check_id),
                        FOREIGN KEY (baseline_id) REFERENCES baselines(id) ON DELETE CASCADE
                    );
                ";
                cmd.ExecuteNonQuery();
                MigrateCheckResultVerdictColumn(conn);
                MigrateBaselineCheckVerdictColumn(conn);
        ...
        /// <summary>
        /// Ruling #4 (2026-07-20): add check_results.verdict to databases created before the column
        /// existed. CREATE TABLE IF NOT EXISTS above is a no-op on an existing table, so without
        /// this every upgraded install keeps writing WARN as an indistinguishable passed=1 row.
        /// Idempotent (duplicate-column is swallowed) and best-effort: a failed migration must
        /// leave history readable, not take the app down. Follows AlertHistoryService's pattern.
        /// </summary>
        private void MigrateCheckResultVerdictColumn(SqliteConnection conn)
        {
            try
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "ALTER TABLE check_results ADD COLUMN verdict TEXT";
                cmd.ExecuteNonQuery();
            }
            ...
            {
                // Already migrated — expected on every run after the first.
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "check_results.verdict migration failed; WARN will not survive a restart");
            }
        }

        /// <summary>
        /// B1 (2026-07-20): add baseline_check_results.verdict to databases whose baselines were
        /// frozen before the column existed. Without the frozen verdict the baseline side is a
        /// bare pass/fail bit, and a WARN frozen INTO a baseline writes passed=1 — so it reads
        /// "was passing" and a later genuine FAIL renders as a pass→fail REGRESSION that never
        /// happened (we never observed it passing).
        ///
        /// Rows left NULL by this migration are handled as UNKNOWN, not back-filled: see
        /// <see cref="BaselineCheckState.HasAssertion"/>.
        /// </summary>
        private void MigrateBaselineCheckVerdictColumn(SqliteConnection conn)
        {
            try
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "ALTER TABLE baseline_check_results ADD COLUMN verdict TEXT";
                cmd.ExecuteNonQuery();
            }
            catch (SqliteException ex) when (
                ex.Message.Contains("duplicate column", StringComparison.OrdinalIgnoreCase) ||
            ...
            {
                _logger.LogWarning(ex, "baseline_check_results.verdict migration failed; baseline transitions fall back to the passed-only basis");
            }
        }
        ...
        /// </summary>
        /// <remarks>
        /// Now unused by CioDashboard.razor (2026-07-13): the Governance Trend bucketing fix
        /// moved weekly grouping to <see cref="SQLTriage.Data.TrendBucketer"/>, which buckets
        /// the same raw <see cref="GetTrend"/> rows the daily view already used — this avoided
        /// the AVG-only week label ("2026-W28") that failed downstream DateTime parsing. Left
        /// in place (not deleted) per the minimal-surface-change call on this service.
        /// </remarks>
                ...
                lock (_writeLock)
                {
                    ...
                    cmd.CommandText = @"
                        INSERT INTO check_results
                            (server_name, check_id, check_name, category, severity, passed, actual_value, expected_value, message, duration_ms, verdict)
                        VALUES
                            (@server, @cid, @cname, @cat, @sev, @passed, @actual, @expected, @msg, @dur, @verdict);
                    ";
                    cmd.Parameters.AddWithValue("@server", serverName);
                    ...
                    // Ruling #4: `passed` cannot encode the tier — PASS/SKIP/INFO/WARN all write 1.
                    cmd.Parameters.AddWithValue("@verdict", (object?)result.Verdict ?? DBNull.Value);
                    cmd.ExecuteNonQuery();
                }
            }
            catch { /* best-effort */ }
        ...
        public List<CheckResult> LoadLatestCheckResults(string serverName, int daysBack = 7)
        {
            ...
            try
            {
                ...
                cmd.CommandText = @"
                    ...
                    SELECT check_id, check_name, category, severity, passed, actual_value, expected_value, message, verdict
                ...
                while (reader.Read())
                {
                    results.Add(new CheckResult
                    {
                        ...
                        // Ruling #4: without this the rehydrated result is Passed=true/Verdict=null
                        // — a WARN indistinguishable from a PASS on every surface downstream.
                        Verdict = reader.IsDBNull(8) ? null : reader.GetString(8)
                    });
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// R1 (2026-07-07): last KNOWN result per check for a server — the newest row per
        /// check_id within <paramref name="daysBack"/>. Used to rehydrate the audit page
        /// after an app restart. Unlike <see cref="LoadLatestCheckResults"/> (whose join
        /// requires rows to share the exact same second), this tolerates runs whose rows
        /// span many seconds. <paramref name="newestRecordedAt"/> = most recent row's
        /// timestamp (UTC), or null when nothing was found.
        /// </summary>
        public List<CheckResult> LoadLastKnownResults(string serverName, out DateTime? newestRecordedAt, int daysBack = 7)
        {
            var results = new List<CheckResult>();
            ...
            try
            {
                using var conn = SqliteCipherHelper.OpenEncrypted(_connectionString);
                using var cmd = conn.CreateCommand();
                // SQLite bare-column semantics: with MAX(recorded_at) in the select list,
                // the non-aggregated columns come from the row carrying that max.
                cmd.CommandText = @"
                    SELECT check_id, check_name, category, severity, passed, actual_value, expected_value, message, MAX(recorded_at), verdict
                    FROM check_results
                    WHERE server_name = @server AND recorded_at >= datetime('now', @days)
                    GROUP BY check_id
                    ORDER BY check_id;
                ";
                cmd.Parameters.AddWithValue("@server", serverName);
                ...
                while (reader.Read())
                {
                    results.Add(new CheckResult
                    {
                        ...
                        ExpectedValue = reader.IsDBNull(6) ? 0 : reader.GetInt32(6),
                        Message = reader.IsDBNull(7) ? "" : reader.GetString(7),
                        // Ruling #4 — index 9: MAX(recorded_at) occupies 8. This is the path the
                        // audit grid rehydrates from after a restart, so a dropped verdict here is
                        // exactly how a WARN turned into a green "✓ pass" the next morning.
                        Verdict = reader.IsDBNull(9) ? null : reader.GetString(9)
                    });
                    if (!reader.IsDBNull(8) &&
                        DateTime.TryParse(reader.GetString(8),
                            System.Globalization.CultureInfo.InvariantCulture,
                            System.Globalization.DateTimeStyles.AssumeUniversal |
                            System.Globalization.DateTimeStyles.AdjustToUniversal, out var ts) &&
                        (newestRecordedAt == null || ts > newestRecordedAt))
                    {
                        newestRecordedAt = ts;
                    }
                }
            ...
            {
                _logger.LogWarning(ex, "Failed to load last-known check results for {Server}", serverName);
            }
            return results;
        }
        ...
        public List<CheckHistoryPoint> GetCheckHistory(string checkId, int days = 90)
        {
            ...
            try
            {
                ...
                using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
                    SELECT server_name, recorded_at, passed, actual_value, severity, check_name, category, verdict
                    FROM check_results
                    WHERE check_id = @cid
                      AND recorded_at >= datetime('now', @days)
                    ORDER BY recorded_at ASC;
                ...
                while (reader.Read())
                {
                    points.Add(new CheckHistoryPoint
                    {
                        ...
                        Verdict = reader.IsDBNull(7) ? null : reader.GetString(7),
                    });
                }
            }
            catch (Exception ex)
        ...
        public List<IntegrityViolation> VerifyIntegrityChain()
        {
            ...
            try
            {
                ...
                // Counted, not assumed: the "verified" log line names this number, and a chain
                // walked over zero records must not read the same as one walked over thousands.
                var recordsVerified = 0;
                using var reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    recordsVerified++;
                ...
                // 2026-08-05: the "verified" line bound the {Count} placeholder to the STRING
                // "intact"/"broken" instead of a count, so it rendered "verified - intact
                // records, no violations" and the record count never appeared. A chain verified
                // over zero records logged exactly the same sentence as one verified over
                // thousands, which is the worst version of this defect: reassurance with no
                // measurement behind it. Both branches now name what was actually walked.
                if (IntegrityBroken)
                    _logger.LogWarning(
                        "Governance integrity chain: {Violations} violation(s) across {Records} record(s)",
                        violations.Count, recordsVerified);
                else if (recordsVerified == 0)
                    _logger.LogInformation(
                        "Governance integrity chain holds no records yet, so nothing was verified");
                else
                    _logger.LogInformation(
                        "Governance integrity chain verified intact across {Records} record(s)",
                        recordsVerified);
            }
            catch (Exception ex)
            {
        ...
        /// EXCLUSIVE upper bound, or null for none (the original behaviour, which every caller
        /// wanting "the latest persisted score" wants).
        ///
        /// <para>A caller computing a day-over-day TREND must pass today's date here. Snapshots are
        /// keyed by date and <see cref="RecordHealthScore"/> writes today's on the first assessment
        /// of each UTC day, so an unbounded "on or after yesterday" query returns TODAY'S OWN ROW
        /// from that moment onward — and the trend then compared a score with itself and reported
        /// "Stable" forever (found 2026-08-08 while rendering the SR-11 fix; the fix to the chip
        /// would otherwise have printed a self-comparison for the whole day after the first load).</para>
        /// </param>
        public async Task<int?> GetLatestHealthScoreAsync(
            string serverName, DateTime fromDate, DateTime? beforeDate = null)
        {
            try
            {
                ...
                using var conn = SqliteCipherHelper.OpenEncrypted(_connectionString);
                using var cmd = conn.CreateCommand();
                cmd.CommandText = beforeDate == null
                    ? @"
                    ...
                    WHERE server_name   = @srv
                      AND recorded_date >= @date
                      AND recorded_date <  @before
                    ORDER BY recorded_date DESC
                    LIMIT 1;";
                cmd.Parameters.AddWithValue("@srv", serverName);
                cmd.Parameters.AddWithValue("@date", dateKey);
                if (beforeDate != null)
                    cmd.Parameters.AddWithValue("@before", beforeDate.Value.ToString("yyyy-MM-dd"));
            ...
            IReadOnlyList<CheckResult> results, string? reason = null)
        {
            ...
            // 2026-07-16: moved to SCORED basis (CheckClassification.IsScorable + CountsAsPass)
            // instead of raw r.Passed — previously this counted a SKIP-via-permission-error
            // (Passed=true) as "passed" and a genuine execution error (Passed=false) as "failed",
            // inconsistent with the main governance trend table above (RecordGovernanceScore,
            // :241-243) which persists score.PassedFindings/score.FailedFindings — the SCORED
            // subset. SEMANTIC CHANGE, forward-only: baselines recorded before this change carry
            // the OLD raw-basis counts; there is no silent backfill — a re-baseline is required to
            // get a scored-basis snapshot (precedent: the 2026-07-12 build-counter re-base was
            // documented, not silently rewritten either). total_checks below stays the RAW total
            // (unchanged) — same TotalFindings-vs-PassedFindings+FailedFindings gap governance
            // already has for SKIP/INFO results.
            ...
            var failed = scorable.Count(r => !CheckClassification.CountsAsPass(r));
                    ...
                    using (var ins = conn.CreateCommand())
                    {
                        ...
                        ins.CommandText = @"
                            INSERT OR REPLACE INTO baseline_check_results
                                (baseline_id, check_id, check_name, category, severity, passed, effort_hours, verdict)
                            VALUES
                                (@bid, @cid, @cname, @cat, @sev, @passed, @effort, @verdict);";
                        ...
                        // B1: freeze the classification tier alongside the pass bit. A baseline row
                        // whose result was WARN/SKIP/INFO carried no assertion about the control,
                        // and only this column can say so after the fact.
                        var pVerdict = ins.Parameters.Add("@verdict", SqliteType.Text);
                        ...
                        foreach (var r in results)
                        {
                            ...
                            pVerdict.Value = (object?)BaselineVerdictFor(r) ?? DBNull.Value;
                            ins.ExecuteNonQuery();
                        }
                    }
            ...
            string serverName, IReadOnlyList<CheckResult> current, int currentCompositeScore)
        {
            ...
            var result = new CheckTransitionResult
            {
                ...
                // A3 (2026-07-20): counted BEFORE any transition is classified, over the whole
                // frozen baseline rather than only the rows that happened to transition — the
                // caveat is a property of the baseline, and it must appear even on a run where
                // nothing moved.
                BaselineCheckCount     = baselineMap.Count,
                UntieredBaselineChecks = baselineMap.Count(kv => !kv.Value.TierRecorded),
            };

            // B1 (2026-07-20) — classification is VERDICT-AWARE on both sides.
            //
            // The rule, once: a transition can only be asserted between two ASSERTIONS. WARN, SKIP
            // and INFO all ride Passed=true (ruling #4 for WARN; CheckExecutionService for the
            // other two) but none of them asserts anything about the control. Comparing the raw
            // Passed bit therefore read "fail → could-not-assess" as a green Fail→Pass RESOLVED —
            // selling a check that LOST VISIBILITY as a check that got FIXED, and summing its
            // effort hours into the client-facing "remediation delivered" figure.
            foreach (var (checkId, cur) in currentMap)
            {
                var curAsserts = CheckClassification.IsScorable(cur);

                if (baselineMap.TryGetValue(checkId, out var wasAt))
                {
                    if (!curAsserts)
                    {
                        // Lost the ability to assess. NOT resolved (it was never shown correct) and
                        // NOT regressed (the config did not get worse — we stopped being able to
                        // tell). Only a transition if the baseline actually held an assertion;
                        // unassessable → unassessable is steady state, not news.
                        if (wasAt.HasAssertion)
                            result.Unassessable.Add(ToTransition(checkId, cur, wasAt.Passed));
                    }
                    else if (!wasAt.HasAssertion)
                    {
                        // Gained visibility over a check the baseline could not assess. A failure
                        // found here is newly ESTABLISHED, not a regression from a state we never
                        // observed; a pass here cannot be claimed as "resolved" for the same reason.
                        if (!cur.Passed)
                            result.NewlyFailing.Add(ToTransition(checkId, cur));
                    }
                    else if (!wasAt.Passed && cur.Passed)
                    {
                        // A3: carries whether the BASELINE side of this comparison recorded a tier.
                        // On an untiered row the "fail at baseline" is the raw passed bit and
                        // nothing more — see CheckTransition.BaselineTierUnknown.
                        result.Resolved.Add(ToBaselineTransition(checkId, cur, wasAt));   // Fail → Pass
                    }
                    else if (wasAt.Passed && !cur.Passed)
                    {
                        result.Regressed.Add(ToBaselineTransition(checkId, cur, wasAt));  // Pass → Fail
                    }
                }
                else if (curAsserts && !cur.Passed)
                {
            ...
            // can't assert resolved (might just not have run), so report separately. Keyed on a
            // baseline row that actually ASSERTED a failure — a baseline WARN/SKIP/INFO never
            // asserted one, so it can't be a "failing at baseline" we claim has disappeared.
            foreach (var (checkId, wasAt) in baselineMap)
            {
                if (wasAt.HasAssertion && !wasAt.Passed && !currentMap.ContainsKey(checkId))
        ...
        /// <summary>B1: an Unassessable transition additionally carries which side of the line the
        /// baseline held it on, so the page can distinguish "was failing at baseline and we can no
        /// longer confirm it was fixed" (urgent) from "was passing and we can no longer confirm it
        /// still is" (a watch item) — instead of a bare shrug.</summary>
        private static CheckTransition ToTransition(string checkId, CheckResult r, bool baselineWasPassing)
        {
            var t = ToTransition(checkId, r);
            t.BaselineWasPassing = baselineWasPassing;
            return t;
        }

        /// <summary>A3 (2026-07-20): a Resolved/Regressed transition, tagged with whether the
        /// baseline row it was compared against actually recorded a verdict tier. See
        /// <see cref="CheckTransition.BaselineTierUnknown"/> for why that matters.</summary>
        private static CheckTransition ToBaselineTransition(
            string checkId, CheckResult r, BaselineCheckState wasAt)
        {
            var t = ToTransition(checkId, r);
            t.BaselineTierUnknown = !wasAt.TierRecorded;
            return t;
        }

        /// <summary>
        /// B1: the frozen baseline state of one check — the pass bit AND the tier that produced it.
        /// </summary>
        /// <param name="Verdict">NULL on baselines frozen before the column existed.</param>
        private readonly record struct BaselineCheckState(bool Passed, string? Verdict)
        {
            /// <summary>
            /// True when this baseline row made a real PASS/FAIL assertion about the control.
            ///
            /// A NULL verdict counts as an assertion ON PURPOSE. Those rows predate the column, and
            /// the honest reading of "we never recorded the tier" is UNKNOWN — so they stay on the
            /// pre-existing passed-based basis rather than being retro-labelled either way. That
            /// keeps every historical baseline reading exactly as it did before B1 (no silent
            /// rewriting of a client's recorded history) and confines the new discipline to
            /// baselines frozen with the tier actually recorded. It is the same rule
            /// <see cref="CheckClassification.IsWarnVerdict"/> already states for
            /// <c>check_results.verdict</c>: null is not a WARN.
            /// </summary>
            /// <summary>
            /// A3 (2026-07-20): whether this baseline row records WHICH tier produced its pass
            /// bit. False for every row frozen before the verdict column existed.
            ///
            /// <para><see cref="HasAssertion"/> deliberately treats those rows as assertions so
            /// history is not retro-labelled — but that choice has a cost, and this property is
            /// what lets the UI state the cost instead of hiding it. On an untiered row a frozen
            /// WARN wrote passed=1 and is indistinguishable from a frozen PASS, so a later genuine
            /// FAIL is filed as "Regressed (pass → fail)" — a regression from a state nobody ever
            /// observed.</para>
            /// </summary>
            public bool TierRecorded => Verdict != null;

            public bool HasAssertion =>
                Verdict == null ||
                (!CheckClassification.IsWarnVerdict(Verdict) &&
                 !string.Equals(Verdict, "SKIP", StringComparison.OrdinalIgnoreCase) &&
                 !string.Equals(Verdict, "INFO", StringComparison.OrdinalIgnoreCase));
        }

        /// <summary>
        /// B1: the tier to freeze into a baseline row. A non-scorable result records the tier that
        /// made it non-scorable (WARN/SKIP/INFO) — that is the whole point of the column. A
        /// scorable result records the plain assertion it made. Derived through
        /// <see cref="CheckClassification"/> rather than off <c>r.Verdict</c> directly, so a check
        /// that is INFO by declared Severity but carries no Verdict string still freezes as INFO.
        /// </summary>
        private static string BaselineVerdictFor(CheckResult r) =>
            CheckClassification.IsWarn(r) ? "WARN" :
            CheckClassification.IsSkip(r) ? "SKIP" :
            CheckClassification.IsInfo(r) ? "INFO" :
            r.Passed ? "PASS" : "FAIL";

        private Dictionary<string, BaselineCheckState> LoadBaselineChecks(long baselineId)
        {
            var map = new Dictionary<string, BaselineCheckState>(StringComparer.OrdinalIgnoreCase);
            try
            {
                using var conn = SqliteCipherHelper.OpenEncrypted(_connectionString);
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "SELECT check_id, passed, verdict FROM baseline_check_results WHERE baseline_id = @bid;";
                cmd.Parameters.AddWithValue("@bid", baselineId);
                using var rdr = cmd.ExecuteReader();
                while (rdr.Read())
                {
                    map[rdr.GetString(0)] = new BaselineCheckState(
                        Passed: rdr.GetInt32(1) == 1,
                        Verdict: rdr.IsDBNull(2) ? null : rdr.GetString(2));
                }
            }
            catch (Exception ex)
            {
    ...
    public class CheckHistoryPoint
    {
        ...
        /// <summary>"Not a failure" — PASS, SKIP, INFO and WARN all set this true. Read it with
        /// <see cref="Verdict"/> before treating a point as a pass (ruling #4, 2026-07-20).</summary>
        public bool Passed { get; set; }

        /// <summary>Ruling #4 (2026-07-20): the raw verdict tier for this point. Null for rows
        /// recorded before check_results.verdict existed — a trend must therefore treat null as
        /// "unknown tier", never as a confirmed PASS.</summary>
    ...
    public class CheckTransition
    {
        ...
        /// <summary>B1: the current raw verdict tier, so an Unassessable row can say WHY it became
        /// unassessable (WARN = could not see; SKIP = no longer applicable; INFO = informational).
        /// Null for a plain scorable pass/fail.</summary>
        public string? Verdict { get; set; }

        /// <summary>B1: for <see cref="CheckTransitionResult.Unassessable"/> only — whether the
        /// baseline had this check PASSING. False means it was failing at baseline and the fix can
        /// no longer be confirmed, which is the more urgent of the two.</summary>
        public bool BaselineWasPassing { get; set; }

        /// <summary>
        /// A3 (2026-07-20): for <see cref="CheckTransitionResult.Resolved"/> and
        /// <see cref="CheckTransitionResult.Regressed"/> — the baseline row this was compared
        /// against carries NO verdict tier, because it was frozen before the column existed.
        ///
        /// <para>The comparison is therefore only as good as the raw <c>passed</c> bit, and that
        /// bit cannot tell a PASS from a WARN/SKIP/INFO: all four wrote 1. So a check that was
        /// WARN at baseline and genuinely FAILS now lands here as "pass → fail", alleging a
        /// regression from a state that was never asserted. This flag does not correct the
        /// classification — correcting it would mean rewriting a client's recorded history, which
        /// the ruling forbids. It marks the row so the caveat appears where the number is READ.</para>
        /// </summary>
        public bool BaselineTierUnknown { get; set; }
    }
    ...
    public class CheckTransitionResult
    {
        ...
        /// <summary>
        /// B1 (2026-07-20): checks the baseline could assess and the current run CANNOT — the
        /// result went to WARN (could not see), SKIP (no longer applicable) or INFO. Neither
        /// Resolved nor Regressed: no assertion was made, so no fix may be claimed and no
        /// regression may be alleged. Kept out of <see cref="Resolved"/> above all, because that
        /// list is summed into the client-facing remediation-hours figure.
        /// </summary>
        public List<CheckTransition> Unassessable { get; set; } = new();

        /// <summary>A3: how many rows the compared baseline holds in total.</summary>
        public int BaselineCheckCount { get; set; }

        /// <summary>
        /// A3 (2026-07-20): how many of those rows were frozen with NO verdict tier — i.e. before
        /// <c>baseline_check_results.verdict</c> existed. Every transition computed against such a
        /// row rests on the raw <c>passed</c> bit alone, which cannot distinguish a PASS from a
        /// WARN/SKIP/INFO. Non-zero means <see cref="Regressed"/> may contain regressions from
        /// states that were never asserted, and <see cref="Resolved"/> may contain fixes to
        /// failures that were never asserted either.
        /// </summary>
        public int UntieredBaselineChecks { get; set; }

        /// <summary>A3: true when ANY compared baseline row predates the verdict column. This is
        /// the condition the UI must caveat — see Pages/BaselineProgress.razor.</summary>
        public bool BaselineIsPreMigration => UntieredBaselineChecks > 0;
