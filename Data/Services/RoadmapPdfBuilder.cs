    ...
    private static void Attestation(IContainer c, RoadmapReport r) =>
        c.Column(col =>
        {
            ...
            col.Item().Text(
                ...
                // reports-r2-07, 2026-08-27. The last sentence read "The SHA-256 hash (recorded
                // post-export in the sibling .manifest.json) lets a recipient verify the PDF they
                // hold is byte-for-byte identical to the one this operator generated." That is a
                // flat assertion that the file exists. Writing it is best-effort by construction:
                // the hash is taken from the PDF, so the PDF is on disk first, and the write can
                // fail after this page has already been rendered and shipped. A recipient holding a
                // PDF with no manifest beside it was reading a promise about a file nobody wrote.
                // The claim is now conditional on what the recipient can actually see.
                ...
                "tool version that produced it. SQLTriage attempts to write a sibling .manifest.json after export, " +
                "carrying this PDF's SHA-256. If that file is beside the PDF you hold, its hash lets you verify the " +
                "bytes are identical to the ones this operator generated. If it is not there, the manifest write " +
                "failed and no such verification is possible from this report alone.")
