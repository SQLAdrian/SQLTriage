...
namespace SQLTriage.Data.Services
{
    ...
    {
        /// <summary>
        /// The Message-prefix SKIP convention on its own ("SKIP — not applicable on EngineEdition
        /// 5"), separate from the other two signals so a consumer that must tell an honest skip from
        /// an execution fault can key on this one alone — <c>DailySummaryBuilder.IsError</c> is the
        /// caller that needs exactly that, because the host-probe skip path carries a diagnostic
        /// ErrorMessage beside its SKIP prefix (CheckExecutionService).
        ///
        /// <para>LEADING WHITESPACE IS TRIMMED (2026-08-27, reports lane fix round). The four copies
        /// of this ladder were consolidated onto <see cref="IsSkip"/> in this lane, and the copy that
        /// <c>ScheduledReportService</c> deleted did <c>Message.TrimStart()</c> first while this one
        /// did not — so a message written "  SKIP — ..." stopped being a skip and fell through to
        /// <c>r.Passed</c> (true for every skip), exporting as a green PASS in an unattended
        /// client-facing PDF. Measured on the current corpus: zero check sources emit a
        /// whitespace-led SKIP, so this closes a latent arm rather than changing a count today.
        /// <see cref="string.TrimStart()"/> returns the same instance when there is nothing to trim,
        /// so the hot path allocates nothing.</para>
        /// </summary>
        public static bool IsMessagePrefixSkip(CheckResult r) =>
        ...
        /// <summary>
        /// The status badge a result renders as — THE ONE DEFINITION, called by
        /// <c>Pages/Checks.razor</c>. Pinned by <c>CheckBadgeClassificationTests</c>, which also
        /// asserts that the component still delegates here rather than carrying its own copy.
        ///
        /// <para><b>⚠ THE ORDER OF THESE ARMS IS THE WHOLE POINT, and getting it wrong shipped a
        /// defect twice.</b> Three tiers ride <c>Passed == true</c> while asserting NOTHING about the
        /// server — WARN, SKIP and INFO — so every one of them must be answered BEFORE the
        /// <c>r.Passed</c> arm is reached. Until 2026-09-15 this expression lived inline in the
        /// component and answered only WARN; a SKIP result rendered a green "✓ pass" to a client.
        /// PROVED by this class's own test before the fix: <c>BadgeLabel</c> returned "✓ pass" for
        /// <c>Verdict="SKIP", Passed=true</c>, and for INFO by verdict, INFO by severity, and a result
        /// carrying an ErrorMessage.</para>
        ///
        /// <para><b>The history matters because it explains the shape of the bug.</b> The 2026-07-16
        /// gate-B1 fix repaired <see cref="IsSkip"/> after 53 of 55 Verdict-SKIP corpus results — 4
        /// Critical, 24 High among them — rendered green PASS badges. It never reached this expression,
        /// because this expression does not call <see cref="IsSkip"/>. Then ruling #4 (2026-07-20)
        /// added the WARN arm, and its comment and its raw-Passed allowlist rows both justified
        /// themselves purely in terms of WARN. Each fix was correct about the case in front of it and
        /// silent about the tiers beside it. <b>If you add a fourth non-scorable tier to this class,
        /// this method is one of the places that will not notice.</b> Prefer
        /// <see cref="IsScorable"/> here if you can express the render that way.</para>
        /// </summary>
        public static string BadgeLabel(CheckResult r) =>
            r.IsCorrupted ? "corrupted"
            : IsWarn(r) ? "⚠ partial"
            : IsSkip(r) ? "skipped"
            : IsInfo(r) ? "info"
            : r.Passed ? "✓ pass"
        ...
        /// <summary>
        /// The colour for <see cref="BadgeLabel"/>, in the same order and from the same definition, so
        /// the words and the colour cannot disagree — a green cell with honest words is still read as a
        /// pass. SKIP and INFO are muted rather than coloured: they are not a verdict in either
        /// direction, and giving them red would be as wrong as giving them green.
        /// </summary>
        public static string BadgeColorVar(CheckResult r) =>
            r.IsCorrupted ? "var(--purple)"
            : IsWarn(r) ? "var(--orange)"
            : IsSkip(r) || IsInfo(r) ? "var(--text-muted)"
            : r.Passed ? "var(--green)"
            : "var(--red)";
