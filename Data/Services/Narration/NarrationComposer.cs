...
namespace SQLTriage.Data.Services.Narration
{
    ...
    public static class NarrationComposer
    {
        ...
        {
            // Gate fix C2 (2026-08-05). The Unknown bucket is keyed to the score this context was
            // built from. When the CALLER has a measured Executive Health mean on the same screen,
            // "nothing has been assessed yet" contradicts the card beside it, so the sentence is
            // conditioned on the same rollup the caption reads rather than on this bucket alone.
            if (ctx.Bucket == ScoreBucket.Unknown || ctx.Evaluated == 0)
            {
                var unknownOpts = ctx.EstateAssessedCount > 0
                    ? NarrationLibrary.HeadlineUnknownBesideAMeasuredMean
                    : NarrationLibrary.Headline[ScoreBucket.Unknown];
                return Fill(Pick(unknownOpts, Seed(ctx)), ctx);
            }
        ...
        private static string Fill(string template, NarrationContext ctx)
        {
            var s = template
                ...
                .Replace("{estateAssessed}", Count(ctx.EstateAssessedCount,
                    ctx.EstateAssessedCount == 1 ? "assessed server" : "assessed servers"))
            ...
            if (ctx.Weakest is { } w)
                s = s.Replace("{dim}", w.Label)
                     .Replace("{score}", w.Score >= 0 ? ((int)System.Math.Round(w.Score)).ToString() : "—")
                     .Replace("{passed}", w.Passed.ToString())
                     .Replace("{total}", w.Total.ToString());
