...
public sealed class WelcomeTourService : IDisposable
{
    ...
    private void RestartAutoAdvance()
    {
        ...
        _ = AdvanceAfterDelayAsync(_autoAdvanceCts.Token);
    }

    // Fire-and-forget, but deliberately NOT wrapped in Task.Run: every caller
    // (Start, ToggleAutoAdvance, GoToStop) runs on Blazor's dispatcher, so the
    // await below captures that synchronization context and Next() -> NavigateTo
    // resumes on the UI thread. A Task.Run wrapper punted the continuation onto
    // the thread pool, where NavigateTo can throw or silently stall the tour.
    private async Task AdvanceAfterDelayAsync(CancellationToken token)
    {
        try
        {
