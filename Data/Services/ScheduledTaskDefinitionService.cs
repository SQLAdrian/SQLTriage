...
namespace SQLTriage.Data.Services
{
    ...
    public class ScheduledTaskDefinitionService
    {
        ...
        /// </summary>
        private static string? GuardTaskQuery(ScheduledTaskDefinition task)
        {
            if (task.TaskType != TaskType.SqlQuery)
                return null;
            var verdict = DangerousExecGuard.Inspect(task.Query);
            return verdict.IsAllowed ? null : verdict.Reason;
        }
