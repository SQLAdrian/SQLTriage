 ...
 *            EffortHours from the v2 corpus bundle.
 *
...
namespace SQLTriage.Data.Services
{
    public class RemediationWeightStore
    {
        ...
        /// Pass the baseline EffortHours from the v2 corpus bundle as the fallback.
        /// </summary>
        ...
        /// <summary>
        /// Persists the weight overrides: write a .tmp, then ONE File.Move that replaces the file in
        /// place. The delete-then-move it used to do left a real interval with no file on disk, so a
        /// crash inside that interval silently discarded every override an operator had set (same
        /// window as PersistedRemediationCreditLedger.Save, which named this class as its mirror).
        /// </summary>
        private void Save()
        {
            try
