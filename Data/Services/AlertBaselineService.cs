...
namespace SQLTriage.Data.Services
{
    ...
    ///   alert's fixed thresholds apply. Nor does a pair with no warning band (crit not beyond
    ///   warn), whatever the IQR (invariant A, 2026-09-19). See <see cref="SelectFences"/>.
    /// </summary>
    public class AlertBaselineService : IDisposable
    {
        ...
        /// <summary>
        /// Progress 0.0–1.0 across the alert/server pairs the seeder is seeding
        /// (<see cref="TrackedSampleCounts"/>).
        ...
        public double SeedingProgress
        {
            get
            {
                var counts = TrackedSampleCounts();
                if (counts.Count == 0) return 0;
                var total = counts.Count * MinSeedSamples;
                var current = counts.Sum(v => Math.Min(v, MinSeedSamples));
                return total == 0 ? 0 : Math.Min(1.0, (double)current / total);
            }
        }

        /// <summary>Number of seeded pairs (<see cref="TrackedSampleCounts"/>) that have reached the minimum sample threshold.</summary>
        public int SeededCount => TrackedSampleCounts().Count(v => v >= MinSeedSamples);
        /// <summary>Total alert/server pairs being seeded (<see cref="TrackedSampleCounts"/>).</summary>
        public int TotalPairCount => TrackedSampleCounts().Count;
        ...
        /// reason.</para>
        ///
        /// <para>INVARIANT A (lane alert-correctness, 2026-09-19): A LEARNED BASELINE NEVER RETURNS
        /// A FENCE PAIR WITH NO WARNING BAND. The harm is "crit does not exceed warn": the evaluator
        /// tests the two fences independently, so when they are equal every learned fire is
        /// Critical and Warning can never be the answer. <c>Iqr &lt;= 0</c> is only a PROXY for
        /// that. With spread, the P95 floor (or the P05 cap, for the lower pair) can still swallow
        /// the critical multiplier: <c>[40x0, 35x1, 20x2, 5x500]</c> has iqr 1.25 and warn = crit
        /// = 26.9. So the pair about to be returned must itself have a strict band: crit &gt; warn
        /// for the upper pair, critLower &lt; warnLower for the lower pair. Only that pair is
        /// gated. A window whose OTHER pair is degenerate keeps its fence:
        /// <c>[6x0, 25x99, 44x100, 25x101]</c> keeps its upper fence (102.125 / 104) although its
        /// lower pair is 0 / 0. The comparison is exact on purpose. <c>Math.Max</c> and
        /// <c>Math.Min</c> hand back one of their inputs unchanged, so a collapsed pair is
        /// bit-identical and needs no tolerance. The negated form also refuses a row persisted
        /// before the clamp existed (crit below warn) and a NaN. The multipliers are unchanged.
        /// Census: AlertBaselineFenceBandCensusTests, which sweeps generated windows through
        /// <see cref="ComputeStats"/> and this method for both operators and goes red on any
        /// returned pair without a strict band. This is the only method that hands a learned
        /// fence to the evaluator (<see cref="GetThresholds"/> is its one caller).</para>
        /// </summary>
        internal static (double? warn, double? crit) SelectFences(BaselineStats s, string op)
        {
            ...
            // Invariant A: never a pair with no warning band. Gate ONLY the pair being returned,
            // and in the negated form, so an inverted row persisted before the clamp and a NaN
            // fence are refused too. Exact comparison, no epsilon: see the para above.
            if (op == "less_than")
                return s.ThresholdCritLower < s.ThresholdWarnLower
                    ? (s.ThresholdWarnLower, s.ThresholdCritLower)
                    : (null, null);

            return s.ThresholdCrit > s.ThresholdWarn
        ...
        /// <summary>
        /// The alerts whose query the seeder may run: enabled, <c>canBaseline</c>, and NOT routed to a
        /// built-in handler.
        ///
        /// <para><b>Why the third clause (lane Q14, 2026-09-18).</b> <see cref="SeedOnePairAsync"/>
        /// executes <c>alert.Query</c> and records whatever the first result set holds as a baseline
        /// sample. A handler-routed alert's query field is inert text nobody else runs (the
        /// connectivity alerts ship <c>SELECT 1 AS value</c>), so seeding one would learn fences over
        /// the constant 1.
        /// <c>sql_response_time</c> shipped <c>canBaseline: true</c> when it became handler-routed in that
        /// lane; no handler-routed alert shipped with <c>canBaseline: true</c> before it, and lane
        /// alert-correctness 2 (2026-09-19, A2) turned it off. INVARIANT: no SHIPPED handler-routed alert
        /// declares a learned baseline, enforced by
        /// AlertRoutedBaselineCensusTests.No_shipped_handler_routed_alert_declares_a_learned_baseline; this
        /// clause stays as the guard for an operator's own routed alert. The routing
        /// test is the evaluator's own <see cref="AlertEvaluationService.IsRoutedToBuiltInHandler"/>,
        /// not a restatement, so the seeder and the evaluator cannot disagree about which text runs.
        /// Guarded by
        /// SpecialAlertPathNeverFiresTests.The_baseline_seeder_never_runs_a_handler_routed_alerts_inert_query,
        /// which drives <see cref="RunSeedCycleAsync"/> itself.</para>
        /// </summary>
        internal static List<AlertDefinition> SeedableAlerts(IEnumerable<AlertDefinition> alerts) =>
            alerts
                .Where(a => a.Enabled && a.CanBaseline && !AlertEvaluationService.IsRoutedToBuiltInHandler(a))
                .ToList();

        /// <summary>
        /// THE ONE SEEDING SET: every (seedable alert, enabled server) pair the seeder visits, in the
        /// order it visits them. <see cref="RunSeedCycleAsync"/> iterates it, and
        /// <see cref="TrackedSampleCounts"/> filters progress and completion to it, so the seeder and
        /// the "seeding complete" answer cannot disagree about which pairs count.
        /// </summary>
        private List<(AlertDefinition Alert, ServerConnection Conn, string Server)> SeedTargets()
        {
            var serverPairs = _connections.GetConnections()
                .Where(c => c.IsEnabled)
                .SelectMany(c => c.GetServerList().Select(s => (Conn: c, Server: s)))
                .ToList();

            return SeedableAlerts(_definitions.GetAllAlerts())
                .SelectMany(a => serverPairs.Select(p => (Alert: a, p.Conn, p.Server)))
                .ToList();
        }

        /// <summary>
        /// The sample counts of the tracked pairs that are also seed targets. Every reader of seeding
        /// progress calls this: <see cref="CheckSeedingComplete"/>, <see cref="SeedingProgress"/>,
        /// <see cref="SeededCount"/> and <see cref="TotalPairCount"/>.
        ///
        /// <para><b>Why (lane Q14 fix round, 2026-09-18).</b> These four used to read every key in
        /// <c>_sampleCounts</c>, and <see cref="LoadSampleCountsAsync"/> fills that from every row in
        /// the samples table. A pair the seeder will never visit could therefore hold completion false
        /// for ever. Lane Q14 added a member: <c>sql_response_time</c> left <see cref="SeedableAlerts"/>,
        /// so an upgraded install holding 1 to 9 of its old samples for a server never finished seeding.
        /// PROVED live by the Q14 gate on <c>.\NEW2022</c> (3 stale samples: SeedingComplete false after
        /// 12 cycles, against true on the base engine). The same rule covers the older members: a
        /// removed or disabled server, and an alert since disabled or no longer <c>canBaseline</c>.
        /// A pair the seeder visits and cannot fill, such as an unreachable server, still counts.
        /// Guarded by
        /// SpecialAlertPathNeverFiresTests.Seeding_completes_when_stale_samples_belong_to_pairs_the_seeder_never_visits.</para>
        /// </summary>
        private List<int> TrackedSampleCounts()
        {
            var targets = new HashSet<string>(
                SeedTargets().Select(t => Key(t.Alert.Id, t.Server)), StringComparer.OrdinalIgnoreCase);
        ...
        private async Task RunSeedCycleAsync()
        {
            ...
            try
            {
                ...
                foreach (var t in targets)
                    _sampleCounts.TryAdd(Key(t.Alert.Id, t.Server), 0);

                // Only seed pairs that still need samples
                var tasks = new List<Task>();
                foreach (var t in targets)
                {
                    if (_sampleCounts.GetValueOrDefault(Key(t.Alert.Id, t.Server), 0) >= MinSeedSamples) continue;

                    tasks.Add(SeedOnePairAsync(t.Alert, t.Conn, t.Server));
                }
        ...
        private void CheckSeedingComplete()
        {
            ...
            _seedingComplete = counts.All(v => v >= MinSeedSamples);
        }
