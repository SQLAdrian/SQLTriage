...
/// <c>tools/extract-update-pubkey.ps1 -Thumbprint &lt;sha1&gt;</c> to overwrite the PEM with
/// the real SubjectPublicKeyInfo, rebuild, and sign releases with the matching private key.
/// (The production cert is a Certum cloud/HSM cert — its key cannot be exported, so there
/// is no .pfx; it is selected by thumbprint from Cert:\CurrentUser\My.)
///
/// ⚠ <see cref="LoadEmbeddedTrustedKey"/> decides "is a real key embedded?" with a plain
/// substring search for the PEM header. That makes the placeholder file's WORDING
/// load-bearing: when the placeholder spelled that header out in its own prose,
/// <see cref="IsTrustedKeyConfigured"/> reported TRUE while the placeholder was still in
/// place — the opposite of the contract documented above. (Verification itself still failed
/// closed, so nothing was ever wrongly trusted.) Fixed 2026-07-21 by removing the header
/// string from the placeholder text. If you harden anything here, make this check require a
/// well-formed BEGIN..END block whose body is decodable base64 rather than a bare substring.
