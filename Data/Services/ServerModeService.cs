...
namespace SQLTriage.Data.Services
{
    ...
    public class ServerModeService : IAsyncDisposable
    {
        ...
        /// <summary>
        /// The address to open ON THIS MACHINE, and the one the toggle's "Open in browser" uses.
        ///
        /// <para><b>It is loopback, and that is a correctness property rather than a preference.</b>
        /// This used to be <c>http://{Dns.GetHostName()}:{Port}</c>. A hostname resolves to the
        /// box's LAN address, so the browser arrived on a NON-loopback socket, and
        /// <see cref="InteractiveAppAdmission"/> refuses an unauthenticated non-loopback caller the
        /// interactive application outright — 302 to <c>/auth/login?error=signin_required</c>.
        /// On an install with no RBAC users and no sign-in method configured, which is what every
        /// fresh install is, that login page has nothing to offer: the operator cannot sign in, so
        /// they cannot reach Settings, so they cannot create the first admin. The app was handing
        /// its own owner the one address at which it locks them out. MEASURED 2026-08-25 against
        /// <c>--server</c> on this box: <c>GET http://MSI:5150/</c> answered
        /// <c>302 /auth/login?error=signin_required</c> with <c>X-SQLTriage-Admission:
        /// sign-in-required</c>, while <c>GET http://localhost:5150/settings</c> answered 200.</para>
        ///
        /// <para>The boundary is not being weakened to fix this — nothing about who is admitted
        /// changes. The app simply stops advertising an address it will not serve. The LAN address
        /// is still published, as <see cref="LanUrl"/>, labelled for what it is.</para>
        /// </summary>
        public string? Url { get; private set; }

        /// <summary>HTTPS form of <see cref="Url"/>, or null when no HTTPS listener is bound.</summary>
        public string? HttpsUrl { get; private set; }

        /// <summary>
        /// The address for someone on ANOTHER machine. Reachable, but not anonymously: the
        /// admission boundary sends an unauthenticated caller from here to
        /// <c>/auth/login</c> and nowhere else, so a sign-in method has to be configured first.
        /// Kept separate from <see cref="Url"/> so the two can be labelled differently rather than
        /// one string having to mean both.
        /// </summary>
        public string? LanUrl { get; private set; }

        /// <summary>HTTPS form of <see cref="LanUrl"/>, or null when no HTTPS listener is bound.</summary>
        public string? LanHttpsUrl { get; private set; }
        ...
        /// <summary>
        /// The address published for use ON THIS MACHINE. Split out from <see cref="StartAsync"/>
        /// for the same reason <c>WindowsServiceHost.StartedLineTemplate</c> is: a rule that can
        /// only be checked by starting a Kestrel host is a rule nobody checks, and this one is a
        /// security-relevant rule — <c>LocalBrowserAdminAccessTests</c> drives the host component
        /// of what this returns against the real admission boundary over a real socket.
        ///
        /// <para>Internal for test visibility (InternalsVisibleTo SQLTriage.Tests).</para>
        /// </summary>
        internal static string ComposeLocalUrl(string scheme, int port) => $"{scheme}://localhost:{port}";

        /// <summary>
        /// The address published for use from ANOTHER machine. Reachable, and refused to an
        /// unauthenticated caller by <see cref="InteractiveAppAdmission"/> — which is correct and
        /// is why it is a separate property from <see cref="ComposeLocalUrl"/> rather than the
        /// single "the URL" this service used to publish.
        /// </summary>
        internal static string ComposeLanUrl(string scheme, string hostName, int port) =>
...
<li><a href='/' style='color:#4af'>Main App (Blazor)</a></li>
                ...
                // no-server-idle ruling: if the estate is empty at startup, announce the idle posture
                // exactly once. Collectors also call AnnounceOnce on their first idle tick (host-agnostic
                // safety net); the shared latch keeps it to a single line whichever fires first.
                try
                {
                    if (app.Services.GetService<IDbConnectionFactory>() is SqlServerConnectionFactory idleFactory
                        && idleFactory.IsUnconfigured)
                    {
                        SQLTriage.Data.NoServerIdleNotice.AnnounceOnce(_logger);
                    }
                }
                catch { /* the idle notice must never break startup */ }
                ...
                Url = ComposeLocalUrl("http", Port);
                HttpsUrl = _selfSignedCert != null ? ComposeLocalUrl("https", HttpsPort) : null;
                LanUrl = ComposeLanUrl("http", hostName, Port);
                LanHttpsUrl = _selfSignedCert != null ? ComposeLanUrl("https", hostName, HttpsPort) : null;
                _logger.LogInformation(
                    "Server mode started. Open {Url} in a browser ON THIS MACHINE. That address is "
                    + "loopback, which is the origin the admission boundary admits without a sign-in. "
                    + "From another machine the address is {LanUrl}, and it requires a configured "
                    + "sign-in method.",
                    Url, LanUrl);
        ...
        internal static void RegisterSharedSingletons(IServiceCollection services, IServiceProvider wpf)
        {
            ...
            // Ruling R1 (2026-09-01): the DDL journal writes both registers. Forwarded from the
            // WPF container like every other service here, so the server-mode UI journals through
            // the same instance the desktop host does.
            TryAdd<DdlJournal>(services, wpf);
            ...
            TryAdd<IRbacEnforcementPostureAccessor>(services, wpf);   // security-8: the shell's narrow posture handle, forwarded from the WPF RbacService singleton
