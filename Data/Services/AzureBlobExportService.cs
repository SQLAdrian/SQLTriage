...
namespace SQLTriage.Data.Services
{
    ...
    {
        /// <summary>
        /// Fallback container name used whenever AzureBlobExport:ContainerName is absent or blank.
        /// MUST stay lowercase: Azure rejects a container name that is not 3–63 chars of
        /// [a-z0-9-] starting and ending alphanumeric, so the old "SQLTriage" fallback made every
        /// unconfigured upload fail at the service. Matches the value Config/appsettings.json ships.
        /// </summary>
        public const string DefaultContainerName = "sqltriage";
        ...
        private void LoadConfiguration()
        {
            ...
            var rawContainer = _configuration["AzureBlobExport:ContainerName"] ?? DefaultContainerName;
            ...
            string containerName, string? blobPrefix = null)
        {
            ...
            containerName = string.IsNullOrWhiteSpace(containerName) ? DefaultContainerName : containerName.Trim('/');
                ...
                foreach (var prop in doc.RootElement.EnumerateObject())
                {
                    if (prop.Name == "AzureBlobExport")
                    {
                        root[prop.Name] = new Dictionary<string, object>
                        {
                            ...
                            ["ContainerName"] = _containerName ?? DefaultContainerName,
            ...
            byte[] uploadBytes, string blobName, string exportName, int rowCount, long originalSize, CancellationToken ct)
        {
            ...
            var headers = new BlobHttpHeaders
            {
                ...
                // Content-Encoding names a compression coding (gzip/br). "utf-8" is not one, and
                // blobs served with it make browsers fail the download with a decoding error —
                // uncompressed uploads must carry NO Content-Encoding at all.
                ContentEncoding = _compressUploads ? "gzip" : null
            };
