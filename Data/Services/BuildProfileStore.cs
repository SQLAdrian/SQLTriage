...
public sealed class BuildProfileStore
{
    ...
    {
        new ModuleInfo("core",            "Core",             ModuleKind.Ship,   "App shell: Index, Login, Onboarding, Settings, Servers, Guide, About, Health, App Metrics."),
    ...
    /// <summary>
    /// The catalog of gated report deliverables (Adrian's ruling 2026-07-21) — the report keys the
    /// community profile can carry, mirrored one-for-one by the SQLT_NO_REPORT_&lt;X&gt; plumbing in
    /// buildprofile.targets and the BuildModules.Reports consts. The four KEEPERS Adrian named
    /// (audit-evidence, risk-register, risk-acknowledgement, compliance-report) are always present
    /// in community and are documented here only so the authoring UI can show them as fixed-on.
    /// </summary>
    public static readonly IReadOnlyList<string> CommunityKeeperReports = new[]
    {
        "audit-evidence", "risk-register", "risk-acknowledgement",
    };

    /// <summary>The gated report keys — present in community ONLY when their value is exactly "on".
    /// compliance-report was briefly a fourth keeper; Adrian ruled it gated 2026-07-21
    /// ("three keepers only").</summary>
    public static readonly IReadOnlyList<string> GatedReports = new[]
    {
        "executive-summary", "dba-handoff", "hadr-posture",
        "findings-pdf", "executive-briefing", "cio-executive", "diagnostics-roadmap",
        "compliance-report",
    };
    ...
    private sealed class ProfileModel
    {
        ...
        public Dictionary<string, string> Reports { get; set; } = new(StringComparer.OrdinalIgnoreCase);
    ...
    /// <summary>Raw state string for a report key ("on"/"off"/…), or null when unavailable/unlisted.</summary>
    public string? GetReportState(string reportKey)
    {
        lock (_lock)
            return _model is not null && _model.Reports.TryGetValue(reportKey, out var s) ? s : null;
    }

    /// <summary>
    /// FAIL-CLOSED per-report rule (Adrian's ruling 2026-07-21), the exact rule buildprofile.targets
    /// enforces at build time: a report deliverable ships in a community build ONLY when its
    /// reports.&lt;key&gt; value is EXACTLY "on". "off", a garbled value, or an ABSENT/unlisted key all
    /// resolve to false — so a future report added without a community-profile "on" entry is gated,
    /// never shipped. The four keeper reports Adrian named are listed "on" in the profile and so
    /// pass this same rule; they additionally carry no exclude in buildprofile.targets.
    /// </summary>
    public bool ReportShipsInCommunity(string reportKey) => ReportShipsRule(GetReportState(reportKey));

    /// <summary>
    /// The pure fail-closed rule (extracted so it is deterministically testable without the
    /// file-locating store): a report deliverable ships in a community build ONLY when its profile
    /// state string is EXACTLY "on" (case-insensitive). null (an unlisted/future report), "off", or
    /// any garbled value ⇒ false. This is the same rule buildprofile.targets enforces at build time
    /// (SQLTExcludeReport&lt;X&gt; defaults true, cleared only by a literal "on").
    /// </summary>
    public static bool ReportShipsRule(string? state) =>
    ...
    private ProfileModel? Load()
    {
        try
        {
            ...
            if (m is not null)
            {
                ...
                m.Reports ??= new(StringComparer.OrdinalIgnoreCase);
                _logger.LogInformation("[BuildProfile] loaded {Modules} module + {Reports} report state(s) from {Path}.", m.Modules.Count, m.Reports.Count, _path);
                return m;
            }
        }
        catch (Exception ex)
