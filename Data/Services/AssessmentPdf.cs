...
public static partial class AssessmentPdf
{
    ...
    public static byte[] BuildFindingsReport(FindingsReport r)
    {
        ...
        }).WithMetadata(BuildPdfMetadata(r.Meta.Title, r.Meta.Subtitle)).GeneratePdf();
    }
#endif
    ...
    /// <summary>
    /// One shared QuestPDF document-metadata builder for every PDF this app generates. Before
    /// this (2026-08-20), no PDF set ANY document metadata — Title/Author/Producer/Creator were
    /// blank at all 14 <c>GeneratePdf()</c> call sites, so a client's PDF viewer showed an empty
    /// "Document Properties" panel and a saved file carried nothing that identified where it came
    /// from. Author/Creator/Producer are always "SQLTriage" (+ the shipped version, when
    /// Config/version.json is readable); Title and Subject vary per report. <paramref name="keywords"/>
    /// is the one place a caller can mirror an ALREADY-COMPUTED value (e.g. Audit Evidence's
    /// integrity hashes) into the file's metadata — never a new computation.
    /// </summary>
    public static DocumentMetadata BuildPdfMetadata(string title, string? subject = null, string? keywords = null)
    {
        var md = DocumentMetadata.Default;
        md.Title = title ?? "";
        md.Author = AppVersionTag.Value;
        md.Creator = AppVersionTag.Value;
        md.Producer = AppVersionTag.Value;
        if (!string.IsNullOrEmpty(subject)) md.Subject = subject;
        if (!string.IsNullOrEmpty(keywords)) md.Keywords = keywords;
        return md;
    }

    // Cached: every PDF build in the process reads this, and Config/version.json does not change
    // mid-process. "SQLTriage {version}", or bare "SQLTriage" when the file is absent/unreadable
    // (e.g. under a test host with no Config/ beside it) — metadata must never fail a PDF build.
    private static readonly Lazy<string> AppVersionTag = new(() =>
    {
        var version = ReadAppVersionForMetadata();
        return version == "unknown" ? "SQLTriage" : $"SQLTriage {version}";
    });

    private static string ReadAppVersionForMetadata()
    {
        try
        {
            var path = System.IO.Path.Combine(AppContext.BaseDirectory, "Config", "version.json");
        ...
        catch { /* metadata is cosmetic -- a missing/malformed version.json must never fail a PDF build */ }
        return "unknown";
    }
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingVertical(6).Column(col =>
                {
                    ...
                    // reports-r1-07 (2026-08-27): partial coverage WITHIN a scored control. The note
                    // above covers whole controls that were excluded; it says nothing about a control
                    // that was scored over part of itself. A check that could not fully assess the
                    // target is in neither the passing count nor the total, so the rate is computed
                    // over a smaller denominator than the control actually has. That fact reached the
                    // web page and stopped there. It travels with the number now.
                    var unassessedTotal = r.Families.Sum(f => f.UnassessedChecks);
                    if (unassessedTotal > 0)
                    {
                        var controlsAffected = r.Families.Count(f => f.UnassessedChecks > 0);
                        col.Item().Text($"Partial coverage: {unassessedTotal} check{(unassessedTotal == 1 ? "" : "s")} across {controlsAffected} control{(controlsAffected == 1 ? "" : "s")} could not fully assess the target and are in neither the passing count nor the total. Every score below marked with a Not-assessed count was computed over less than the whole control, and so was the headline percentage.")
                            .FontSize(8).Italic().FontColor(Warn(cb));
                    }

                    col.Item().Table(table =>
                    {
                        table.ColumnsDefinition(d => { d.ConstantColumn(90); d.RelativeColumn(); d.ConstantColumn(56); d.ConstantColumn(74); d.ConstantColumn(108); });
                        table.Header(h =>
                        {
                            foreach (var head in new[] { "Control ID", "Control", "Score", "Not assessed", "Status" })
                                h.Cell().BorderBottom(1).BorderColor(Muted).PaddingVertical(3).PaddingRight(4)
                                    .Text(head).FontSize(7.5f).Bold().FontColor(Muted);
                        });
                        var alt = false;
                        foreach (var f in r.Families)
                        {
                            ...
                            // A zero prints as an em dash rather than "0", the same empty-cell glyph
                            // the Score column one line above and KeyValueTable both use: a control
                            // with nothing unassessed has no partial-coverage fact to state.
                            table.Cell().Background(bg).PaddingVertical(3)
                                .Text(f.UnassessedChecks > 0 ? f.UnassessedChecks.ToString() : "—")   // voice-lint:allow
                                .FontSize(7.5f).FontColor(f.UnassessedChecks > 0 ? Warn(cb) : "#555555");
    ...
    public static byte[] BuildExecutiveSummaryBundle(ExecutiveSummaryBundle b)
    {
        ...
        // reports-r1-01 (2026-08-27): the estate test was ONLY `b.TopRisks.Any(r => r.ServersTotal > 0)`,
        // which is false for an estate with no findings — so a clean or unmeasured estate skipped the
        // scope block and rendered the donut branch instead. The DTO now states its own scope; the
        // risk-row inference is kept as a fallback for callers that predate the flag.
        var execEstate = b.EstateScope || b.TopRisks.Any(r => r.ServersTotal > 0);
        ...
        return Document.Create(c => c.Page(page =>
        {
            ...
            page.Content().PaddingVertical(6).Column(col =>
            {
                ...
                if (execEstate)
                {
                    var serverTotal = Math.Max(
                        b.EstateServerCount,
                        b.TopRisks.Select(r => r.ServersTotal).DefaultIfEmpty(0).Max());
                    col.Item().Background("#f7f7fb").Border(1).BorderColor(Line).Padding(10).Column(cc =>
                    {
                        ...
                        // No estate-wide health score exists, so the cover says that in words rather
                        // than leaving the reader to assume the missing ring means a perfect one.
                        cc.Item().PaddingTop(4).Text("No overall health score is shown: this is a roll-up across servers, not one measured instance.")
                ...
                if (b.TopRisks.Count == 0)
                    // reports-r2-06 (2026-08-27): this said "run a Vulnerability Assessment first".
                    // The 2026-07-16 honesty ruling made this bundle corpus-only (see
                    // ReportBundleService.GetMergedFindings), so a VA run cannot populate this table
                    // at all. The reader was sent to a scan that would leave the page exactly as
                    // empty. The HTML twin already said the right thing; the PDF is now the same.
                    col.Item().Text("No cached corpus findings. Run the check suite (Audit Assessment) first.").FontSize(10).Italic().FontColor(Muted);
                else
                    col.Item().Element(x => BundleFindingsTable(x, b.TopRisks, cb, framework: false, showRemediation: false, serversImpacted: execEstate, businessVoice: true, showStatus: true));
        ...
        })).WithMetadata(BuildPdfMetadata(b.Meta.Title, b.Meta.Subtitle)).GeneratePdf();
    }
#endif
        ...
        return Document.Create(c => c.Page(page =>
        {
            ...
            page.Content().PaddingVertical(6).Column(col =>
            {
                ...
                if (b.AllFindings.Count == 0)
                    // reports-r2-06 (2026-08-27): same correction as the Executive Summary above.
                    // The 2026-07-22 fix caught the HEADING on this bundle and missed the empty
                    // state three lines below it, so the deliverable still sent the reader to a VA
                    // scan that cannot populate a corpus-only table.
                    col.Item().Text("No cached corpus findings. Run the check suite (Audit Assessment) first.").FontSize(10).Italic().FontColor(Muted);
                else
                    col.Item().Element(x => BundleFindingsTable(x, b.AllFindings, cb, framework: false, showRemediation: false, serversImpacted: dbaEstate, showStatus: true));
        ...
        return Document.Create(c => c.Page(page =>
        {
            ...
            page.Content().PaddingVertical(6).Column(col =>
            {
                ...
                if (b.Findings.Count == 0)
                    // reports-r2-06 checked this site and left it alone, deliberately. The other three
                    // bundles were corrected because they are corpus-only and a VA run cannot populate
                    // them. Audit Evidence is the exception the 2026-07-16 ruling carved out: it reads
                    // GetCachedFindings (VulnerabilityAssessmentStateService), not GetMergedFindings,
                    // so a Vulnerability Assessment IS the scan that fills this table. Do not sweep
                    // this string with the other three.
                ...
                {
                    // reports-r2-09: an unmeasured count is stated, never printed as "0".
                    ("Total Audit Events", b.AuditEventCountAssessed
                        ? b.AuditEventCount.ToString("N0")
                ...
                col.Item().Element(x => KeyValueTable(x, new List<(string, string)>
                {
                    ...
                    // reports-r1-03 (2026-08-27): this row read ("Report Period","Last 30 days"), a
                    // hard-coded literal two rows above the findings hash. GatherAuditEvidence takes
                    // no date parameter and applies no time filter, so the window described nothing
                    // the hashed body was selected by. The 30-day window belongs to the audit-EVENT
                    // count in the table above and to nothing else on this page. Same reasoning as
                    // the 2026-07-22 removal of the identical literal from the Executive Summary.
                    ("Findings Scope", "Most recent completed assessment. No date filter is applied to the hashed findings."),
                    ("Document SHA-256",  b.Sha256),
                    ("Scan-data SHA-256", b.ScanDataSha256),
                }));
            });
            // The hashes are mirrored into the FILE'S metadata AFTER GatherAuditEvidence computed
            // them (ReportBundleService.cs) -- never recomputed here, and the formula they came
            // from is untouched. Keywords rather than Subject: Subject already carries the report
            // scope via BuildPdfMetadata's own default when one is supplied.
        })).WithMetadata(BuildPdfMetadata(b.Meta.Title, b.Meta.Subtitle,
            keywords: $"doc-sha256:{b.Sha256};scan-sha256:{b.ScanDataSha256}")).GeneratePdf();
    }
    ...
    public static byte[] BuildRiskRegisterBundle(RiskRegisterBundle b)
    {
        ...
        return Document.Create(c => c.Page(page =>
        {
            ...
            page.Content().PaddingVertical(6).Column(col =>
            {
                ...
                if (b.Rows.Count == 0)
                    // reports-r2-06 (2026-08-27): the third and last of the corpus-only bundles that
                    // sent the reader to a Vulnerability Assessment. GatherRiskRegister reads
                    // GetMergedFindings, which is corpus-only since the 2026-07-16 ruling, so a VA
                    // run leaves this register exactly as empty. Matches the HTML twin.
                    col.Item().Text("No outstanding bad-state risks. Run the check suite (Audit Assessment) first, or the estate is clean.")
        ...
        c.Table(table =>
        {
            ...
            foreach (var r in rows)
            {
                ...
                table.Cell().Background(bg).PaddingVertical(3).PaddingRight(4).Text(r.ReviewByUtc.HasValue ? r.ReviewByUtc.Value.ToString("yyyy-MM-dd") : "—")
                    .FontSize(7.5f).FontColor("#555555");
                if (estate)
                    table.Cell().Background(bg).PaddingVertical(3).Text(r.ServersImpactedLabel).FontSize(7.5f).FontColor("#555555");
    ...
    private static void BundleTitleBand(IContainer c, AssessmentMeta m, string tagline) =>
        c.BorderBottom(2).BorderColor(Brand).PaddingBottom(8).Row(row =>
        {
            row.RelativeItem().Column(cc =>
            {
                ...
                // The scope qualifier, directly beneath the scope it qualifies (2026-08-27). An
                // estate cover that says "All Servers (N)" over a seat-filtered set says what the
                // set leaves out in the same band; empty prints nothing, so every single-server
                // bundle's title band is unchanged.
                if (!string.IsNullOrWhiteSpace(m.CoverageNote))
                    cc.Item().Text(m.CoverageNote).FontSize(8.5f).Italic().FontColor(Warn(m.ColorBlind));
    ...
    /// <summary>
    /// The shared findings table for the report bundles.
    ///
    /// <para><paramref name="showStatus"/> (reports-r2-02, 2026-08-27): renders the Status the
    /// service already computed. Without it, a check that ERRORED and a check that genuinely FAILED
    /// were indistinguishable rows in a client deliverable — same severity badge, same table, no
    /// signal at all — while the Risk Register built from the SAME result set filtered them out. Two
    /// deliverables from one dataset disagreed about what a finding is. "Unassessed" is spelled out
    /// as "Could not run" because the client reading this is not holding the corpus vocabulary.</para>
    /// </summary>
    private static void BundleFindingsTable(IContainer c, List<BundleFinding> rows, bool cb, bool framework, bool showRemediation, bool serversImpacted = false, bool businessVoice = false, bool showStatus = false) =>
        c.Table(table =>
        {
            table.ColumnsDefinition(d =>
            {
                d.ConstantColumn(74);                 // id
                d.ConstantColumn(58);                 // severity
                if (showStatus) d.ConstantColumn(62); // status
            ...
            var heads = new List<string> { "ID", "Severity" };
            if (showStatus) heads.Add("Status");
            heads.Add("Check");
            heads.Add("Category");
            if (framework) heads.Add("Framework");
            heads.Add(lastHead);
            table.Header(h =>
            {
                foreach (var head in heads)
            ...
            {
                var bg = alt ? "#fafafa" : "#ffffff"; alt = !alt;
                var unassessed = string.Equals(f.Status, ComplianceScoreService.UnassessedStatus,
                ...
                // An unassessed row's declared severity is what the finding WOULD have been. It is
                // not a verdict on this server, so it is not painted like one.
                table.Cell().Background(bg).PaddingVertical(3).Text(string.IsNullOrWhiteSpace(f.Severity) ? "—" : f.Severity)
                    .FontSize(7.5f).Bold().FontColor(unassessed && showStatus ? Muted : Sev(f.Severity, cb));
                if (showStatus)
                    table.Cell().Background(bg).PaddingVertical(3).PaddingRight(4)
                        // The em dash is this file's established empty-cell glyph (KeyValueTable, the
                        // severity and category cells above). A Status column that used a different
                        // one would read as a value.
                        .Text(unassessed ? "Could not run" : (string.IsNullOrWhiteSpace(f.Status) ? "—" : f.Status)) // voice-lint:allow empty-cell glyph
                        .FontSize(7.5f).FontColor(unassessed ? Muted : "#555555");
                table.Cell().Background(bg).PaddingVertical(3).PaddingRight(6).Text(f.Name).FontSize(8).FontColor("#333333");
                table.Cell().Background(bg).PaddingVertical(3).PaddingRight(4).Text(string.IsNullOrWhiteSpace(f.Category) ? "—" : f.Category)
                    .FontSize(7.5f).FontColor("#555555");
                if (framework)
                    table.Cell().Background(bg).PaddingVertical(3).PaddingRight(4).Text(string.IsNullOrWhiteSpace(f.Framework) ? "—" : f.Framework)
                        .FontSize(7.5f).FontColor("#555555");
                if (serversImpacted)
                    // The count states what was measured, or the cell states that nothing was. The
                    // Status cell to its left already says "Could not run"; a number beside that
                    // sentence contradicted it. See BundleFinding.ServersImpactedLabel.
                    table.Cell().Background(bg).PaddingVertical(3).PaddingRight(2).Text(f.ServersImpactedLabel)
                        .FontSize(7.5f).FontColor(f.ServersImpactedAssessed ? "#555555" : Muted);
                else
                {
                    ...
                    // see the private meta repo worklist f540989.)
...
public sealed class ComplianceFamilyRow
{
    ...
    /// <summary>
    /// reports-r1-07 (2026-08-27): checks mapped to this control that could not fully assess the
    /// target, carried through from <see cref="ComplianceScoreService.ControlFamilyScore.UnassessedChecks"/>.
    /// Non-zero means <see cref="Percent"/> was computed over less than the whole control.
    ///
    /// <para>This field did not exist, so the exported PDF — the artefact that leaves the building
    /// as audit evidence — dropped a number the web page has rendered all along. Every Score cell
    /// and the "X% COMPLIANCE" headline were partial rates presented as full ones, with nothing in
    /// the bytes saying so. The count exists precisely so a consumer can say it out loud.</para>
    /// </summary>
    public int UnassessedChecks;
}
...
public sealed class BundleFinding
{
    ...
    /// <summary>reports-r2-02, second round (2026-08-27): false when <see cref="ServersImpacted"/> is
    /// not a measurement. A check that could not RUN on any server cannot be known to impact one, so
    /// there is no number to print. Defaults FALSE on purpose: a producer that sets nothing gets a
    /// cell that says nothing, never a count nobody took.</summary>
    public bool ServersImpactedAssessed;
    /// <summary>
    /// The impact cell as the client reads it. One property, because the PDF and its HTML twin were
    /// printing the same row two different ways and must not drift again.
    ///
    /// <para>WHAT THIS CLOSES. The first round of reports-r2-02 rendered the Status the service had
    /// already computed, so an errored check finally read "Could not run" — and left the cell beside
    /// it printing "1 of 1". The gate quoted that row off a live estate render: a client cover page
    /// stating, in one row, that a check could not run and that it impacted one of one servers. The
    /// same document's "Failed Checks by Server" appendix listed only the check that genuinely
    /// failed, so the deliverable contradicted itself inside one PDF. A stated self-contradiction in
    /// place of a silent over-claim. The count is now printed only when it counts something.</para>
    /// </summary>
    public string ServersImpactedLabel =>
        ServersImpactedAssessed ? $"{ServersImpacted} of {ServersTotal}" : "—"; // voice-lint:allow the established empty-cell glyph in these tables
}
...
public sealed class ExecutiveSummaryBundle
{
    ...
    /// <summary>reports-r1-01 (2026-08-27): true = an estate roll-up, which has no single health
    /// score. Carried explicitly because the builder used to INFER estate scope from the top-risk
    /// rows, and a clean estate has none — so an estate nothing had measured fell through to the
    /// single-server branch and drew a red 0/100 donut over a score nobody computed.</summary>
    public bool EstateScope;
    /// <summary>Servers in the estate scope. Rendered as the "N servers scanned" line; only read
    /// when <see cref="EstateScope"/> is true.</summary>
    public int EstateServerCount;
...
public sealed class AuditEvidenceBundle
{
    ...
    /// <summary>reports-r2-09 (2026-08-27): false when NOTHING measured the event count — no audit
    /// service, or the read threw. <see cref="AuditEventCount"/> is then meaningless and the row
    /// prints a sentence instead of a number, because "0 events" is a compliance claim.
    ///
    /// <para>Defaults FALSE (fix round, same day). It shipped defaulting TRUE, which is the exact
    /// shape of reports-r1-01 itself: <c>ScoreAssessed</c> defaulted true, one writer forgot to set
    /// it, and a client cover page printed a red 0/100 for an estate nobody had measured. The single
    /// caller today sets this explicitly, so the default was latent rather than live — and a latent
    /// fail-open default is a defect waiting for its second caller.</para></summary>
    public bool AuditEventCountAssessed;
...
public sealed class RiskRegisterRow
{
    ...
    public int ServersTotal;
    /// <summary>reports-r2-02, second round (2026-08-27): false when <see cref="ServersImpacted"/> is
    /// not a measurement — see <see cref="BundleFinding.ServersImpactedAssessed"/> for the shape this
    /// closes. The register admits a row on "Status is Failed OR Severity is Error", and a check that
    /// ERRORED carries Status = Unassessed while keeping its declared severity, so the Severity arm
    /// would admit a check that never ran. Latent on today's corpus, which declares no such
    /// severity; see ReportBundleService.BuildRiskRow for that measurement. Defaults FALSE.</summary>
    public bool ServersImpactedAssessed;
    /// <summary>The impact cell as the client reads it: the count when it was measured, this file's
    /// empty-cell glyph when it was not. The register carries no Status column, so the glyph is all
    /// this row can honestly say; whether such a row belongs in a risk register at all is a product
    /// question, recorded for Adrian rather than decided here.</summary>
    public string ServersImpactedLabel =>
        ServersImpactedAssessed ? $"{ServersImpacted} of {ServersTotal}" : "—"; // voice-lint:allow the established empty-cell glyph in these tables
}
