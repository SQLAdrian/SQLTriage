...
public sealed class HistoricalPerformanceService : IDisposable
{
        ...
        string? dbPath = null)
    {
        ...
        // Tick every hour. The initial rollup is kicked explicitly at app startup
        // (App.xaml.cs) so the hourly/daily tables populate from already-collected
        // raw snapshots immediately rather than waiting a full hour for the first tick.
    ...
    /// <param name="fullBackfill">When true, the hourly rollup processes ALL raw
    /// history (not just the last 2 completed hours). Used for the startup pass so
    /// accumulated snapshots surface immediately instead of trickling in.</param>
    public async Task RunRollupAsync(CancellationToken ct, bool fullBackfill = false)
    {
        try
        {
            var hourlyCount = await RollupHourlyAsync(ct, fullBackfill).ConfigureAwait(false);

            // Daily rollup re-aggregates every present day (incl. today) each tick.
            // It is idempotent (INSERT OR REPLACE keyed on server+day+wait_type) and
            // cheap (the hourly table is small), so today's data shows in the default
            // 30-day daily view without waiting for the next calendar day.
            var dailyCount = await RollupDailyAsync(ct).ConfigureAwait(false);

            await PurgeAsync(ct).ConfigureAwait(false);

            _logger.LogInformation("[ROLLUP] hourly +{H} rows, daily +{D} rows{Full}",
                hourlyCount, dailyCount, fullBackfill ? " (full backfill)" : "");
        }
        catch (Exception ex)
        {
    ...
    private async Task<int> RollupHourlyAsync(CancellationToken ct, bool fullBackfill = false)
    {
        ...
        // Incremental ticks only need the last couple of completed hours; the startup
        // full-backfill processes everything so accumulated history surfaces at once.
        var fromHour = fullBackfill
            ? DateTime.UnixEpoch                       // all raw history up to the current hour
            : cutoffHour.AddHours(-2);                 // last 2 completed hours (clock-skew margin)
    ...
    {
        // Re-aggregate every day present in the hourly table (including today, which is
        // still accumulating). Idempotent INSERT OR REPLACE, bounded by daily retention.
        var cutoffDay = DateTime.UtcNow.Date.AddDays(-_dailyRetentionDays).ToString("yyyy-MM-dd");
        int rowsWritten = 0;
        try
        {
            using var conn = await SqliteCipherHelper.OpenEncryptedAsync(_connectionString).ConfigureAwait(false);
            ...
            cmd.CommandText = @"
                ...
                WHERE substr(hour_utc, 1, 10) >= @cutoff
                GROUP BY server_name, day_utc, wait_type;
            ";
            cmd.Parameters.AddWithValue("@cutoff", cutoffDay);
            rowsWritten = await cmd.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
