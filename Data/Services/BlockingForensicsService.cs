...
namespace SQLTriage.Data.Services
{
    ...
    public sealed class BlockingForensicsService : IDisposable
    {
        ...
        /// <summary>
        /// Live execution plan for a SPID on the chosen instance, or null when there is nothing
        /// to show — the session is sleeping, has no compiled plan yet, or the instance is not
        /// resolvable. Read-only: one DMV SELECT, no state touched.
        /// <para>
        /// A blocking row carries only SPIDs (no sql_handle/plan_handle), which is fine because
        /// the DMV resolves the plan from the request itself. Note the asymmetry the UI must
        /// live with: the BLOCKED session almost always has a live request, while the BLOCKER
        /// is frequently idle-with-an-open-transaction and will legitimately return null.
        /// </para>
        /// </summary>
        public async Task<string?> GetLivePlanForSpidAsync(string serverName, int spid, CancellationToken ct = default)
        {
            var resolved = Resolve(serverName);
            ...
            try
            {
                using var sqlConn = new SqlConnection(
                    resolved.Value.conn.GetConnectionString(resolved.Value.server, "master"));
                await sqlConn.OpenAsync(ct);
                using var cmd = new SqlCommand(SessionDataService.LivePlanForSpidSql, sqlConn)
                {
                    CommandTimeout = 10
                };
            ...
            {
                // Never let a plan lookup break the forensics page — it is a side panel.
                _logger.LogDebug(ex, "Live plan lookup failed for SPID {Spid} on {Server}", spid, serverName);
                return null;
            }
        }
            ...
            ServerConnection conn, string serverName, CancellationToken ct)
        {
            ...
            try
            {
                return await ReadAndRecordDeadlocksAsync(sqlConn, DeadlockXelFileSql, serverName, ct);
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                // No .xel file-target access (permissions / hosted platforms) — the ring buffer
                // misses events (its serialized XML is capped) but is better than nothing.
                _logger.LogDebug(ex, "system_health .xel read failed for {Server}; falling back to ring buffer", serverName);
                return await ReadAndRecordDeadlocksAsync(sqlConn, DeadlockRingBufferSql, serverName, ct);
            }
        }

        private async Task<int> ReadAndRecordDeadlocksAsync(
            SqlConnection sqlConn, string sql, string serverName, CancellationToken ct)
        {
            int inserted = 0;
        ...
        // Deadlocks from the system_health event_file (.xel) target — the durable source. The
        // ring-buffer target's serialized XML view is size-capped and silently drops events, so
        // the files are authoritative. Returns (event_time, deadlock_xml-as-text) newest-first.
        private const string DeadlockXelFileSql = @"
            SELECT TOP (500)
                x.xed.value('(event/@timestamp)[1]', 'datetime2')                                        AS event_time,
                CAST(x.xed.query('(event/data[@name=""xml_report""]/value/deadlock)[1]') AS NVARCHAR(MAX)) AS deadlock_xml
            FROM sys.fn_xe_file_target_read_file(N'system_health*.xel', NULL, NULL, NULL) f
            CROSS APPLY (SELECT CAST(f.event_data AS XML)) x(xed)
            WHERE f.object_name = N'xml_deadlock_report'
            ORDER BY event_time DESC";

        // Ring-buffer fallback (used only when the .xel read fails, e.g. no file access on hosted
        // platforms). Fast-path: skip the XML cast entirely when the buffer contains no deadlock
        // reports. Returns (event_time, deadlock_xml-as-text).
