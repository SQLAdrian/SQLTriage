...
/// Uses AssessmentEngine with ruleset.json (loaded from the active bundle) for full check coverage.
/// Supports SQL, PowerShell, WMI, and Registry checks.
/// When the bundle is locked the service falls back to built-in comprehensive checks.
/// </summary>
public class SqlAssessmentService
{
    ...
    private List<AssessmentCheckDefinition>? _cachedCheckDefs;
    private Dictionary<string, AssessmentCheckDefinition>? _cachedCheckDefById;
    private bool _checkDefsLoaded;

    public SqlAssessmentService(
        ILogger<SqlAssessmentService> logger,
        ...
        AzureBlobExportService? blobExport = null)
    {
        ...
        {
            lock (_lock) { _checkDefsLoaded = false; _cachedCheckDefs = null; _cachedCheckDefById = null; }
        };
    }

    /// <summary>
    /// Returns the lazily-parsed check definitions from the bundle.
    /// Thread-safe; invalidated by BundleStateChanged.
    /// Falls back to built-in comprehensive checks when ruleset.json is absent from the bundle.
    /// </summary>
    private (List<AssessmentCheckDefinition> Checks, Dictionary<string, AssessmentCheckDefinition> ById) GetCheckDefinitions()
    {
        lock (_lock)
        {
            if (_checkDefsLoaded)
                return (_cachedCheckDefs!, _cachedCheckDefById!);

            _checkDefsLoaded = true;
            var text = _bundle.GetText("Config/ruleset.json");
            if (text is null)
            {
                _logger.LogWarning(
                    "ruleset.json not in current bundle (tier={Tier}); SqlAssessmentService using built-in fallback checks.",
                    _bundle.Tier);
                _cachedCheckDefs = GetComprehensiveChecks();
                _cachedCheckDefById = BuildById(_cachedCheckDefs);
                return (_cachedCheckDefs, _cachedCheckDefById);
            }
            ...
            try
            {
                // Parse bundle text — same logic as the original file-based loader
                ...
                else
                {
                    ...
                    foreach (var ruleEl in rulesEl.EnumerateArray())
                    {
                        ...
                        var businessImpact = ruleEl.TryGetProperty("business_impact", out var bi) ? bi.GetString() ?? "" : "";
                        ...
                        var checkDef = new AssessmentCheckDefinition
                        {
                            ...
                            Remediation = ruleEl.TryGetProperty("remediation", out var r) ? r.GetString() ?? "" : "",
                            Eli5Remediation = ruleEl.TryGetProperty("eli5Remediation", out var er) ? er.GetString() ?? "" : "",
            ...
            {
                _logger.LogError(ex, "Failed to parse ruleset.json from bundle; using built-in fallback checks.");
                checks = GetComprehensiveChecks();
            }

            _cachedCheckDefs = checks;
            _cachedCheckDefById = BuildById(checks);
            return (_cachedCheckDefs, _cachedCheckDefById);
        }
    }

    private static Dictionary<string, AssessmentCheckDefinition> BuildById(List<AssessmentCheckDefinition> checks) =>
        checks
    ...
    /// </summary>
    /// <param name="autoSaveCsv">
    /// Whether to auto-plant the per-server CSV in the exe's own output\ folder when the run
    /// finishes (see <see cref="MaybeSaveCsvToOutputFolderAsync"/>). Default true — unchanged
    /// behaviour for the desktop VA page's single-server (VulnerabilityAssessment.razor.cs:468)
    /// and multi-server (RunMultiServerAssessmentAsync, below) call sites, which is the file the
    /// page's own Import modal and VA File Browser glob back in. CliAuditHost's
    /// <c>--report audit-evidence</c> path passes false: it already writes its own artifacts
    /// under the operator's <c>--out</c>, and must not also plant an unrequested file beside the
    /// exe (observed defect, app-lane follow-up item 8).
    /// </param>
    public async Task<AssessmentSummary> RunServerAssessmentAsync(
        string connectionString, CancellationToken ct = default, bool autoSaveCsv = true)
    {
        var summary = new AssessmentSummary();
        var (checkDefs, checkDefsById) = GetCheckDefinitions();
        ...
        try
        {
            ...
            try
            {
                ...
                var dbTasks = dbNames.Select(async dbName =>
                {
                    ...
                    try
                    {
                        // SMO GetAssessmentResults can't be aborted mid-call; the most we
                        // can do is not start the assessment for a database not yet begun.
                ...
                foreach (var r in engineResults)
                {
                    ...
                    var checkId = check.GetType().GetProperty("Id")?.GetValue(check)?.ToString()
                                   ?? check.ToString() ?? "";
                    var displayId = check.GetType().GetProperty("DisplayId")?.GetValue(check)?.ToString() ?? "";
                    ...
                    var foundInRuleset = checkDefsById.TryGetValue(checkId, out var defForResult);
                    if (!foundInRuleset)
            ...
            if (!engineSucceeded)
            {
                ...
                foreach (var check in checkDefs)
                {
                    TotalChecksRun++;
                    try
                    {
            ...
            await MaybeSaveCsvToOutputFolderAsync(summary, serverName, autoSaveCsv);
...
ORDER BY gs.avg_user_impact DESC;";
            ...
            // S-1: same session-safety preamble the check lane uses — this reads three DMVs on a
            // live server and must not take shared locks or wait indefinitely behind DDL.
            using var cmd = new SqlCommand(SqlSessionSafety.DefaultPrefix + sql, conn) { CommandTimeout = 30 };
    ...
    private void AddInfoResult(AssessmentCheckDefinition check, string targetName, AssessmentSummary summary)
    {
        summary.Results.Add(new AssessmentResult
        {
            ...
            Eli5Description = check.Eli5Description,
...
SELECT @ThisDomain   [ThisDomain],
       ...
       CONVERT(VARCHAR, GETUTCDATE(), 120) [UTCDateTime];";
        ...
        try
        {
            // S-1: same session-safety preamble the check lane uses (see BuildMissingIndexesAsync).
            using var cmd = new SqlCommand(SqlSessionSafety.DefaultPrefix + sql, connection) { CommandTimeout = 30 };
            using var reader = await cmd.ExecuteReaderAsync();
    ...
    public string ExportToCsv(AssessmentSummary summary)
    {
        ...
        {
            var row = $"\"{EscapeCsv(result.SafeDisplayId)}\",\"{EscapeCsv(result.DisplayName)}\",\"{EscapeCsv(result.Message)}\",\"{result.Severity}\",\"{result.RawSeverity}\",\"{EscapeCsv(result.TargetName)}\",\"{result.TargetType}\",\"{EscapeCsv(result.Category)}\",\"{EscapeCsv(result.Description)}\",\"{EscapeCsv(result.Remediation)}\",\"{result.HelpLink}\",\"{result.Status}\"";
    ...
    /// <summary>
    /// The on/off gate <see cref="RunServerAssessmentAsync"/> calls instead of
    /// <see cref="SaveCsvToOutputFolderAsync"/> directly, so the CLI's opt-out is a real branch a
    /// test can drive (internal, not private — InternalsVisibleTo SQLTriage.Tests) rather than a
    /// claim only readable from source. <paramref name="autoSaveCsv"/> false is a no-op: no
    /// directory is touched, no file is written.
    /// </summary>
    internal async Task MaybeSaveCsvToOutputFolderAsync(AssessmentSummary summary, string serverName, bool autoSaveCsv)
    {
        if (autoSaveCsv)
    ...
    ///
    /// <para><b>Never attests nothing (lane9-05, 2026-08-28).</b> Adrian's 2026-07-16
    /// ruling was only half enforced: the Vulnerability Assessment page refuses to auto-export a run
    /// that measured nothing, and the multi-server runner then called this method unconditionally
    /// anyway. Proved by producing one - a four-target run in which every target failed wrote
    /// <c>VulnerabilityAssessment_MultiServer_4_&lt;stamp&gt;.csv</c>, 202 bytes, a header row and
    /// nothing else, for a run that measured nothing. The refusal lives HERE rather than at that one
    /// call site because the same shape reaches this method from the per-server path too: a server
    /// that connects and returns no rows produced the identical header-only artifact.</para>
    /// </summary>
    private async Task SaveCsvToOutputFolderAsync(AssessmentSummary summary, string serverName)
    {
        if (summary?.Results is null || summary.Results.Count == 0)
        {
            // A header row is not a report. An empty CSV in the output folder is picked up by the
            // page's own Import modal and File Browser, which glob this directory, so it is
            // offerable to a client as an assessment of the servers named in its filename.
            _logger.LogWarning(
                "CSV export skipped for {Server}: the run produced no assessment results, so there "
                + "is nothing to attest.", LogAnon.S(serverName));
            return;
        }
    ...
    ///
    /// <para><b>Returns a <see cref="MultiServerAssessmentRun"/>, not a bare summary
    /// (pages-r2-02).</b> A per-server failure is caught here so one bad target cannot fault the
    /// whole run, and until 2026-08-28 the only trace of it was a log line: the caller got the
    /// merge and had no way to tell which of its targets were in it. The Vulnerability Assessment
    /// page therefore named every TARGET on the client PDF cover beside a pass rate measured on
    /// the ones that answered. The outcome per target is now recorded where it happens and
    /// travels back with the merge. See the run type's own doc for why the reporting set cannot
    /// be recovered from the merged rows.</para>
    /// </summary>
    public async Task<MultiServerAssessmentRun> RunMultiServerAssessmentAsync(
        IReadOnlyList<(string ConnectionString, string ServerName)> targets,
        int parallelism = 3,
        IProgress<ServerAssessmentProgress>? progress = null,
        CancellationToken ct = default)
    {
        var allSummaries = new List<AssessmentSummary>(targets.Count);
        // Keyed by target index, not by name: two connections may legitimately name the same
        // instance, and a dictionary keyed by name would silently merge their outcomes.
        var outcomes = new ServerAssessmentOutcome?[targets.Count];
        ...
        var tasks = targets.Select(async (target, index) =>
        {
            ...
            try
            {
                // The Microsoft SQL Assessment Engine (SMO GetAssessmentResults) is a
                // synchronous, non-cancellable call — we can stop launching new servers
                // but not abort one already in flight. Skip any server not yet started.
                if (ct.IsCancellationRequested)
                {
                    outcomes[index] = new ServerAssessmentOutcome(
                        target.ServerName, 0, "the run was stopped before this server was attempted");
                    progress?.Report(new ServerAssessmentProgress(
                        completed, targets.Count, target.ServerName, ServerAssessmentPhase.Cancelled));
                    return;
                }
                progress?.Report(new ServerAssessmentProgress(
                    completed, targets.Count, target.ServerName, ServerAssessmentPhase.Starting));
                var summary = await RunServerAssessmentAsync(target.ConnectionString, ct);
                int done;
                lock (lockObj)
                {
                ...
                // Rows, not "it returned": a server that answers with nothing has measured
                // nothing, and a report that names it is asserting coverage it does not have.
                outcomes[index] = new ServerAssessmentOutcome(
                    target.ServerName,
                    summary.Results.Count,
                    summary.Results.Count > 0 ? null : "the assessment returned no results for this server");
                progress?.Report(new ServerAssessmentProgress(
                    Completed: done, Total: targets.Count, ServerName: target.ServerName,
                    ServerAssessmentPhase.Completed, RowCount: summary.Results.Count));
            }
            catch (OperationCanceledException)
            {
                // Cancelled before/at start - drop this server, but say so rather than letting it
                // vanish into a coverage figure that reads as if it had been assessed.
                outcomes[index] = new ServerAssessmentOutcome(
                    target.ServerName, 0, "the run was stopped before this server produced results");
                progress?.Report(new ServerAssessmentProgress(
                    completed, targets.Count, target.ServerName, ServerAssessmentPhase.Cancelled));
            }
            catch (Exception ex)
            {
                // One unreachable/failed server must not fault the whole multi-server run
                // (Task.WhenAll would surface the first exception and abandon the rest).
                // Log it, record WHY it produced nothing, advance progress, carry on.
                _logger.LogError(ex, "Assessment failed for {Server}; skipping and continuing", LogAnon.S(target.ServerName));
                int done;
                lock (lockObj) { done = ++completed; }
                outcomes[index] = new ServerAssessmentOutcome(target.ServerName, 0, ex.Message);
                progress?.Report(new ServerAssessmentProgress(
                    Completed: done, Total: targets.Count, ServerName: target.ServerName,
                    ServerAssessmentPhase.Failed, Error: ex.Message));
            }
            finally
            {
                semaphore.Release();
        ...
        return new MultiServerAssessmentRun
        {
            Summary = merged,
            PlannedServers = targets.Select(t => t.ServerName).ToList(),
            // A slot is only null if a task neither ran nor threw, which Task.WhenAll above rules
            // out; the coalesce is here so an unforeseen path degrades to "produced nothing, reason
            // unknown" rather than to a silently shorter coverage list.
            Outcomes = outcomes
                .Select((o, i) => o ?? new ServerAssessmentOutcome(targets[i].ServerName, 0, null))
                .ToList(),
        };
    }
    ...
    /// Supported forms:
    ///   Server[@Name='MSI']
    ///   Server[@Name='MSI']/Database[@Name='Sales_PowerBI']
    ///
    ...
    private sealed class MergeEntry
    {
        ...
        internal AssessmentResult ToResult()
        {
            var r = new AssessmentResult
            {
                ...
                DisplayId = _prototype.DisplayId,
                ...
                Eli5Description = _prototype.Eli5Description,
                HelpLink = _prototype.HelpLink,
                Remediation = _prototype.Remediation,
                Eli5Remediation = _prototype.Eli5Remediation,
...
public class AssessmentResult
{
    ...
    public string SafeDisplayId => !string.IsNullOrWhiteSpace(DisplayId) ? DisplayId : CheckId;
    public string DisplayName { get; set; } = "";
    ...
    /// <summary>Provenance of this finding for report bundles. "Microsoft VA" for the MS Vulnerability Assessment scan; "Corpus" for SQLTriage corpus check results.</summary>
    public string Source { get; set; } = "Microsoft VA";
