    ...
    public class AlertEvaluationService : IDisposable
    {
        ...
        public async Task EvaluateAllAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // the _lastEvaluation stamp stay strictly sequential and in catalogue order, and
                ...
                foreach (var alert in alerts)
                {
                    ...
                    var isSpecial = !string.IsNullOrEmpty(alert.QueryMode) && alert.QueryMode != "standard";
        ...
        private async Task<double?> CheckIoErrorsAsync(ServerConnection connection, string serverName)
        {
            try
            {
                ...
                try
                {
                    using var cmd = new SqlCommand(BuildIoErrorCheckSql(), (SqlConnection)sqlConn)
                    { CommandTimeout = 10 };
                    var result = await cmd.ExecuteScalarAsync();
                    return result == null || result == DBNull.Value ? 0 : Convert.ToDouble(result);
                }
            ...
            ServerReachabilityProbe? reach = null)
        {
            ...
            try
            {
                ...
                if (raw == null)
                {
                    // The query ran and returned no value: no rows, or a NULL scalar. That is NOT
                    // zero and NOT healthy - it is "nothing was measured this cycle" - so nothing is
                    // compared and no state changes. It used to return in silence, which left no
                    // trace anywhere that the alert had produced no reading; the reason is now said
                    // out loud, because an absence that states no reason is indistinguishable from
                    // an alert that measured a clean result (strings-r2-05: the windowed latency
                    // alerts reach this path legitimately whenever no I/O completed in the window).
                    _logger.LogDebug(
                        "Alert {AlertId} on {Server} not measured this cycle: its query returned no value (no rows, or a NULL scalar). No comparison was made and the alert's state is unchanged; this is not evidence the server is healthy",
        ...
        /// its SQL reads <c>CAST(COUNT(*) AS FLOAT) * 100.0 / MaxConnections</c>, which the text
        /// reader sees as a count and which is in fact a PERCENTAGE with a deliberate band at 80.
        /// Treating 1 as its smallest alertable value would have called a correct alert broken.</para>
        /// </summary>
        /// <summary>
        ...
        internal static double? SmallestAlertableValue(AlertDefinition alert)
        {
            ...
            // An alert routed by queryMode is evaluated by a built-in handler that runs its OWN SQL,
            // so the shape comes from the handler, not from the query field.
            if (!string.IsNullOrWhiteSpace(alert.QueryMode)
                && !string.Equals(alert.QueryMode, "standard", StringComparison.OrdinalIgnoreCase))
            {
                return SpecialHandlerValueShape(alert.QueryMode) switch
                {
