...
namespace SQLTriage.Data.Services
{
    ...
    public sealed class ImportBatchOutcome
    {
        ...
        /// <summary>
        /// Directory paths that were passed to <see cref="ImportResultsService.ImportPaths"/> and
        /// contained NO *.json files. Without this an <c>--import</c> against an empty folder reported
        /// "complete. 0/0 file(s) imported" and exited 0 — the "N path(s) to import" line counts the
        /// arguments, not files found, so nothing said the path yielded nothing (platform-r2-04).
        /// </summary>
        public IReadOnlyList<string> PathsWithNoFiles { get; init; } = Array.Empty<string>();
    ...
    public sealed class ImportResultsService
    {
        ...
        public ImportBatchOutcome ImportPaths(IEnumerable<string> paths)
        {
            ...
            foreach (var p in paths)
            {
                ...
                {
                    var found = Directory.GetFiles(p, "*.json", SearchOption.TopDirectoryOnly);
                    if (found.Length == 0)
                        emptyPaths.Add(p); // a directory that yielded no *.json — reported, not silent
                    else
            ...
            return new ImportBatchOutcome { Files = outcomes, PathsWithNoFiles = emptyPaths };
        }
