...
namespace SQLTriage.Data.Services
{
    ...
    public class WindowsServiceHost
    {
        ...
        private static int RunServer(string[] args)
        {
            ...
            try
            {
                ...
                app.Lifetime.ApplicationStarted.Register(() => _ = Task.Run(() =>
                {
                    ...
                    // Which ADDRESS to open, not just which port is bound. The two are not the
                    // same fact and the difference is what locks an operator out:
                    // InteractiveAppAdmission admits an unauthenticated caller only on loopback,
                    // so http://localhost:{port} reaches Settings on an install with no RBAC
                    // users, and http://{machine}:{port} — the same box, the same listener —
                    // answers 302 /auth/login?error=signin_required. On a fresh install that
                    // login page has no sign-in method to offer, which is a dead end. Measured
                    // 2026-08-25 against this host on this box. The port line above says the
                    // service is up; this line says where its owner can actually get in.
                    // ⚠ Two tokens, two arguments. Serilog binds message-template tokens to
                    // arguments positionally, so a template naming {Port} twice against one
                    // argument renders the second token as the literal text "{Port}" — a startup
                    // line telling the operator to open a URL with a placeholder in it.
                    Log.Information(
                        "[Access] Open http://localhost:{Port} in a browser ON THIS MACHINE to administer "
                        + "SQLTriage. Loopback is the origin the admission boundary admits without a "
                        + "sign-in, so it is the only way to configure the first admin: go to "
                        + "{FirstAdminUrl}, add an Admin account, then switch RBAC on. Requests from "
                        + "another machine are redirected to /auth/login and need a sign-in method "
                        + "configured first.",
                        port, $"http://localhost:{port}/settings?tab=security");
        ...
        {
            // no-server-idle ruling: NO implicit "Server=." local default (see SqlServerConnectionFactory.IsUnconfigured).
            ...
            // #88 change-item ledger. Registered here since 2026-09-01. This container already
            // RESOLVED ChangeItemService for the follow-up seam below without ever registering it,
            // so on a service-only install that GetService returned null and the seam was dead. It
            // is also the second register ruling R1 requires for every app-issued DDL, and the DDL
            // surfaces are reachable in service mode, so a missing registration would have made
            // "both registers" false in exactly the host that runs unattended.
            services.AddSingleton<ChangeItemService>();
            services.AddSingleton<DdlJournal>(sp => new DdlJournal(
                sp.GetService<AuditLogService>(),
                sp.GetService<ChangeItemService>(),
                sp.GetService<ILogger<DdlJournal>>()));
        ...
        internal static IReadOnlyList<BackgroundStop> InitializeBackgroundServices(IServiceProvider services)
        {
            ...
            // ── no-server-idle: the HOST says it, not whichever collector ticks first ────────────
            //
            // Adrian's ruling on residual 3 (2026-09-08). NoServerIdleNotice.AnnounceOnce is
            // process-once, and until now the --server lane had no call of its own: the collectors
            // (MetricHistoryCollectorService, AlertEvaluationService) and the desktop server-mode
            // path each announce, so on the 2026-09-08 live run the single idle line was emitted by
            // whichever collector reached its first tick. The line's logger CATEGORY therefore named
            // a collector, and the host's own startup path claimed nothing at all about the posture
            // it had just composed.
            //
            // Announcing HERE — before anything below is started, with the host's own logger — makes
            // the host the author of the line, and turns the collectors' later calls into the no-ops
            // the once-only latch was always meant to make them. Guarded on the same predicate the
            // factory exposes (SqlServerConnectionFactory.IsUnconfigured; see RegisterAllServices),
            // so a configured install says nothing.
            var idleFactory = services.GetService<SqlServerConnectionFactory>();
            if (idleFactory != null && idleFactory.IsUnconfigured)
            {
                var hostLogger = services.GetService<ILoggerFactory>()?
                    .CreateLogger(typeof(WindowsServiceHost).FullName ?? nameof(WindowsServiceHost));
                if (hostLogger != null)
                    SQLTriage.Data.NoServerIdleNotice.AnnounceOnce(hostLogger);
            }
            ...
            // Retained metric history (DECISIONS 2026-08-26 18:23 ruling 4). Started HERE as well as in
            // App.xaml.cs deliberately: a service install has no browser attached, so without this the
            // only writer would be a dashboard fetch that never happens and retention would be a
            // desktop-only feature.
            var metricHistory = services.GetService<MetricHistoryCollectorService>();
            metricHistory?.Start();
            Capture(metricHistory, s => s.Stop());
