...
namespace SQLTriage.Data.Services
{
    ...
    public class BenchmarkService
    {
        ...
        public BenchmarkService(
            ...
            liveQueriesCacheStore cacheStore,
            ConnectionHealthService? healthService = null)
        {
        ...
        {
            const string sql = "SELECT AVG(CASE WHEN s.runnable_tasks_count > 0 THEN 1.0 ELSE 0 END) * 100 AS PctSchedulersWithRunnableTasks, AVG(CAST(s.runnable_tasks_count AS FLOAT)) AS AvgRunnableTasks FROM sys.dm_os_schedulers AS s WITH (NOLOCK) WHERE s.status = 'VISIBLE ONLINE';";
            ...
            if (await reader.ReadAsync())
            {
                ...
                var avgLoadFactor = Convert.ToDouble(reader[1] ?? 0);
                return avgRunnableTasks;
            }
        ...
        private async Task StoreBenchmarkResultsAsync(BenchmarkResult result)
        {
            ...
            // Fire-and-forget dedicated table storage for comparison view
            var _ = StoreBenchmarkResultsToDedicatedTablesAsync(result);
        }

        private void EnsureBenchmarkTables()
        {
            if (_tablesEnsured) return;
            try
            {
                using var conn = _cacheStore.CreateExternalConnection();
                conn.Open();
                ...
                cmd.CommandText = @"
                    ...
                    CREATE INDEX IF NOT EXISTS idx_benchmark_runs_server
                        ON benchmark_runs(server_name, captured_at_utc);
                ";
                cmd.ExecuteNonQuery();
                _tablesEnsured = true;
            }
            ...
            {
                Serilog.Log.Error(ex, "BenchmarkService: failed to ensure benchmark tables");
            }
        }

        private async Task StoreBenchmarkResultsToDedicatedTablesAsync(BenchmarkResult result)
        {
            ...
            try
            {
                ...
                {
                    var caps = _healthService.GetCapabilities(result.ServerName);
                    if (caps != null)
                    {
                        serverVersion = caps.MajorVersion > 0 ? caps.MajorVersion.ToString() : null;
                        serverEdition = caps.Edition;
                    }
                }
                ...
                {
                    cmd.CommandText = @"
                        INSERT INTO benchmark_runs (server_name, captured_at_utc, server_version, server_edition)
                        VALUES (@server, @utc, @ver, @edition);
                        SELECT last_insert_rowid();
                    ";
                    cmd.Parameters.AddWithValue("@server", result.ServerName);
                    cmd.Parameters.AddWithValue("@utc", result.Timestamp.ToString("o"));
                    cmd.Parameters.AddWithValue("@ver", (object?)serverVersion ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@edition", (object?)serverEdition ?? DBNull.Value);
                    runId = (long)(await cmd.ExecuteScalarAsync())!;
                }

                var metrics = new[]
                {
                    ("cpu_integer_ms",     result.CpuIntegerBenchmarkMs, "ms"),
                    ("string_ops_ms",      result.StringOpsBenchmarkMs,  "ms"),
                    ("memory_access_ms",   result.MemoryAccessBenchmarkMs,"ms"),
                    ("signal_wait_pct",    result.SignalWaitPercentage,  "%"),
                    ("scheduler_runnable_avg", result.CpuSchedulerDelayMs,  "tasks"),
                };

                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                        INSERT INTO benchmark_metrics (run_id, metric_name, value_numeric, value_unit)
                        VALUES (@runId, @name, @val, @unit);
                    ";
                    ...
                    foreach (var (name, val, unit) in metrics)
                    {
                        pRunId.Value = runId;
                        pName.Value  = name;
                        pVal.Value   = val;
            ...
            {
                Serilog.Log.Error(ex, "BenchmarkService: failed to store to dedicated tables for {Server}", result.ServerName);
            }
        }

        /// <summary>
        /// Returns the most recent benchmark run per server for comparison.
        /// </summary>
        public List<BenchmarkRunRecord> GetLatestRuns()
        {
            EnsureBenchmarkTables();
            var results = new List<BenchmarkRunRecord>();
            try
            {
                using var conn = _cacheStore.CreateExternalConnection();
                conn.Open();
                using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
                    SELECT r.run_id, r.server_name, r.captured_at_utc, r.server_version, r.server_edition,
                           m.metric_name, m.value_numeric, m.value_unit
                    FROM benchmark_runs r
                    JOIN benchmark_metrics m ON m.run_id = r.run_id
                    WHERE r.run_id IN (
                        SELECT MAX(run_id) FROM benchmark_runs GROUP BY server_name
                    )
                    ORDER BY r.server_name, m.metric_name;
                ";
                using var reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    results.Add(new BenchmarkRunRecord(
                        reader.GetInt64(0),
                        reader.GetString(1),
            ...
            {
                Serilog.Log.Error(ex, "BenchmarkService: failed to read benchmark runs");
            }
            return results;
        }
    }
    ...
    /// <summary>
    /// A single metric row from the benchmark_runs + benchmark_metrics join,
    /// used by the comparison grid on the /benchmark page.
    /// </summary>
