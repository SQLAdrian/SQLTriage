...
namespace SQLTriage.Data.Services
{
    ...
    public class AlertHistoryService : IDisposable
    {
            ...
            try
            {
                ...
                cmd.CommandText = @"
                    ...
                    CREATE TABLE IF NOT EXISTS alert_history (
                        ...
                        value           REAL NOT NULL,
                        -- REAL NOT NULL predates the firing-basis work and stays that way so an
                        -- existing install needs no table rebuild. A fire with NO threshold
                        -- (a trend, a baseline deviation, or an unrecorded condition) stores 0
                        -- here as a COLUMN FILLER and is reconstructed as null from basis_kind on
                        -- the way out; no reader treats this 0 as a measurement. Only
                        -- FixedThreshold and LearnedBaseline fires store a real number.
        ...
        /// <summary>
        /// C3 (2026-08-05). The stored threshold is only a measurement when the basis that fired
        /// carried one, and two of the five do: FixedThreshold and LearnedBaseline. Trend,
        /// baseline-deviation and unrecorded fires store 0 as a column filler (the column is REAL
        /// NOT NULL and predates this work), so this hands back null for them rather than a zero
        /// that reads as "the threshold was 0.00". A deviation fire is built by
        /// <c>FiringBasis.Deviation</c>, which leaves Threshold null by construction, so its stored
        /// 0 was never anything but the filler.
        ///
        /// The test names the bases that DO carry a number, not the ones that do not: a sixth basis
        /// added later then reads back as null until someone states that it measures a threshold,
        /// which is the safe direction to be wrong in.
        ///
        /// A row written BEFORE basis_kind existed has an empty basis, and those keep their stored
        /// number: we do not know that no threshold fired, and inventing a null would be the same
        /// error in the other direction.
        /// </summary>
        private static double? ThresholdFromRow(SqliteDataReader reader)
        {
            var stored = reader.GetDouble(reader.GetOrdinal("threshold_value"));
            var basis = BasisKindFromRow(reader);
            return basis is "" or "FixedThreshold" or "LearnedBaseline" ? stored : null;
        }

        /// <summary>Reads basis_kind, empty when the column has not been migrated in yet.</summary>
        private static string BasisKindFromRow(SqliteDataReader reader)
        {
            try
            {
                var ord = reader.GetOrdinal("basis_kind");
        ...
        private void MigrateIncidentColumns()
        {
            var columns = new[]
            {
                ...
                ("incident_notes",    "TEXT"),
                // C3 (2026-08-05): which of the four conditions fired. Without it a stored
                // threshold of 0 is ambiguous between "the threshold was zero" and "there was
                // no threshold", and the second is what a trend fire actually means.
        ...
        public long UpsertAlert(AlertState state)
        {
            try
            {
                ...
                // alerts-r1-13: the UPDATE branch used to refresh value/severity/message but NOT
                // threshold_value or basis_kind, so an already-Active alert that re-fired on a
                // DIFFERENT basis (fixed threshold no longer breached, learned fence or trend now
                // firing) kept the first fire's stale threshold and basis on disk. ThresholdFromRow
                // then handed that stale number back as a measurement because the stale basis_kind
                // said it was one. Same column-filler convention as INSERT: a fire with no threshold
                // stores 0 and is reconstructed as null from basis_kind on the way out.
                ...
                updateCmd.Parameters.AddWithValue("@threshold", state.ThresholdValue ?? 0d);
                updateCmd.Parameters.AddWithValue("@basisKind", state.BasisKind ?? "");
                ...
                insertCmd.CommandText = @"
                    INSERT INTO alert_history
                        (alert_id, alert_name, server_name, severity, value, threshold_value,
                         basis_kind, status, hit_count, first_triggered, last_triggered, message)
                    VALUES
                        (@alertId, @alertName, @server, @severity, @value, @threshold,
                         @basisKind, @status, 1, @first, @last, @message);
                ...
                // Column filler only: threshold_value is REAL NOT NULL and a fire with no
                // threshold has none to store — a trend, a baseline deviation, or an unrecorded
                // condition. basis_kind beside it is what makes the 0 readable, and
                // ThresholdFromRow reconstructs the null from it on the way out.
                insertCmd.Parameters.AddWithValue("@threshold", state.ThresholdValue ?? 0d);
                insertCmd.Parameters.AddWithValue("@basisKind", state.BasisKind ?? "");
        ...
        /// <summary>
        /// Acknowledges one active alert by its history row id. Returns true when a row moved from
        /// Active to Acknowledged, false when no active row carries that id (already acknowledged,
        /// resolved, or never existed). The REST <c>/alerts/{id}/acknowledge</c> endpoint uses this
        /// so its response reflects what actually happened rather than a blanket success (r2-06).
        /// </summary>
        public bool AcknowledgeById(long id)
        {
            try
            {
                using var conn = SqliteCipherHelper.OpenEncrypted(_connectionString);
            ...
            {
                _logger.LogError(ex, "Failed to acknowledge alert id {Id}", id);
                return false;
            }
        }

        /// <summary>Acknowledges every Active alert. Returns the number of rows moved to Acknowledged.</summary>
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    var rec = new AlertHistoryRecord
                    {
                        ...
                        ThresholdValue = ThresholdFromRow(reader),
                        BasisKind = BasisKindFromRow(reader),
