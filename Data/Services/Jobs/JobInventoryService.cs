...
namespace SQLTriage.Data.Services.Jobs
{
    public class JobInventoryService
    {
        ...
        /// <summary>
        /// Every Agent job on <paramref name="serverName"/>, with its steps in step order.
        /// Returns an empty list (never throws) when the instance is unreachable or has no
        /// Agent — the callers are UI surfaces that must degrade, not crash.
        /// </summary>
        public async Task<IReadOnlyList<AgentJobDefinition>> GetJobsAsync(
            string serverName, CancellationToken ct = default)
        {
            var conn = ResolveConnection(serverName);
            if (conn is null)
            {
                _logger.LogDebug("Job inventory: no connection resolves instance {Server}", serverName);
                return Array.Empty<AgentJobDefinition>();
            }
            ...
            return new List<AgentJobDefinition>(byId.Values);
        }
