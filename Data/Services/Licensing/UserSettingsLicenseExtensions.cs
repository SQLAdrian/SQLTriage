...
public static class UserSettingsLicenseExtensions
{
    ...
    /// <summary>
    /// Resolves the settings file path by ASKING the instance, via
    /// <c>UserSettingsService.SettingsFilePath</c>.
    ///
    /// <para>It used to take the service and discard it — the parameter was named <c>_</c> — and
    /// re-derive <c>%APPDATA%\SQLTriage\user-settings.json</c> itself, "mirroring" the constructor.
    /// Two copies of one path is a defect waiting for the day they disagree, and 2026-08-04 was
    /// that day: giving <see cref="UserSettingsService"/> a test seam moved the service's file
    /// without moving this one, so <see cref="ClearLicense"/> and <c>SaveLicense</c> would have gone
    /// on writing the operator's REAL profile while every test looked green. Derive the path from
    /// the object, never alongside it.</para>
    /// </summary>
    private static string ResolveFilePath(SQLTriage.Data.UserSettingsService userSettings)
        => userSettings.SettingsFilePath;
