...
public static class BundleCrypto
{
    ...
    /// <summary>
    /// Whether a file's bytes are shaped like a bundle this codec can read, and if not, why not
    /// in a sentence an operator can act on.
    ///
    /// <para>Deliberately says nothing about whether the bundle will DECRYPT — that needs the
    /// key and the customer name, and conflating "this is the wrong file" with "this is the
    /// right file and your key is stale" is what left the activation card offering a list of
    /// three guesses instead of the one true cause. This answers the first question only, from
    /// the same magic and wire-version constants <see cref="DecryptManifest"/> enforces, so the
    /// installer cannot drift from the reader.</para>
    /// </summary>
    public static bool LooksLikeBundle(ReadOnlySpan<byte> wire, out string problem)
    {
        if (wire.Length < HeaderSize + 1)
        {
            problem = $"the file is {wire.Length} bytes, which is shorter than a bundle header "
                    + $"({HeaderSize} bytes), so it is truncated or it is not a bundle.";
            return false;
        }

        if (wire[0] != Magic[0] || wire[1] != Magic[1] || wire[2] != Magic[2] || wire[3] != Magic[3])
        {
            problem = "the file does not start with the SQLTriage bundle marker, so it is not a "
                    + "SQLTriage licence bundle (a renamed .zip or a partial download looks like this).";
            return false;
        }
        ...
        if (ver != WireVersion)
        {
            problem = $"the file is bundle wire version {ver} and this build reads version {WireVersion}. "
                    + "Install the SQLTriage version the bundle was issued for.";
            return false;
        }
