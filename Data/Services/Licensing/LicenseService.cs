...
public sealed class LicenseService
{
    ...
    [SQLTriage.Data.I1ReadFailureMayWrite(
        "The reads guarded here are of INBOUND key files an operator dropped beside the exe, not of a "
        + "config store this method then overwrites. The two writes reachable from the handler are "
        + "RenameRejected, which renames a rejected key file aside and preserves it rather than "
        + "deleting it, and TryActivate, which persists licence state derived from a pair that DID "
        + "open. Neither puts a built-in default on a customer's configuration.")]
