...
public sealed class ComplianceMappingService
{
    ...
    /// excluding any framework listed in Compliance:HiddenFrameworks (default: none).
                    ...
                    foreach (var fwEl in fwArray.EnumerateArray())
                    {
                        ...
                        if (fwEl.TryGetProperty("categories", out var catArray))
                        {
                            foreach (var catEl in catArray.EnumerateArray())
                            {
                                ...
                                // notTested: control SQLTriage does not actually test — rendered un-scored and
                                // excluded from the aggregate (see control_mappings.json $schema.notes). Honesty gate.
                                var notTested = catEl.TryGetProperty("notTested", out var nt) && nt.ValueKind == JsonValueKind.True;
                                var notTestedReason = catEl.TryGetProperty("notTestedReason", out var ntr) ? ntr.GetString() ?? "" : "";

                                categories.Add(new FrameworkCategory
                                {
                                    Id = cid,
                                    Name = cname,
                                    SqlCheckHints = hints,
                        ...
                        {
                            // A control SQLTriage does not test is never advertised as satisfiable by a
                            // finding — skip it in the reverse map so the VA "Compliance:" hints stay honest.
                            if (cat.NotTested) continue;
    ...
    public sealed class FrameworkCategory
    {
        ...
        /// <summary>True when SQLTriage does not actually test this control — it is rendered un-scored
        /// ("Not currently tested") and excluded from the framework aggregate score.</summary>
        public bool NotTested { get; init; }
        /// <summary>Honest one-line explanation shown alongside a NotTested control. Empty when unset.</summary>
