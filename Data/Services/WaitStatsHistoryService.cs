...
public class WaitStatsHistoryService : IDisposable
{
    ...
    /// <summary>
    /// Per-server wait-pressure profile: total delta wait ms summed by category over
    /// the window, idle/benign excluded. Feeds the CIO per-server wait radar.
    /// </summary>
    public Dictionary<string, double> GetCategoryProfile(string serverName, int hours = 1)
    {
        ...
        try
        {
            ...
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"
                SELECT category, SUM(delta_wait_ms)
                FROM wait_stats_history
                WHERE server_name = @s AND recorded_at >= @cutoff
                  AND category <> 'IdleBenign'
                GROUP BY category;
            ";
            cmd.Parameters.AddWithValue("@s", serverName);
            cmd.Parameters.AddWithValue("@cutoff", DateTime.UtcNow.AddHours(-hours).ToString("o"));
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
                profile[reader.GetString(0)] = reader.IsDBNull(1) ? 0 : reader.GetDouble(1);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to read wait-category profile for {Server}", serverName);
        }
        return profile;
    }
