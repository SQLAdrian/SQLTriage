...
public static class ExecutionPlanParser
{
    private static string GenerateUniqueIndexName(string? table, List<string> keyCols)
    {
        ...
        // Deterministic suffix over the FULL column list so two suggestions that share
        // the first three key columns still get distinct names (same input → same name).
        var keyMaterial = table + "|" + string.Join("|", keyCols);
        var hash = System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(keyMaterial));
        var suffix = "_" + Convert.ToHexString(hash, 0, 3);
        if (baseName.Length + suffix.Length > 128) baseName = baseName.Substring(0, 128 - suffix.Length);
    ...
    {
        // Fallback ids go negative (below the -1 SELECT root) so they can never
        // collide with a real XML NodeId (>= 0) parsed later in the same graph.
    ...
    private static PlanSummary ExtractSummary(XElement stmt, XElement queryPlan)
    {
        ...
        // NOTE: CachedPlanAge is an AGE IN SECONDS, not a timestamp — do not use it here.
        var creationTime = stmt.Attribute("CreationTime")?.Value;
        ...
        // GoodEnoughPlanFound is the optimizer's NORMAL end state — only warn on
        // genuine aborts (TimeOut, MemoryLimitExceeded).
        if (summary.EarlyAbortReason != null
            && !summary.EarlyAbortReason.Equals("GoodEnoughPlanFound", StringComparison.OrdinalIgnoreCase))
