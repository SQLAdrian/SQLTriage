...
public class XEventService
{
    ...
    ///
    /// <para>⚠ <b>UNESCAPED, AND NOT RULED (2026-08-25).</b> <paramref name="sessionName"/> is
    /// interpolated raw into <c>[{sessionName}]</c> and into the .xel filename literal. It is NOT a
    ...
    /// event sessions. It is NOT privilege escalation and it is NOT reachable in a community build.
    /// The caller is gated on <c>run_scripts</c>, the permission that already grants arbitrary
    /// T-SQL, and buildprofile.targets Content-Removes Pages/XEvents.razor from community. The
    /// shape predates this lane and is unchanged by it. Escaping (<c>]</c> doubled for the
    /// identifier, <c>'</c> doubled for the filename) is a one-line change waiting on a ruling; an
    /// earlier lane report claimed every caller passed a literal, which was false.</para>
    /// </summary>
