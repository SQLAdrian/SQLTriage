    ...
    public class ServerConfigScriptService
    {
        ...
        public async Task RunAsync(bool apply, string? operatorName, Action<string, bool> onMessage, CancellationToken ct = default)
        {
            ...
            foreach (var batch in batches)
            {
                ...
                {
                    // Reported as INFORMATION (isError: false) deliberately. A non-readable AG replica is
                    // a SKIP, not a failure - see AvailabilityGroupReadability for the invariant and the
                    // client case that produced it.
                    //
                    // ⚠ THIS GUARD IS COSMETIC. It is the e57319d artefact, kept on purpose - do not delete
                    // it, and do not believe it protects a verdict. THIS METHOD HAS NO batchErrors LIST and
                    // produces no verdict: RunAsync returns a bare Task, and PreviewIsReadable has nothing
                    // here to read. PROVED 2026-09-14 by IL census over the compiled assembly - neither
                    // exception handler in <RunAsync>d__20 contains a List<string>::Add, while the twin in
                    // <RunScriptCoreAsync>d__33 does (IL 092E). The verdict-bearing guard is the one in
                    // RunScriptCoreAsync; THAT is the fix that works.
                    //
                    // ⚠ An earlier version of this comment claimed the opposite - that passing true "would
                    // add this to batchErrors, and PreviewIsReadable treats ANY batch error as the whole
                    // preview being unreadable". That is the rationale for the guard 592 lines below, not
                    // for this one, and attaching it here is exactly how e57319d passed human review while
                    // changing nothing. Source placement cannot tell a verdict-bearing filter from a
                    // cosmetic one; only an IL census can. If you are about to move or remove this, run
                    // the census first (worked example: Tests/SQLTriage.Tests/I1ConfigStoreAnalyzer.cs).
                    //
                    // WHY IT STAYS despite being cosmetic: without it, the single-instance lane would print
                    // a red ERROR for the identical condition the multi-instance lane treats as a skip that
                    // still yields a verdict. Deleting it buys nothing and costs that agreement.
                    //
                    // It is still PRINTED and still names the database (via the server's own text),
                    // because an unreported skip is its own defect.
                    onMessage($"[batch {batchNo}] {AvailabilityGroupReadability.DescribeSkip(ex)} Server said: {ex.Message}", false);
                    _logger.LogInformation(ex, "Server config script batch {Batch} skipped: database not readable on this AG replica", batchNo);
                }
            ...
            RollbackScriptCapture? rollback = null)
        {
            ...
            foreach (var batch in batches)
            {
                ...
                catch (DbException ex) when (ex is SqlException agSql && AvailabilityGroupReadability.IsNotReadableOnThisReplica(agSql))
                {
                    // ⚠ THIS is the verdict-bearing path. The catch below populates batchErrors, and
                    // PreviewIsReadable treats ANY entry there as "the whole preview is unreadable",
                    // so one unreadable database withholds the verdict for every database that DID
                    // read. A non-readable AG replica must therefore be reported WITHOUT landing
                    // there: still printed, still named by the server's own text, but not an error.
                    //
                    // ⚠ IF YOU ADD AN AG GUARD, ADD IT HERE. The sibling guard in RunAsync is
                    // COSMETIC: RunAsync has no batchErrors list and no verdict gate, and its only
                    // consumer uses the isError flag to pick a text colour. Proved 2026-09-12 by an
                    // exception-handling census over the COMPILED assembly - RunAsync filter=1,
                    // RunScriptCoreAsync filter=0 - after the first attempt at this fix guarded the
                    // wrong method and changed nothing the client could see. Both surfaces that
                    // withhold a verdict (RunPreviewAsync, RunPreviewForInstanceAsync) route here.
                    var skip = $"[batch {batchNo}] {AvailabilityGroupReadability.DescribeSkip(agSql)} Server said: {ex.Message}";
                    onMessage?.Invoke(skip, false);
                    _logger.LogInformation(ex, "Server config script batch {Batch} skipped: not readable on this AG replica ({Mode})", batchNo, apply ? "apply" : "preview");
                }
