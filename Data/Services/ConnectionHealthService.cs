...
namespace SQLTriage.Data.Services
{
    ...
    public class ConnectionHealthService : IDisposable
    {
        ...
        // B1 (2026-07-13, ruling R-B1): canonical server identity, keyed by whatever
        // connection alias the user entered (".", "localhost", an IP, a DNS name).
        // Value = SERVERPROPERTY('ServerName'), probed once per alias alongside the
        // existing capability probe below. Two aliases that point at the SAME physical
        // instance resolve to the identical canonical value — live-verified 2026-07-13:
        // ".", "localhost", and the machine name all returned canonical "MSI" for the
        // default instance via sqlcmd. Consumers key their estate-aggregate dictionaries
        // on this value instead of the raw alias so a re-aliased server isn't double
        // counted; the alias itself is untouched everywhere else (display, navigation,
        // connection-status lookups all still use the alias).
        private readonly ConcurrentDictionary<string, string> _canonicalName =
            new(StringComparer.OrdinalIgnoreCase);

        /// <summary>
        /// Resolves a connection alias to its canonical server identity
        /// (SERVERPROPERTY('ServerName')), or returns the alias unchanged if it hasn't
        /// been probed yet (first few seconds after startup, or a breaker-skipped server).
        /// Safe by construction: an unresolved alias just behaves as its own canonical key,
        /// so no consumer can regress below current (alias-keyed) behaviour.
        /// </summary>
        public string ResolveCanonical(string serverName) =>
            _canonicalName.TryGetValue(serverName, out var canonical) && !string.IsNullOrWhiteSpace(canonical)
                ? canonical
                : serverName;
        ...
        private async Task CheckAllAsync(CancellationToken ct = default)
        {
            ...
            // Micro-nit fix (2026-07-13): a deleted server's last-known status otherwise
            // persists in _status forever, so OnlineCount (surfaced as "N online" in NavMenu)
            // never decrements when a server is removed from /servers. Prune any tracked key
            // that's no longer in the current enabled server set before notifying subscribers
            // — NavMenu already subscribes to OnStatusChanged, so this self-heals within one
            // poll interval with no new event needed.
            foreach (var staleKey in _status.Keys)
            {
                if (!currentServers.Contains(staleKey))
                    _status.TryRemove(staleKey, out _);
            }
        ...
        private async Task CheckServerAsync(ServerConnection conn, string serverName, CancellationToken outerCt)
        {
            ...
            // Invariant B-prime (2026-09-19): our own fault is not evidence about the server. Building
            // the connection string is OUR code (a stored credential that will not decrypt, a missing
            // SQL-auth user name) and asks the server nothing, so a failure here neither counts against
            // the breaker nor marks the server Offline: it is Unknown, with the reason kept. It is done
            // BEFORE the try below because that try's catch must stay broad (see there).
            string connStr;
            try
            {
                connStr = conn.GetConnectionString(serverName, "master") +
                          ";Connect Timeout=5;Application Name=SQLTriage-HealthCheck";
            }
            catch (Exception ex)
            {
                var before = GetStatus(serverName);
                _status[serverName] = new HealthEntry(ServerStatus.Unknown, DateTime.UtcNow, ex.Message);
                if (before.Status != ServerStatus.Unknown || before.Error != ex.Message)
                    _logger.LogWarning("Health check could not be attempted for {Server}: {Error}. This is not evidence the server is down", serverName, ex.Message);
                return;
            }
            ...
            try
            {
                ...
                // (Azure detection), ProductMajorVersion, Edition, and ServerName
                // (B1 canonical identity — see _canonicalName above).
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = @"
                            SELECT
                                ...
                                CAST(SERVERPROPERTY('ServerName') AS nvarchar(256)) AS CanonicalServerName";
                        ...
                        if (await rdr.ReadAsync(cts.Token))
                        {
                            ...
                            canonicalName = rdr.IsDBNull(3) ? null : rdr.GetString(3);
                        }
                        _isAzure[serverName] = ee == 5 || ee == 8;

                        if (!string.IsNullOrWhiteSpace(canonicalName))
                            _canonicalName[serverName] = canonicalName;
                        ...
                        // B1: log the alias→canonical resolution at Information so a live-probe
                        // can confirm two aliases for the same instance land on one canonical key.
                        _logger.LogInformation("B1 canonical identity: alias '{Alias}' → canonical '{Canonical}'",
                            serverName, canonicalName ?? "(unresolved)");
                    }
                    catch (Exception ex)
                    {
            ...
            {
                // The only server call left in the try above is OpenAsync, the login: the SERVERPROPERTY
                // probe has its own catch and needs no permission. So everything that lands here is a
                // failed login, and a failed login stays a breaker failure and stays Offline (ruled
                // 2026-09-19: a wrong or expired credential retried every poll can lock an Active
                // Directory account, and the breaker's back-off is what prevents that). The catch stays
                // broad ON PURPOSE: CancelAfter(6 s) turns a HUNG server into an
                // OperationCanceledException, a non-SQL exception that IS a genuine no-answer, so a
                // "non-SQL means ours" rule here would blind the breaker to hung servers. The cause goes
                // to the breaker, whose shared classifier finds nothing answered in a login failure.
                _breaker?.RecordFailure(serverName, ex);
