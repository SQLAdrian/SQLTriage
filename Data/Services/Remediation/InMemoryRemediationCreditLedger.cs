...
namespace SQLTriage.Data.Services.Remediation
{
    public sealed class InMemoryRemediationCreditLedger : IRemediationCreditLedger
    {
        ...
        /// <summary>
        /// Never damaged: this ledger has no file to fail to read. Stated rather than inherited so
        /// a test using it cannot accidentally exercise the damaged path and think it proved
        /// something about the persisted one.
        /// </summary>
        public bool IsStoreDamaged => false;

        /// <summary>Nothing to recover — there is no store on disk.</summary>
        public string DescribeStoreRecovery() => string.Empty;

        /// <summary>
        /// Post-hoc credit-back after a CONFIRMED undo (Phase 3). This ledger decrements the balance
        /// at reserve time and keeps spend implicit, so giving credits back IS putting them back on
        /// the balance — the same motion <see cref="Grant"/> makes, deliberately reusing it rather
        /// than opening a second way to move this number.
        /// </summary>
        public bool TryCreditBack(string serverName, int credits)
        {
            if (string.IsNullOrWhiteSpace(serverName) || credits <= 0) return false;
            Grant(serverName, credits);
            return true;
        }
