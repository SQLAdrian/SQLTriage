...
namespace SQLTriage.Data.Services.Remediation
{
    ...
    public sealed class RemediationPreview
    {
        ...
        /// <summary>
        /// True when the preview could not be taken because the target is not READABLE from here --
        /// today, a non-readable availability-group replica. It is a SKIP, not a fault: the answer
        /// will be identical on every retry until someone changes the AG's configuration.
        ///
        /// <para><b>It is deliberately paired with <see cref="Succeeded"/> = false, never true.</b>
        /// Succeeded=false is what already stops every approval surface from offering an Apply
        /// button, and what already renders the reason in amber rather than the red reserved for a
        /// gate refusal. A skip marked Succeeded=true would render <see cref="WhatIfText"/> and an
        /// approve button on the backup and CHECKDB panels -- a proposed change against a database
        /// nobody could read.</para>
        ///
        /// <para><b>Why a field and not a prose match.</b> The reason sentence lives in
        /// <see cref="Error"/>, but a consumer must not have to parse it: the approval page (Pages/Remediation.razor:1296) already decides
        /// whether to offer the force-install tick by string-matching an error
        /// (<c>MaintenanceSolutionOpRenderer.LooksLikePartialInstallRefusal</c>), and that is the
        /// coupling this field exists to avoid -- the same argument that put
        /// <see cref="CanRollBack"/> here, so a surface can style on the fact and a test can assert
        /// the fact rather than the wording.</para>
        ///
        /// <para>Default false means every consumer that ignores it behaves exactly as before.
        /// Nothing is required to read it; it only ever NARROWS an existing failure.</para>
        /// </summary>
