    ...
    public sealed class DbatoolsRemediationExecutor : IRemediationExecutor
    {
            ...
            string connString, RemediationRequest request, CancellationToken ct)
        {
                ...
                && RemediationOpRenderer.TryRender(t.Operation, oldValue.Value, out var rollbackSql, out _))
            {
                ...
                // This site already read the server back rather than assuming. It now routes that
                // read through the shared helper so all rollback confirmations agree on what a
                // thrown read means, and so no future edit can reintroduce the fail-open default.
                var (cfgState, cfgError) = await ConfirmRollbackAsync(
                    async token => TryParseInt(await ScalarAsync(connString, readSql, token).ConfigureAwait(false)) == oldValue.Value,
                    $"Rollback did not restore '{t.Operation.ConfigName}' to {oldValue.Value}.", ct).ConfigureAwait(false);
                return new RemediationExecution
                {
                    Outcome = RemediationOutcome.AppliedVerifyFailed,
                    Error = verifyError,
                    RollbackState = cfgState,
                    RollbackError = cfgError
                };
            }
