...
namespace SQLTriage.Data
{
    ...
    public class QuickCheckStateService : IDisposable
    {
        ...
        /// <summary>
        /// Targets this run ATTEMPTED, not targets it measured. The counter behind it increments
        /// after both catch blocks close (Pages/QuickCheck.razor), so a server that threw is
        /// counted identically to one that succeeded.
        ///
        /// <para>Ruled 2026-08-28 (pages-r1-12): the count is not a fabricated value, because
        /// <see cref="DescribeRunCompleteness"/> is computed from the same run and states how many
        /// planned servers produced no results at all, on the page and on both PDF paths. What was
        /// wrong was the word: the page said "tested" and the PDF chip said "SERVERS". Both now say
        /// attempted. Anything that renders this number must describe it as an attempt.</para>
        /// </summary>
