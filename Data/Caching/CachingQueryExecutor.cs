...
namespace SQLTriage.Data.Caching
{
    ...
    public class CachingQueryExecutor
    {
                        ...
                        case "DeltaStatCard":
                            {
                                ...
                                // strings-r2-03: publish only a reading that is actually there. What
                                // stood here read row[0] by ORDINAL and substituted `double val = 0`
                                // for a DBNull, an unparsable string or a table with no columns at
                                // all - see TryReadCachedStat for the whole account.
                                if (TryReadCachedStat(dt, out var val))
                                {
                                    statResults[panel.Id] = new StatValue
                                    {
                                        // The live path fills these from the panel too
                                        // (DynamicDashboard.razor). Leaving them empty made a
                                        // preloaded card render its number without the unit that says
                                        // what the number is.
                                        Label = panel.Title,
                                        Value = val,
                                        Unit = panel.StatUnit ?? string.Empty
                                    };
                                }
                                break;
                            }
        ...
        /// <summary>
        /// Reads a stat card's number out of a cached row, or reports that there is none. The single
        /// rule: <b>publish a reading only when one is present.</b> Absence stays absence.
        ///
        /// <para>WHY THIS EXISTS (strings-r2-03, 2026-08-28). The cached pre-load used to declare
        /// <c>double val = 0</c> and overwrite it only if the first column parsed, then publish the
        /// StatValue unconditionally. So a panel whose SQL returns NULL - which two shipped panels did,
        /// <c>pmemory.buffer_pool</c> and <c>livetempdb.used_mb</c>, both fixed in this same commit -
        /// rendered a confident <b>0</b>. The live path
        /// (Components/Shared/DynamicDashboard.razor) does the opposite for the same panel: it
        /// publishes nothing and logs "returned no data or NULL value". Two code paths, one panel,
        /// opposite answers, and the 0 is the one a client sees first.</para>
        ///
        /// <para>AND IT IS BADGED AS FRESH. DynamicDashboard marks a preloaded panel with
        /// <c>_cachedPanelIds</c>, and clears that mark per panel as each finishes its live fetch -
        /// while <c>_statResults</c> is only swapped for the live dictionary once EVERY panel has
        /// finished. Between those two moments the fabricated 0 is on screen with its cache badge
        /// already removed. "0 MB of buffer pool" reads as a measurement, and a blank tile reads as
        /// nothing wrong; neither is true, and the first is worse.</para>
        ///
        /// <para>THREE DEFECTS, NOT ONE. (1) The DBNull substitution above. (2) It read
        /// <c>row[0]</c> by ORDINAL where the live path reads <c>["Value"]</c> by NAME, so a cached
        /// table whose first column is a label published the label's failed parse - another 0. (3) It
        /// parsed with the CURRENT CULTURE. SqliteCacheStore serialises every cell to an
        /// invariant JSON number and deserialises every cell back as a STRING, so on a machine whose
        /// locale uses a comma decimal separator <c>double.TryParse("532.53")</c> returns 53253:
        /// not a fabricated zero but a fabricated reading three orders of magnitude out. All three are
        /// closed here, and all three are pinned by CachedStatHonestyTests.</para>
        ///
        /// <para>Returns false, with <paramref name="value"/> 0, for: a null table, no rows, no
        /// <c>Value</c> column, a DBNull cell, or a cell that is not a number. The caller publishes
        /// nothing in every one of those cases.</para>
        /// </summary>
        internal static bool TryReadCachedStat(DataTable? dt, out double value)
        {
            value = 0;
            if (dt is null || dt.Rows.Count == 0) return false;
            if (!dt.Columns.Contains("Value")) return false;

            var cell = dt.Rows[0]["Value"];
            if (cell is null || cell == DBNull.Value) return false;

            // The SQLite tier hands every cell back as a string written with an invariant separator;
            // the hot tier hands back the original CLR type. Both must land on the same number.
            ...
            else
            {
                try
                {
                    value = Convert.ToDouble(cell, System.Globalization.CultureInfo.InvariantCulture);
                }
                catch (Exception)
                {
                    return false;                              // not a number, so not a reading
                }
            }

            // "NaN" and "Infinity" both round-trip through the invariant parse, and neither is a
            // measurement. A stat card cannot render them and must not pretend it has a number.
