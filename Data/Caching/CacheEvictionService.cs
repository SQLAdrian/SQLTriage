...
namespace SQLTriage.Data.Caching
{
    ...
    public class CacheEvictionService : IDisposable
    {
        ...
        private async Task RunPressureEvictionAsync()
        {
            ...
            try
            {
                // Genuinely aggressive: under pressure keep only the last 2 hours and squeeze the
                // cache to half its normal size cap. The routine pass already evicts >24h, so the
                // previous 24h call here was a no-op against recent bloat (which is what causes pressure).
                await _cache.EvictOlderThanAsync(TimeSpan.FromHours(2));
                await _cache.EnforceSizeLimitAsync(_maxCacheSizeBytes / 2);
                _logger.LogWarning("Aggressive cache eviction (last 2h, half size cap) triggered due to memory pressure");
            }
            catch (Exception ex)
            {
