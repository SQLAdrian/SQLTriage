...
namespace SQLTriage.Data
{
    ...
    public class AuditLogService : IDisposable
    {
        ...
        /// <summary>
        /// Stable surface identifiers for an exec-guard refusal on an UNATTENDED surface, so an
        /// auditor can filter "every scheduled task the guard refused" without matching on prose.
        /// </summary>
        public static class ExecSurfaces
        {
            /// <summary>A scheduled SqlQuery task refused at its tick, before the server was contacted.</summary>
            public const string ScheduledTaskRun = "exec.scheduled-task-run";
            /// <summary>A scheduled SqlQuery task refused at save, before it was ever persisted or scheduled.</summary>
            public const string ScheduledTaskSave = "exec.scheduled-task-save";
        }

        /// <summary>
        /// Records a statement the dangerous-exec guard REFUSED on an unattended surface — a scheduled
        /// task, which runs on a timer with no operator present. Journaled at
        /// <see cref="AuditSeverity.Critical"/> and the refused text is kept in full: on an unattended
        /// surface the audit row is often the ONLY place a human will ever see that something tried.
        /// The exec record and the toast carry the same reason, but both are scoped to one task; this
        /// row is the one an auditor can find without knowing which task to open.
        /// </summary>
        public void LogUnattendedExecBlocked(string surface, string taskName, string serverName, string statement, string reason)
        {
            Enqueue(new AuditLogEntry
            {
                EventType = AuditEventType.SecurityBlock,
                Severity = AuditSeverity.Critical,
                Message = $"Scheduled '{taskName}' from {surface} on '{serverName}' was BLOCKED by the dangerous-exec guard: {reason}",
                Details = new Dictionary<string, string>
                {
                    ["Surface"] = surface,
        ...
        /// <summary>
        /// Records the START or the END of the bootstrap-admin posture — the state in which access
        /// control is switched off and every caller from this machine is therefore served as a full
        /// administrator (RbacService.PostureKind.Off).
        ///
        /// <para>⚠ WHY IT IS AN AUDIT EVENT AT ALL. Adrian's ruling of 2026-09-06 accepts the
        /// posture and requires it to be LOUD: an installed service has no screen, so an auditor
        /// reading this log after the fact is the only reader who can see when an install began
        /// serving every local caller as a full administrator, and whether that window was later
        /// CLOSED by enforcement. A log line at WARNING is the operator's
        /// channel; this is the auditor's, and it is chained and HMAC'd like every other entry.</para>
        ///
        /// <para>⚠ THE SENTENCES ARE HANDED IN, NOT COMPOSED HERE. <paramref name="headline"/> and
        /// <paramref name="detail"/> come from <c>RbacService.DescribeEnforcementPosture</c>, the
        /// single register the banner and the log already share, so this entry cannot claim a state
        /// the service did not establish. The end entry in particular names the posture the install
        /// moved TO rather than asserting "access control is now configured".</para>
        ///
        /// <para>⚠ THE END ENTRY DELIMITS THE WINDOW ONLY WHEN ENFORCEMENT CLOSED IT. <paramref
        /// name="enforcedNow"/> is true only when the install moved Off -> Enforcing, and only then
        /// does the entry say the bootstrap-admin posture "ended". Off -> Lapsed and Off -> a damaged
        /// config (ConfigDidNotLoad) leave the machine serving every request as though access control
        /// were off: the wide-open window has NOT closed, only changed cause, so the entry says so and
        /// names the un-enforced destination rather than claiming an end. Calling either an end would
        /// be the same over-claim the enforcement banner shipped seven rounds running, and it would
        /// tell the auditor above that the exposure was bounded when it was not.</para>
        ///
        /// <para>Called once per TRANSITION (RbacService dedupes on posture kind + reasons), never
        /// per request and never per render.</para>
        /// </summary>
        public void LogBootstrapAdminPosture(bool active, string trigger, string headline, string detail, bool enforcedNow = false)
        {
            Enqueue(new AuditLogEntry
            {
                ...
                Message = active
                    ? "bootstrap-admin posture active: loopback callers are full admin until access control is configured"
                    : enforcedNow
                        ? "bootstrap-admin posture ended. " + headline
                        : "bootstrap-admin posture no longer applies, but access control is STILL NOT ENFORCED: the wide-open window has NOT closed. " + headline,
                Details = new Dictionary<string, string>
                {
                    ["Trigger"] = trigger ?? string.Empty,
                    ["Posture"] = headline ?? string.Empty,
                    ["Scope"] = detail ?? string.Empty,
                    ["Machine"] = Environment.MachineName
                }
            });
        }
