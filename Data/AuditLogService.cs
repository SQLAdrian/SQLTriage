    ...
    public class AuditLogService : IDisposable
    {
        ...
        public void LogRemediationRolledBack(string templateKey, string serverName, bool success, string? errorMessage = null)
        {
            Enqueue(new AuditLogEntry
            {
                ...
                Message = success
                    ? $"Remediation '{templateKey}' on '{serverName}' rolled back"
                    : $"Rollback of remediation '{templateKey}' on '{serverName}' FAILED: {errorMessage}",
                Details = new Dictionary<string, string>
                {
                    ["TemplateKey"] = templateKey,
                    ["ServerName"] = serverName,
        ...
        /// <para>
        /// Recorded at Info: a receipt arriving is the NORMAL daily outcome, and filing routine
        /// success as a warning is how a severity-triaged reader learns to ignore the category.
        /// </para>
        /// </summary>
            ...
            string day, string headSignature, int entryCount, string observerSignature, string observedAtUtc)
        {
            Enqueue(new AuditLogEntry
            {
                ...
                Severity = AuditSeverity.Info,
                Message = "An independent observer's signed receipt for the chain head of " + day +
                          " was verified and recorded here. It shows that head existed off this box on " +
                          "that day, so a later rewrite of this chain would disagree with a value already " +
                          "held elsewhere. It says nothing about entries written since, and nothing about " +
                          "the window before the head was first recorded.",
                Details = new Dictionary<string, string>
                {
                    ["Day"] = day,
