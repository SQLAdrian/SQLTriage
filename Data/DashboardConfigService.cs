...
namespace SQLTriage.Data
{
    ...
    /// When the file is missing or damaged it falls back to the 27-dashboard layout EMBEDDED IN THIS
    /// BUILD and writes nothing; <see cref="DefaultConfigGenerator"/>'s three-dashboard stub is the last
    /// resort and reaching it means the embedded resource is absent, which is a BUILD defect.
    ...
    public class DashboardConfigService
    {
        ...
        //
        // ⚠ 2026-09-12 (lane I1-census): THE TWO FALLBACKS BELOW USED TO BE
        // `DefaultConfigGenerator.Generate()`, AND THAT WAS A SECOND WAY TO SERVE THE STUB SILENTLY.
        // Neither is a reachable "the file is fine" answer, and both reported one. A file holding the
        // JSON literal `null` is valid JSON, so no exception fires, so Load()'s handler never ran: the
        // `??` handed back the 3-dashboard stub, Load() took its SUCCESS branch, set
        // Origin = ConfigOrigin.OperatorFile — a FALSE origin, the operator's file being served is not
        // the operator's file — logged at Debug, and every later save wrote the stub over the real
        // layout. The absent-path `??` was the same shape in a race: Load() tests File.Exists and this
        // method tests it again, so a delete between the two produced a stub reported as OperatorFile.
        //
        // Both now THROW instead, which routes into the handler this method's own comment insists on:
        // the unreadable file is copied to .corrupt.<timestamp>.json, an Error is logged, the fallback
        // is the EMBEDDED SHIPPED 27-dashboard layout, Origin says ShippedDefaultsAfterUnreadableFile,
        // and nothing is written. Pinned by DashboardConfigAbsentFileTests.
        // A_config_holding_the_json_literal_null_is_left_alone_and_never_reported_as_the_operators.
        //
        private DashboardConfigRoot LoadConfigFromDisk()
        {
            ...
            if (!File.Exists(configPath))
                throw new FileNotFoundException(
                    "Neither the dashboard config nor its backup is present at read time. The caller "
                    + "tested for the file before calling, so reaching here means it was removed in "
                    + "between. That is an absence, never a config.", configPath);

            using var stream = File.OpenRead(configPath);
            return JsonSerializer.Deserialize<DashboardConfigRoot>(stream, SerializerOptions)
                   ?? throw new JsonException(
                       $"{configPath} is valid JSON but deserialised to null (the file holds the literal "
                       + "`null`). There is no configuration in it, so it is damage and is reported as "
                       + "damage rather than answered with built-in defaults.");
        }
        ...
        public DashboardConfigService(ILogger<DashboardConfigService> logger)
            : this(logger,
                   Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Config", "dashboard-config.json"),
                   Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Config", "dashboard-config.backup.json"),
                   watchForChanges: true)
        {
        }

        /// <summary>
        /// The DISK constructor with its paths supplied — the production one delegates to it. Added
        /// 2026-09-11 (lane seeder-stub-freeze) because the absent-file path had <b>no test seam at all</b>:
        /// the only way to reach <see cref="Load"/> was through a constructor that hard-coded
        /// <c>AppDomain.CurrentDomain.BaseDirectory</c>, so the behaviour this lane changes could only ever
        /// be asserted by reading the source. It runs the same two steps in the same order as production
        /// (migrator, then load) so what the tests exercise is the production path, not a copy of it.
        /// <paramref name="watchForChanges"/> is the one difference: a test must not leave a
        /// FileSystemWatcher running on a temp folder it is about to delete.
        /// </summary>
        internal DashboardConfigService(ILogger<DashboardConfigService> logger,
                                        string configPath,
                                        string backupPath,
                                        bool watchForChanges)
        {
            ...
            _config = Load();
            if (watchForChanges) SetupFileWatcher();
        }
        ...
        private void SetupFileWatcher()
        {
            try
            {
                // Derived from _configPath rather than recomputed from BaseDirectory (2026-09-11): the two
                // were the same string in production and only the same BY COINCIDENCE, so a seam that moved
                // the config path would have left the watcher pointed at the old folder.
        ...
        {
            // Compared against the bound path's own file name rather than a retyped literal, for the same
            // reason the watched folder is (2026-09-11): the literal and _configPath agreed only by
            // coincidence, and a seam that moved the path would have watched for the wrong name.
            if (e.Name?.Equals(Path.GetFileName(_configPath), StringComparison.OrdinalIgnoreCase) == true)
            {
        ...
        /// <summary>Where the configuration this service is serving actually came from. Exposed because
        /// "somebody must be told" has to be inspectable, not only log-shaped.</summary>
        public enum ConfigOrigin
        {
            /// <summary>Read from the file on disk. The normal case, and the only one that is the
            /// operator's.</summary>
            OperatorFile,

            /// <summary>The file was not there. Running on the shipped dashboard config embedded in this
            /// assembly — the right layout, but not the operator's file.</summary>
            ShippedDefaultsFromAssembly,

            /// <summary>The file was there and did not parse. Running on the embedded shipped config; the
            /// unreadable file has been copied aside and is untouched.</summary>
            ShippedDefaultsAfterUnreadableFile,

            /// <summary><b>The worst case.</b> Neither the file nor the embedded shipped config could be
            /// read, so this is <see cref="DefaultConfigGenerator"/>'s stub — three dashboards where the
            /// product ships 27. An absent or empty embedded resource is a BUILD defect, never a skip.</summary>
            BuiltInGeneratorStub
        }

        /// <summary>How <see cref="Load"/> resolved the configuration currently in memory.</summary>
        public ConfigOrigin Origin { get; private set; } = ConfigOrigin.OperatorFile;

        /// <summary>
        /// Loads configuration from disk. When the file is absent or does not parse, falls back — and
        /// <b>WRITES NOTHING</b>.
        ///
        /// <para><b>WHAT CHANGED ON 2026-09-11 AND WHY (lane seeder-stub-freeze, RULED by Adrian).</b>
        /// This method used to end with <c>if (!File.Exists(_configPath)) { Save(); }</c>, which PERSISTED
        /// <see cref="DefaultConfigGenerator"/>'s stub — three dashboards where the product ships 27, a
        /// measured 100,274 bytes against the shipped 481,763 — into <c>config\dashboard-config.json</c>.
        /// Two things then made it permanent. <see cref="ConfigDefaultsSeeder"/> is create-if-absent, so
        /// every later seeding pass found the stub present and recorded <c>dashboard-config.json:Kept</c>;
        /// and <see cref="Save"/> copies the previous file to <c>dashboard-config.backup.json</c> ONLY if
        /// one exists, so on the absent path there was no backup by construction. A packaging slip became
        /// a permanent three-dashboard product with no trace of the 27 anywhere on the install.</para>
        ///
        /// <para><b>THE RULING: on absence, never persist; run on the best available default; warn loud.
        /// Do NOT refuse to start</b> — turning a packaging slip into a customer outage is the worse
        /// failure.</para>
        ///
        /// <para><b>"BEST AVAILABLE DEFAULT" IS THE EMBEDDED SHIPPED CONFIG, NOT THE GENERATOR.</b>
        /// <c>SQLTriage.csproj</c> embeds <c>Config\dashboard-config.json</c> as a resource, and
        /// <see cref="DashboardConfigMigrator.ReadShippedDefaults"/> already read it one line before this
        /// method wrote the wrong thing — the correct artefact was in the assembly the whole time. So the
        /// fallback is the real 27-dashboard layout; the generator stub is the last resort, and reaching it
        /// means the embedded resource is absent or unreadable, which is a BUILD defect and is said as
        /// one.</para>
        ///
        /// <para>The pins are <c>Tests\SQLTriage.Tests\DashboardConfigAbsentFileTests.cs</c>, which
        /// exercise this method through the internal disk constructor rather than reading the source.</para>
        /// </summary>
        private DashboardConfigRoot Load()
        {
            if (File.Exists(_configPath))
            {
                try
                {
                    ...
                    if (config != null)
                    {
                        ...
                        _config = config;
                        Origin = ConfigOrigin.OperatorFile;
            ...
            // ── THE FILE IS NOT THERE, OR IT WAS THERE AND DID NOT PARSE ──────────────────────────
            var fileIsAbsent = !File.Exists(_configPath);
            var shipped = TryReadShippedConfigFromAssembly(out var shippedDetail);

            if (shipped != null)
            {
                _config = shipped;
                Origin = fileIsAbsent
                    ? ConfigOrigin.ShippedDefaultsFromAssembly
                    : ConfigOrigin.ShippedDefaultsAfterUnreadableFile;
                RebuildQueryCache(shipped);

                _logger.LogError(
                    "[DashboardConfig] {Path} {State}. The dashboards are being served from the layout "
                    + "EMBEDDED IN THIS BUILD ({Dashboards} dashboard(s)). That is the shipped layout, but NOT your "
                    + "file. NOTHING HAS BEEN WRITTEN: no file has been created at that path and no backup "
                    + "has been taken, so whatever is or is not there is exactly as it was. If you had tuned "
                    + "your dashboards, that tuning is not being read. Restore the file, or re-extract the "
                    + "release to let first-run seeding create it from {DefaultsFolder}\\.",
                    _configPath,
                    fileIsAbsent ? "does not exist" : "exists and could not be read",
                    shipped.Dashboards?.Count ?? 0,
                    Services.ConfigDefaultsSeeder.DefaultsFolderName);

                return shipped;
            }

            // ── LAST RESORT. Reaching here means the embedded shipped config is absent or unreadable,
            // which is a defect in THIS BUILD rather than in the install — ABSENT OR EMPTY IS A FAILURE,
            // never a skip, so it is named as a build defect and not logged as a normal fallback.
            var defaultConfig = DefaultConfigGenerator.Generate();
            _config = defaultConfig;
            Origin = ConfigOrigin.BuiltInGeneratorStub;
            RebuildQueryCache(defaultConfig);

            _logger.LogError(
                "[DashboardConfig] {Path} {State} AND the dashboard layout embedded in this build could not "
                + "be read ({Detail}). That second half is a BUILD DEFECT, not an install problem. Falling "
                + "back to the built-in generator, which produces {Count} dashboard(s) where this product "
                + "ships 27, so most of the product's dashboards are simply not present in this process. "
                + "NOTHING HAS BEEN WRITTEN. Report this build.",
                _configPath,
                fileIsAbsent ? "does not exist" : "exists and could not be read",
                shippedDetail,
                defaultConfig.Dashboards.Count);

            // ⚠ THE Save() THAT USED TO BE HERE IS GONE ON PURPOSE, and it is the whole point of the lane.
            // Writing a built-in default to the operator's path is the silent replacement this method
            // existed to stop. Re-adding it, on any branch, re-opens it — DashboardConfigAbsentFileTests
            // goes red on exactly that mutation.
            return defaultConfig;
        }

        /// <summary>
        /// The shipped dashboard layout embedded in this assembly, or null with a reason. The resource is
        /// the same <c>Config\dashboard-config.json</c> the payload ships, so this is the real
        /// 27-dashboard layout rather than a reconstruction of it.
        ///
        /// <para><b>Null is returned for absent, empty, unparseable AND for a parse that yields no
        /// dashboards.</b> A config with zero dashboards would render an empty product while reporting
        /// success, which is the fail-toward-clean direction this codebase has been bitten by.</para>
        /// </summary>
        internal static DashboardConfigRoot? TryReadShippedConfigFromAssembly(out string detail)
        {
            string? text;
            try
            {
                text = DashboardConfigMigrator.ReadShippedDefaults();
            }
            catch (Exception ex)
            {
                detail = "the embedded resource threw " + ex.GetType().Name;
                return null;
            }

            if (string.IsNullOrWhiteSpace(text))
            {
                detail = "no embedded " + DashboardConfigMigrator.ShippedResourceSuffix + " resource, or it is empty";
                return null;
            }

            try
            {
                var config = JsonSerializer.Deserialize<DashboardConfigRoot>(text, SerializerOptions);
                if (config?.Dashboards is null || config.Dashboards.Count == 0)
                {
                    detail = "the embedded resource parsed but declares no dashboards";
                    return null;
                }
                detail = "read " + config.Dashboards.Count + " dashboard(s) from the embedded resource";
                return config;
            }
            catch (Exception ex)
            {
                detail = "the embedded resource did not parse: " + ex.GetType().Name;
                return null;
            }
        }
