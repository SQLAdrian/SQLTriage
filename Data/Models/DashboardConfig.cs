...
namespace SQLTriage.Data.Models
{
    ...
    public class DashboardDefinition
    {
        ...
        /// <summary>
        /// Every panel this dashboard owns, across BOTH shapes: the flat <see cref="Panels"/> list
        /// (single-page dashboards) and every <see cref="Tabs"/> entry's panels (tabbed dashboards,
        /// e.g. memory's Overview/Analysis split). Unlike <see cref="GetActivePanels"/>, this is not
        /// scoped to one tab — used by nav-visibility, which must see an opt-out panel regardless of
        /// which tab it lives on.
        /// </summary>
        public IEnumerable<PanelDefinition> AllPanels()
        {
    ...
    public class PanelDefinition
    {
        ...
        /// <summary>
        /// Opts this panel into RETAINED history (DECISIONS 2026-08-26 18:23 ruling 4). Only meaningful
        /// for <c>TimeSeries</c> panels. When true, every sample is also written, downsampled, to the
        /// durable <c>metric_history</c> table, which outlives the cache, the 24-hour eviction sweep and
        /// a process restart — so the panel can show a real trend and the Baseline compare has a real
        /// 7-day-ago window to draw.
        ///
        /// <para>Default false, and that is a deliberate bound, not an oversight: retention costs disk
        /// on a client's production server. Turning it on for every TimeSeries panel would breach the
        /// cache size cap. See <c>MetricRetentionOptions</c> for the arithmetic.</para>
        /// </summary>
        [JsonPropertyName("retainHistory")]
        ...
        /// <summary>
        /// Panel-level override of the dashboard's <see cref="DashboardDefinition.RequiresDatabase"/>.
        /// <c>null</c> (unset) inherits the dashboard's requirement — the panel is gated exactly like
        /// the rest of the dashboard. <c>""</c> (explicit empty string) opts the panel OUT: it needs
        /// no database precondition even when the dashboard needs one, so it renders and loads on
        /// installs that don't have that database (e.g. a native panel living inside an otherwise
        /// PerfMon-gated dashboard). A distinct explicit db name is accepted by the schema but is not
        /// independently resolved — it currently behaves the same as an unset (inherited) value.
        /// </summary>
        [JsonPropertyName("requiresDatabase")]
        public string? RequiresDatabase { get; set; }
