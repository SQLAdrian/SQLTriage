...
namespace SQLTriage.Data.Scheduling
{
    ...
    public sealed class QueryResult
    {
        ...
        /// <summary>
        /// True only when the ORCHESTRATOR failed the request before its <see cref="QueryRequest.Work"/>
        /// ran: a global or per-server concurrency slot it could not grant within the request's
        /// timeout. Set at the origin, in QueryOrchestrator, and nowhere else.
        ///
        /// <para><b>INVARIANT B-prime (lane alert-correctness, 2026-09-19): our own fault is not
        /// evidence about the server.</b> Such a failure is about OUR resources, so a caller must not
        /// count it against the server it was going to query (the alert loop used to record a
        /// circuit-breaker failure for it). A COMMAND timeout is different: it happens inside the
        /// work, arrives as a SqlException, and stays a server failure. This flag distinguishes the
        /// two by where the failure happened, not by the exception's type name. Default false, so a
        /// result built anywhere else keeps its old meaning. Census:
        /// QueryOrchestratorSlotTimeoutTests.</para>
        /// </summary>
        public bool FailedBeforeWorkStarted { get; init; }
    }
