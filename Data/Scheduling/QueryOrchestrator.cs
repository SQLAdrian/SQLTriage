...
namespace SQLTriage.Data.Scheduling
{
    ...
    public sealed class QueryOrchestrator : IQueryOrchestrator, IDisposable
    {
        ...
        // "Orchestrator:GlobalConcurrency" / ":PerServerConcurrency" are absent from the shipped
        // appsettings.json, so these constants are what actually governs a default install. The
        // Settings tuning card reads the same constants — before that it defaulted to 5/3, so
        // opening the card and saving silently cut the engine to a fraction of these limits.
        /// <summary>Queries in flight across all servers when the config key is absent.</summary>
        public const int DefaultGlobalConcurrency = 45;
        /// <summary>Queries in flight against one server when the config key is absent.</summary>
        public const int DefaultPerServerConcurrency = 13;
        ...
        public QueryOrchestrator(ILogger<QueryOrchestrator> logger, IConfiguration configuration, QueryRegistry registry)
        {
            ...
            _globalConcurrency = configuration.GetValue<int>("Orchestrator:GlobalConcurrency", DefaultGlobalConcurrency);
            _perServerConcurrency = configuration.GetValue<int>("Orchestrator:PerServerConcurrency", DefaultPerServerConcurrency);
        ...
        private async Task ExecuteWorkAsync(QueuedRequest queued)
        {
            ...
            var globalWait = await WaitForSlotAsync(_globalSemaphore, timeout, token, req.CancellationToken);
            if (globalWait != SlotWait.Granted)
            {
                CompleteWithoutRunning(queued, sw, globalWait, "Global concurrency slot unavailable within timeout.");
                return;
            }
            ...
            {
                var serverWait = await WaitForSlotAsync(sem, timeout, token, req.CancellationToken);
                if (serverWait != SlotWait.Granted)
                {
                    ReleaseServerSemaphores(serverSemaphores);
                    _globalSemaphore.Release();
                    Interlocked.Decrement(ref _inFlightCount);
                    CompleteWithoutRunning(queued, sw, serverWait, "Per-server concurrency slot unavailable within timeout.");
                    return;
                }
            }
        ...
        private enum SlotWait { Granted, TimedOut, Cancelled }

        /// <summary>
        /// Waits for one concurrency slot, and ALWAYS comes back with a verdict.
        ///
        /// <para><b>The defect this closes (lane alert-correctness, 2026-09-19, MEASURED on the
        /// build-4082 binary).</b> The wait was <c>WaitAsync(timeout, token)</c>, where
        /// <c>token</c> already carries a CancellationTokenSource armed with the SAME timeout and
        /// started a moment EARLIER. So when no slot came free, the token usually fired first and
        /// <c>WaitAsync</c> THREW OperationCanceledException instead of returning false. Nothing
        /// caught it: ExecuteWorkAsync died before setting the request's completion, and the caller's
        /// <c>await EnqueueAsync</c> never returned. Twelve runs with one slot held: 10 never
        /// completed, 1 produced the TimeoutException result, 1 got the slot.
        /// QueryOrchestratorSlotTimeoutTests reproduces the shape against this method.</para>
        ///
        /// <para>A cancellation that came from the request's own timeout is the same event as the
        /// timed-out return, a slot we could not grant, and is reported the same way. A cancellation
        /// from the caller or from shutdown is a cancellation. Census:
        /// QueryOrchestratorSlotTimeoutTests.</para>
        /// </summary>
        private async Task<SlotWait> WaitForSlotAsync(
            SemaphoreSlim slot, TimeSpan timeout, CancellationToken linked, CancellationToken caller)
        {
            try
            {
                return await slot.WaitAsync(timeout, linked) ? SlotWait.Granted : SlotWait.TimedOut;
            }
            catch (OperationCanceledException) when (!caller.IsCancellationRequested && !_cts.IsCancellationRequested)
            {
                return SlotWait.TimedOut;   // the request's own timeout fired first
            }
            catch (OperationCanceledException)
            {
        ...
        /// <summary>Completes a request whose work never ran. A slot we could not grant is marked
        /// <see cref="QueryResult.FailedBeforeWorkStarted"/> at this, its only origin (invariant
        /// B-prime: our own fault is not evidence about the server the work would have queried).</summary>
        private void CompleteWithoutRunning(QueuedRequest queued, System.Diagnostics.Stopwatch sw, SlotWait wait, string timeoutMessage)
        {
            if (wait == SlotWait.Cancelled)
            {
                queued.Completion.TrySetCanceled();
                RecordLatency(queued, CreateResult(queued, sw.Elapsed, null, success: false));
                return;
            ...
            var result = CreateResult(queued, sw.Elapsed, new TimeoutException(timeoutMessage), failedBeforeWorkStarted: true);
            queued.Completion.TrySetResult(result);
            RecordLatency(queued, result);
        }
        ...
        private static QueryResult CreateResult(QueuedRequest queued, TimeSpan duration, Exception? ex, bool? success = null,
            bool failedBeforeWorkStarted = false)
        {
            ...
            return new QueryResult
            {
                ...
                Duration = duration,
                Exception = ex,
                FailedBeforeWorkStarted = failedBeforeWorkStarted
            };
        }
