        ...
        public static List<ServerConnection> Import(string json, string passphrase)
        {
            ...
            foreach (var entry in bundle.Servers ?? [])
            {
                var conn = new ServerConnection
                {
                    ...
                    // Fall back to the model default (not a hard-coded 15) so it tracks ServerConnection.
                    ConnectionTimeout = entry.ConnectionTimeout > 0 ? entry.ConnectionTimeout : new ServerConnection().ConnectionTimeout,
                };
