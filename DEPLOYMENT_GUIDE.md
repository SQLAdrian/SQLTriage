...
The production certificate is a Certum **"Open Source Code Signing in the Cloud"** cert. Its
private key is HSM-resident and **cannot be exported — there is no `.pfx`**, and CI can never
sign. The certificate is identified by its **SHA-1 thumbprint** in `Cert:\CurrentUser\My`, and
signing requires **SimplySign Desktop running and logged in**. Full runbook:
`.handoff/CODESIGN_LAUNCH_CHECKLIST.md`.

To enable signed updates:

1. **Embed the public key** (one-time, and again only if the cert changes). Reads only the
   public half, so no SimplySign session is needed:
   ```powershell
   .\tools\extract-update-pubkey.ps1 -Thumbprint <sha1-thumbprint>
   ```
...
2. **Rebuild** so the key is embedded into the assembly (it is embedded at *compile* time —
   skipping this ships the old/placeholder key and every client rejects the update):
   ```powershell
   dotnet build SQLTriage.csproj -c Release
   ```

3. **Publish and sign the release** (signs `SQLTriage.exe` and the installer via signtool,
   produces the detached `<zip>.sig`, and verifies all three before shipping):
   ```powershell
   .\publish-release.ps1 -SigningThumbprint <sha1-thumbprint>
   ```
   `signtool` (Windows SDK) is located automatically — it is **not** on `PATH` on the release
   machine. The legacy `-SigningPfx <path> -SigningPassword <pwd>` still works for file-based
   test certificates.
