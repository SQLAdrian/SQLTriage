...
- **Read-only by default** — the write paths are separate, opt-in pages an operator has to open and confirm; nothing writes during an assessment, on a schedule, or on connect. They are: Deploy SQLWATCH and Deploy Darling PerformanceMonitor (each creates its own database, objects and extended event sessions), Best Practice scripts, and the XEvents page (creates / starts / drops event sessions). No-Pants mode gates the dangerous ones and Experimental Mode gates previews — both are UI confirmation gates, not authentication.
