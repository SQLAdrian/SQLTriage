...
[![Latest release](https://img.shields.io/github/v/release/SQLAdrian/SQLTriage?sort=semver)](https://github.com/SQLAdrian/SQLTriage/releases/latest)
[![Downloads](https://img.shields.io/github/downloads/SQLAdrian/SQLTriage/total)](https://github.com/SQLAdrian/SQLTriage/releases)
[![Windows](https://img.shields.io/badge/Windows-x64-0078D6?logo=windows)](https://github.com/SQLAdrian/SQLTriage/releases/latest)

**The audit your CIO can actually defend.** Free, open-source, agentless-by-default SQL Server audit and compliance assessment — 500+ checks, every one of them free, most of them mapped to 30+ compliance frameworks including NIST SP 800-53, CIS, DISA STIG, ISO 27001, SOC 2, PCI-DSS and HIPAA.

The open alternative to Microsoft SQL Vulnerability Assessment, with deeper coverage. Every check names where its rule comes from. Every control is traceable.
...
A board-ready governance score (Bronze → Silver → Gold → Platinum), a 5-category breakdown, and licensing-cost visibility — generated from real DMV reads on real servers — the assessment installs no agent and nothing on the target.
...
| Maturity roadmap mapping most findings to the framework controls auditors care about. Per-server progress tracking + multi-server comparison strip. | 195 sp_BLITZ findings imported and grouped in a single grid with priority/category badges. No key, no signup. |

**Everything in the download is free.** The complete check corpus — 500+ checks, most of them framework-mapped — the maturity roadmap, governance scoring, and first-class sp_BLITZ CSV import. No key, no signup, no paywalled checks, use it indefinitely.
...
| Audit-grade checks | **500+ (all free)** | 331 (measured 2026-09) | 87 (measured 2026-09) | - |
| Framework mapping (NIST / CIS / STIG / ISO / SOC 2 / PCI / HIPAA) | ✅ | ❌ | partial | ❌ |
| Maturity scoring (Bronze → Platinum) | ✅ | ❌ | ❌ | ❌ |
| Named source per check | ✅ | partial | ❌ | ❌ |
...
- **Agentless by default** — every check is a SELECT against DMVs. Running an assessment installs nothing on the SQL Server and changes no schema.
- **Read-only by default** — the write paths are separate, opt-in pages an operator has to open and act on; nothing writes during an assessment, on a schedule, or on connect. They are: Server Configuration & Hardening, Best Practice scripts, Remediation, the Extended Events page (creates / starts / stops / drops event sessions), the query plan viewer's index-create button, and a dashboard panel's action cell (no shipped panel defines one). Each states its own gate in the full write-paths table; they are not all the same. The dashboard action cell asks for a per-action confirmation, Remediation asks for its own confirmation on the fixes it marks not reversible, and the index-create button asks for none. No-Pants mode gates the dangerous ones and Experimental Mode gates previews — both are safety interlocks, not authentication.
- **Audit-grade** - every check names where its rule comes from. 401 of 523 link to the published source. The report your auditor reads is the same report your DBA reads.
...
- **Unsigned today (signing planned)** — Windows/AV may warn on first run; here's [why it happens and how to verify the download](https://sqladrian.github.io/SQLTriage/security-and-antivirus) (open source, SHA-256 provenance, VirusTotal-clean, no telemetry).
